...
async function main() {
  ...
  try {
    ...
    console.log('\n=== Campaign Items ===');
    const items = await db.prepare('SELECT id, campaign_id, tts_batch_id, ffmpeg_status, upload_status, drive_link, result_json FROM pillar_campaign_items WHERE campaign_id = ?').all(campaignId);
