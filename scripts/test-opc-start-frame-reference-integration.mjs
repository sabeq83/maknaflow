...
await tenantContext.run('default_tenant', async () => {
  ...
  console.log(JSON.stringify({ campaign_id:campaign.id, item_id:item.id, clip_index:clipIndex, reference_source:initial.audit.reference_source_field, reference_sha256:initial.audit.reference_sha256, request_fingerprint:initial.audit.request_fingerprint }, null, 2));
});
