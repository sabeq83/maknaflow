...
await tenantContext.run('default_tenant', async () => {
  const productId = 'pe_sync_1781148697786_850';
  const campaign = (await pgQuery(`SELECT * FROM pillar_campaigns WHERE tenant_id=$1 AND target_product_id=$2 AND visual_mode='hybrid_lock' ORDER BY created_at DESC LIMIT 1`, ['default_tenant', productId])).rows[0];
  assert.ok(campaign, 'Campaign Product OPC Dev tidak tersedia.');
  const product = (await pgQuery(`SELECT id,product_name,active_photo,generated_photo_url,clean_photo_url,cleaned_photo_url,photo_url,raw_photo_url FROM product_extractions WHERE tenant_id=$1 AND id=$2`, ['default_tenant', productId])).rows[0];
  assert.ok(product, 'Fixture produk Omura tidak tersedia.');
  assert.equal(product.active_photo, 'generated_photo_url');
  assert.ok(product.clean_photo_url);
  assert.notEqual(product.clean_photo_url, product.generated_photo_url);
  ...
  console.log(JSON.stringify({ campaign_id:campaign.id, item_id:item.id, product_id:product.id, clean_photo_url:product.clean_photo_url, clip_index:clipIndex, non_bridge_clip_index:nonBridgeClipIndex, reference_source:initial.audit.reference_source_field, reference_sha256:initial.audit.reference_sha256, reference_byte_length:payloadMetadata.byteLength, request_fingerprint:initial.audit.request_fingerprint }, null, 2));
});
