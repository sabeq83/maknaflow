...
export async function processProductionJob(jobId) {
  ...
  try {
    ...
    if (job.job_kind === 'voiceover') {
      ...
      const voiceProvider = episode?.voice_provider || 'google_tts';
      const voicePersona = episode?.voice_persona || 'Orus';
      const ext = voiceProvider === 'minimax' ? 'mp3' : 'wav';

      const filename = `yt_vo_${pkg.id}_scene_${asset.scene_index}.${ext}`;
      ...
      outputAssetJson = {
        ...
        selected_voice_speed: Number(episode?.voice_speed ?? 1.0)
      };
