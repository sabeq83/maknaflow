...
export function validateSceneScript(script, blueprint, targetDuration, context = {}) {
  ...
  const manifest = validateSpeakerManifest(actualScript.speaker_manifest || []);
  const manifestIds = manifest.map(item => item.speaker_id);
  if (new Set(manifestIds).size !== manifestIds.length) throw new Error('Speaker manifest contains duplicate speaker IDs');
  if (Array.isArray(context.allowedSpeakerIds)) {
    for (const speakerId of manifestIds) {
      if (!context.allowedSpeakerIds.includes(speakerId)) {
        const error = new Error(`Speaker "${speakerId}" is not registered in the active casting roster.`);
        error.code = 'YT_UNKNOWN_SPEAKER';
        throw error;
      }
    }
  }
  ...
  actualScript.scenes.forEach(s => {
    ...
    if (actualScript.schema_version === 2) {
      ...
      const allowedBlockTypes = ['narration', 'dialogue', 'whisper', 'breath', 'sound_event', 'ambience', 'silence'];
      s.audio_blocks.forEach((ab, abIdx) => {
        if (!ab.block_id) throw new Error(`Scene ${currentIdx} block ${abIdx + 1} missing block_id`);
        if (!allowedBlockTypes.includes(ab.type)) throw new Error(`Scene ${currentIdx} block ${ab.block_id} has invalid type ${ab.type}`);
        const spoken = ['narration', 'dialogue', 'whisper'].includes(ab.type);
        if (spoken && !ab.speaker_id) throw new Error(`Scene ${currentIdx} block ${ab.block_id} missing speaker_id`);
        if (spoken && (!ab.text || !ab.text.trim())) throw new Error(`Scene ${currentIdx} block ${ab.block_id} text is empty`);
        if (ab.type === 'sound_event' && !(ab.text || ab.source || ab.action)) throw new Error(`Scene ${currentIdx} block ${ab.block_id} sound event is empty`);
        if (ab.type === 'silence' && !(Number(ab.duration_seconds || ab.estimated_duration_seconds) > 0)) throw new Error(`Scene ${currentIdx} block ${ab.block_id} silence duration is invalid`);
        if (spoken && Array.isArray(context.allowedSpeakerIds) && !context.allowedSpeakerIds.includes(ab.speaker_id)) {
          const error = new Error(`Speaker "${ab.speaker_id}" is not registered in the active casting roster.`);
          error.code = 'YT_UNKNOWN_SPEAKER';
          throw error;
        }
        if (spoken && !manifestIds.includes(ab.speaker_id)) throw new Error(`Speaker "${ab.speaker_id}" is missing from the speaker manifest`);
      });
    } else {
  ...
  if (context.audioExperience) {
    const blocks = actualScript.scenes.flatMap(scene => scene.audio_blocks || []);
    const hasSpoken = blocks.some(block => ['narration', 'dialogue', 'whisper'].includes(block.type));
    const hasWhisper = blocks.some(block => block.type === 'whisper');
    const hasSoundEvent = blocks.some(block => block.type === 'sound_event');
    if (context.audioExperience === 'no_talking_asmr' && hasSpoken) throw new Error('no_talking_asmr strictly forbids spoken blocks');
    if (context.audioExperience === 'spoken_asmr' && !hasWhisper) throw new Error('spoken_asmr requires at least one whisper block');
    if (context.audioExperience === 'mixed_asmr' && (!hasSpoken || !hasSoundEvent)) throw new Error('mixed_asmr requires spoken and sound_event blocks');
  }
  return script;
}
