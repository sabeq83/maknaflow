...
PRINSIP INTEGRASI PRODUK NATURAL & ROTASI KATALOG:
- Produk klien harus digunakan secara nyata dan masuk akal dalam aksi memasak (misal: susu oat dituang saat frothing, bubuk cocoa dilarutkan, blender digunakan untuk menghaluskan buah/es).
- JANGAN memaksakan slot iklan terpisah (affiliate insert terpisah) dan JANGAN mengarang klaim yang tidak masuk akal.
- ROTASI WAJIB: Terdapat ${productsSnapshot.length || 1} produk dalam katalog. Kamu WAJIB merotasikan seluruh produk tersebut secara proporsional ke dalam ${count} ide resep yang dibuat. Setiap produk harus mendapatkan porsi tampil sebagai primary_product_id.
...
DAFTAR PRODUK KATALOG TERSEDIA (${productsSnapshot.length} Produk - Rotasikan Semua):
