...
export function getPgPool() {
  if (!pool) {
    const rawSearchPath = process.env.PG_SEARCH_PATH || 'public';
    const schemas = rawSearchPath.split(',').map(s => s.trim()).filter(Boolean);
    ...
    const migrateContentPlannerDualMode = async () => {
      try {
        ...
        console.log('[PostgreSQL] Content Planner dual-mode & archive migration completed.');
      } catch (err) {
    ...
    // Recipe Campaign support (Recipe Planner, Lineage & Full Recipe ContentFlow)
    const migrateRecipeCampaignSupport = async () => {
      try {
        await pool.query(`
          ALTER TABLE content_planners
        ...
        await pool.query(`
          ALTER TABLE content_planner_rows
            ...
            ALTER COLUMN category_cep DROP NOT NULL,
            ALTER COLUMN ws_matrix DROP NOT NULL,
            ALTER COLUMN context DROP NOT NULL,
            ALTER COLUMN vfo DROP NOT NULL,
            ALTER COLUMN strategic_angle DROP NOT NULL,
        ...
        await pool.query(`
          ALTER TABLE pillar_campaign_items
            ...
            ADD COLUMN IF NOT EXISTS recipe_revision INTEGER DEFAULT 1;
        `);
        console.log('[PostgreSQL] Recipe Campaign migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Recipe Campaign migration:', err.message);
      }
    };
    migrateRecipeCampaignSupport();

    // AI Token Usage & Cost Meter Ledger
    const migrateAiTokenUsageLedger = async () => {
      try {
        await pool.query(`
          ...
          CREATE INDEX IF NOT EXISTS idx_ai_token_ledger_tenant_date ON ai_token_usage_ledger (tenant_id, created_at DESC);
          CREATE INDEX IF NOT EXISTS idx_ai_token_ledger_feature ON ai_token_usage_ledger (feature_name);
          CREATE INDEX IF NOT EXISTS idx_ai_token_ledger_model ON ai_token_usage_ledger (model_name);
        `);
        console.log('[PostgreSQL] AI Token Usage Ledger migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] AI Token Usage Ledger migration:', err.message);
      }
    };
    migrateAiTokenUsageLedger();

    // Hermes Research to Content Planner Lineage & Row Evidence Migration (Tahap 1)
    const migrateContentPlannerResearchAndEvidence = async () => {
      try {
        await pool.query(`
          ALTER TABLE content_planners
            ADD COLUMN IF NOT EXISTS research_revision_id TEXT,
            ADD COLUMN IF NOT EXISTS research_snapshot_sha256 TEXT,
            ...
            ADD COLUMN IF NOT EXISTS research_source_policy TEXT,
            ADD COLUMN IF NOT EXISTS research_schema_version TEXT;
        `);
        ...
        await pool.query(`
          CREATE TABLE IF NOT EXISTS content_planner_row_evidence (
            ...
            CONSTRAINT uq_cpr_evidence UNIQUE (tenant_id, planner_id, planner_row_id)
          );
        `);

        await pool.query(`CREATE INDEX IF NOT EXISTS idx_cpr_evidence_planner ON content_planner_row_evidence (tenant_id, planner_id);`);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_cpr_evidence_revision ON content_planner_row_evidence (tenant_id, research_revision_id);`);
        console.log('[PostgreSQL] Content Planner Research & Evidence migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Content Planner Research & Evidence migration failed:', err.message);
      }
    };
    migrateContentPlannerResearchAndEvidence();
    ...
    pool.query(`
      ALTER TABLE pillar_campaigns
        ...
        ADD COLUMN IF NOT EXISTS clip_duration INTEGER DEFAULT 8;
      ALTER TABLE re_campaigns
        ADD COLUMN IF NOT EXISTS clip_duration INTEGER DEFAULT 8;
      ALTER TABLE sheets_campaigns
        ADD COLUMN IF NOT EXISTS clip_duration INTEGER DEFAULT 8;
      ALTER TABLE recipe_campaigns
        ADD COLUMN IF NOT EXISTS clip_duration INTEGER DEFAULT 8;
    `).catch(err => {
      console.warn('[PostgreSQL Auto-Migration Warning] Gagal migrasi demographic & clip_duration:', err.message);
    });
      ...
      try {
        ...
        await migrationClient.query(`
          ...
          ALTER TABLE content_automation_schedules
            ...
            ADD COLUMN IF NOT EXISTS execution_mode TEXT NOT NULL DEFAULT 'recurring';
      ...
      try {
        ...
        await migrationClient.query(`
          ...
          ALTER TABLE pillar_campaign_items
            ...
            ADD COLUMN IF NOT EXISTS generation_progress TEXT;
        ...
        await migrationClient.query(`
          ...
          CREATE INDEX IF NOT EXISTS pillar_campaign_item_assets_isolation_idx
            ON pillar_campaign_item_assets(tenant_id,campaign_item_id,reference_critical,status,next_attempt_at);
          ...
          CREATE TABLE IF NOT EXISTS glabs_image_lane_leases (
            id TEXT PRIMARY KEY,
            tenant_id TEXT NOT NULL,
          ...
          CREATE INDEX IF NOT EXISTS glabs_image_lane_active_idx ON glabs_image_lane_leases (lane_key, status, lease_expires_at);
          CREATE INDEX IF NOT EXISTS glabs_image_lane_waiter_idx ON glabs_image_lane_leases (lane_key, status, requested_at);

          UPDATE glabs_image_lane_leases
          SET status = 'expired', released_at = CURRENT_TIMESTAMP, release_reason = 'wait_abandoned_startup_cleanup'
          WHERE status = 'waiting' AND requested_at < CURRENT_TIMESTAMP - INTERVAL '5 minutes';
          ...
          ALTER TABLE opc_start_frame_request_audits
            ADD COLUMN IF NOT EXISTS reference_name TEXT,
            ADD COLUMN IF NOT EXISTS reference_position INTEGER,
            ADD COLUMN IF NOT EXISTS reference_mime_type TEXT,
            ADD COLUMN IF NOT EXISTS reference_byte_length INTEGER,
            ADD COLUMN IF NOT EXISTS requested_model TEXT,
            ADD COLUMN IF NOT EXISTS effective_model TEXT,
            ADD COLUMN IF NOT EXISTS lane_key TEXT,
            ADD COLUMN IF NOT EXISTS lane_mode TEXT,
            ADD COLUMN IF NOT EXISTS lane_wait_started_at TIMESTAMPTZ,
            ADD COLUMN IF NOT EXISTS lane_acquired_at TIMESTAMPTZ,
            ADD COLUMN IF NOT EXISTS sibling_active_count_at_submit INTEGER,
            ADD COLUMN IF NOT EXISTS lifecycle_status TEXT NOT NULL DEFAULT 'prepared',
            ADD COLUMN IF NOT EXISTS provider_submitted_at TIMESTAMPTZ,
    ...
    const migrateAffiliateStudioContentPlannerConnection = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_affiliate_studio_content_planner_connection_v1'));`);
        ...
        console.log('[PostgreSQL] Affiliate Studio Content Planner Connection migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Affiliate Studio Content Planner Connection migration:', err.message);
      } finally {
        if (client) { try { await client.query('SELECT pg_advisory_unlock_all()'); } catch(_) {} client.release(); }
      }
    };
    migrateAffiliateStudioContentPlannerConnection();

    const migrateAffiliateStudioProductionVisibility = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_affiliate_studio_production_visibility_v1'));`);
        ...
        console.log('[PostgreSQL] Affiliate Studio Production Visibility migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Affiliate Studio Production Visibility migration:', err.message);
      } finally {
        if (client) { try { await client.query('SELECT pg_advisory_unlock_all()'); } catch(_) {} client.release(); }
      }
    };
    migrateAffiliateStudioProductionVisibility();

    const migrateAffiliateStudioCreativeIntelligence = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_affiliate_studio_creative_intelligence_v1'));`);

        await client.query(`
          ALTER TABLE affiliate_programs ADD COLUMN IF NOT EXISTS target_demographic TEXT;
        `);
        await client.query(`
          ALTER TABLE affiliate_programs ADD COLUMN IF NOT EXISTS ai_directive TEXT;
        `);
        await client.query(`
          ALTER TABLE affiliate_programs ADD COLUMN IF NOT EXISTS mandatory_outro_line TEXT;
        `);

        console.log('[PostgreSQL] Affiliate Studio Creative Intelligence migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Affiliate Studio Creative Intelligence migration:', err.message);
      } finally {
        if (client) { try { await client.query('SELECT pg_advisory_unlock_all()'); } catch(_) {} client.release(); }
      }
    };
    migrateAffiliateStudioCreativeIntelligence();

    const migrateAffiliateStudioPerformanceFoundation = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_affiliate_studio_performance_foundation_v1'));`);

        await client.query(`
          CREATE TABLE IF NOT EXISTS affiliate_performance_snapshots (
            id TEXT PRIMARY KEY,
            tenant_id TEXT NOT NULL,
            brand_profile_id TEXT NOT NULL,
            affiliate_program_id TEXT NOT NULL REFERENCES affiliate_programs(id) ON DELETE CASCADE,
            affiliate_content_run_id TEXT REFERENCES affiliate_content_runs(id) ON DELETE SET NULL,
            ...
            status TEXT NOT NULL CHECK (status IN ('measured', 'partial', 'stale', 'unavailable')),
            snapshot_date TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
            created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
          );
        `);
        await client.query(`
          CREATE INDEX IF NOT EXISTS idx_aff_perf_snapshots_lookup
            ON affiliate_performance_snapshots (tenant_id, brand_profile_id, affiliate_program_id);
        `);

        console.log('[PostgreSQL] Affiliate Studio Performance Foundation migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Affiliate Studio Performance Foundation migration:', err.message);
      } finally {
        if (client) { try { await client.query('SELECT pg_advisory_unlock_all()'); } catch(_) {} client.release(); }
      }
    };
    migrateAffiliateStudioPerformanceFoundation();

    const migrateAffiliateStudioWorkflowV2Lineage = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_affiliate_studio_workflow_v2_lineage_v1'));`);
        ...
        await client.query(`
          CREATE TABLE IF NOT EXISTS affiliate_content_lineage (
            ...
            content_schedule_id TEXT,
            content_planner_id TEXT,
            planner_row_id TEXT,
            affiliate_content_run_id TEXT,
            publishing_queue_id TEXT,
            external_post_id TEXT,
            lifecycle_stage TEXT NOT NULL DEFAULT 'calendar'
              CHECK (lifecycle_stage IN ('calendar', 'planner', 'approved', 'production', 'ready', 'publishing', 'published', 'measured', 'archived')),
            metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
            created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
          );
        `);
        await client.query(`
          CREATE INDEX IF NOT EXISTS idx_aff_lineage_lookup
            ON affiliate_content_lineage (tenant_id, brand_profile_id, lifecycle_stage);
          CREATE INDEX IF NOT EXISTS idx_aff_lineage_schedule
            ON affiliate_content_lineage (tenant_id, content_schedule_id);
          CREATE INDEX IF NOT EXISTS idx_aff_lineage_planner_row
            ON affiliate_content_lineage (tenant_id, planner_row_id);
          CREATE INDEX IF NOT EXISTS idx_aff_lineage_run
            ON affiliate_content_lineage (tenant_id, affiliate_content_run_id);
        `);
        ...
        await client.query(`
          CREATE TABLE IF NOT EXISTS affiliate_production_scene_projection (
            ...
            duration_sec NUMERIC(5,2) DEFAULT 3.0,
            ...
            t2i_prompt TEXT,
            i2v_engine TEXT,
            i2v_motion_prompt TEXT,
            audio_asset_url TEXT,
            image_asset_url TEXT,
            video_asset_url TEXT,
            scene_status TEXT NOT NULL DEFAULT 'pending'
              CHECK (scene_status IN ('pending', 'generating', 'rendered', 'error')),
            metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
            created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
            UNIQUE (tenant_id, affiliate_content_run_id, scene_index)
          );
        `);
        await client.query(`
          CREATE INDEX IF NOT EXISTS idx_aff_scene_proj_run
            ON affiliate_production_scene_projection (tenant_id, affiliate_content_run_id, scene_index);
        `);

        console.log('[PostgreSQL] Affiliate Studio Workflow V2 Lineage migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Affiliate Studio Workflow V2 Lineage migration:', err.message);
      } finally {
        if (client) { try { await client.query('SELECT pg_advisory_unlock_all()'); } catch(_) {} client.release(); }
      }
    };
    migrateAffiliateStudioWorkflowV2Lineage();
    ...
    const migratePublishingScheduler = async () => {
      ...
      try {
        ...
        await client.query(`
          ...
          CREATE INDEX IF NOT EXISTS publishing_media_staging_expires_idx
            ON publishing_media_staging (expires_at) WHERE deleted_at IS NULL;
        `);
    ...
    const migrateContentFlowPublishingColumns = async () => {
      ...
      try {
        ...
        // Auto-heal ContentFlow items that have live permalinks or successful publishing_jobs
        await client.query(`
          UPDATE content_flow_items
          SET facebook_status = 'Published'
          WHERE facebook_status = 'Not Published' 
            AND (
              (permalink_facebook IS NOT NULL AND permalink_facebook <> '')
              OR id IN (SELECT content_id FROM publishing_jobs WHERE platform = 'facebook' AND status = 'published')
              OR video_id IN (SELECT content_id FROM publishing_jobs WHERE platform = 'facebook' AND status = 'published')
            );
        `);
        console.log('[PostgreSQL] ContentFlow publishing columns migration completed (tiktok/youtube/threads/linkedin & auto-heal).');
      } catch (err) {
      ...
      try {
        ...
        const runSafeQuery = async (sql) => {
          try {
            ...
            console.warn(`[PostgreSQL Auto-Migration Safe Warning] ${e.message}`);
          }
        };
        ...
        await runSafeQuery(`ALTER TABLE re_deconstructed_assets ALTER COLUMN created_at TYPE TIMESTAMPTZ USING created_at AT TIME ZONE 'UTC';`);
        await runSafeQuery(`ALTER TABLE re_deconstruct_batches ALTER COLUMN created_at TYPE TIMESTAMPTZ USING created_at AT TIME ZONE 'UTC';`);
        await runSafeQuery(`CREATE INDEX IF NOT EXISTS re_deconstruct_assets_library_idx ON re_deconstructed_assets (tenant_id, status, niche, created_at DESC);`);
        await runSafeQuery(`CREATE INDEX IF NOT EXISTS re_deconstruct_assets_queue_idx ON re_deconstructed_assets (tenant_id, batch_id, status, created_at);`);
    ...
    const migrateMultiplierTasksTenantAndBatch = async () => {
      ...
      try {
        ...
        // Check if relation is an ordinary table ('r')
        const relCheck = await client.query(`
          SELECT relkind FROM pg_class c
          JOIN pg_namespace n ON n.oid = c.relnamespace
          WHERE n.nspname = current_schema() AND c.relname = 're_multiplier_tasks';
        `);
        const relkind = relCheck.rows[0]?.relkind;

        if (relkind === 'r') {
          const cols = [
            `ALTER TABLE re_multiplier_tasks ADD COLUMN IF NOT EXISTS tenant_id TEXT NOT NULL DEFAULT 'default_tenant';`,
            `ALTER TABLE re_multiplier_tasks ADD COLUMN IF NOT EXISTS batch_id TEXT;`,
            `ALTER TABLE re_multiplier_tasks ADD COLUMN IF NOT EXISTS target_product_id TEXT;`,
            `ALTER TABLE re_multiplier_tasks ADD COLUMN IF NOT EXISTS row_index INTEGER;`,
            `ALTER TABLE re_multiplier_tasks ADD COLUMN IF NOT EXISTS product_snapshot_json JSONB;`,
            `ALTER TABLE re_multiplier_tasks ADD COLUMN IF NOT EXISTS error_message TEXT;`,
            `ALTER TABLE re_multiplier_tasks ADD COLUMN IF NOT EXISTS t2i_images_json TEXT;`,
            `ALTER TABLE re_multiplier_tasks ADD COLUMN IF NOT EXISTS paused_previous_status TEXT;`,
          ...
          for (const colSql of cols) {
            try {
              ...
              console.warn('[PostgreSQL Auto-Migration Safe Warning]', colErr.message);
            }
          }
        }
    ...
    // ── YouTube Studio Voice Speed (Fase 3.5C) ──
    const migrateYouTubeStudioVoiceSpeed = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_youtube_studio_voice_speed_v1'));`);
        await client.query(`
          ALTER TABLE youtube_episodes ADD COLUMN IF NOT EXISTS voice_speed REAL DEFAULT 1.0;
        `);
        console.log('[PostgreSQL] YouTube Studio Voice Speed migration completed.');
      } catch (e) {
        console.warn('[PostgreSQL Auto-Migration Warning] Voice Speed migration failed:', e.message);
      } finally {
        if (client) {
          try { await client.query('SELECT pg_advisory_unlock(hashtext(\'makna_youtube_studio_voice_speed_v1\'));'); } catch(_) {}
          client.release();
        }
      }
    };
    migrateYouTubeStudioVoiceSpeed();

    // ── YouTube Studio Narration & Duration Sync (Fase 3.5D) ──
    const migrateYouTubeNarrationDurationSync = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_youtube_narration_duration_sync_v1'));`);
        await client.query(`
          ALTER TABLE youtube_episodes ADD COLUMN IF NOT EXISTS narration_profile_key TEXT DEFAULT 'general_id';
          ALTER TABLE youtube_production_packages ADD COLUMN IF NOT EXISTS duration_analysis_json JSONB;
        `);
        console.log('[PostgreSQL] YouTube Studio Narration & Duration Sync migration completed.');
      } catch (e) {
        console.warn('[PostgreSQL Auto-Migration Warning] Narration & Duration Sync migration failed:', e.message);
      } finally {
        if (client) {
          try { await client.query('SELECT pg_advisory_unlock(hashtext(\'makna_youtube_narration_duration_sync_v1\'));'); } catch(_) {}
          client.release();
        }
      }
    };
    migrateYouTubeNarrationDurationSync();

    // ── YouTube Studio Multi-Speaker Narrative (Stage A) ──
    const migrateYouTubeMultiSpeakerNarrative = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_youtube_multi_speaker_narrative_v1'));`);
        await client.query(`
          ALTER TABLE youtube_episodes
        ...
        `);
        console.log('[PostgreSQL] YouTube Studio Multi-Speaker Narrative migration completed.');
      } catch (e) {
        console.warn('[PostgreSQL Auto-Migration Warning] Multi-Speaker Narrative migration failed:', e.message);
      } finally {
        if (client) {
          try { await client.query('SELECT pg_advisory_unlock(hashtext(\'makna_youtube_multi_speaker_narrative_v1\'));'); } catch(_) {}
          client.release();
        }
      }
    };
    migrateYouTubeMultiSpeakerNarrative();

    const migrateYouTubeChannelCharacterVoiceRegistry = async () => {
      let client;
      try {
        client = await pool.connect();
        await client.query(`SELECT pg_advisory_lock(hashtext('makna_youtube_channel_character_voice_registry_v1'));`);
        await client.query(`
          ...
          CREATE UNIQUE INDEX IF NOT EXISTS idx_yt_channel_audio_configs_active
            ON youtube_channel_audio_configs (tenant_id, channel_id, locale)
            WHERE status = 'active';
        ...
        `);
        console.log('[PostgreSQL] YouTube Channel Audio & Speaker Registry migration completed.');
      } catch (e) {
        console.warn('[PostgreSQL Auto-Migration Warning] Channel Audio Registry migration failed:', e.message);
      } finally {
        if (client) {
          try { await client.query('SELECT pg_advisory_unlock(hashtext(\'makna_youtube_channel_character_voice_registry_v1\'));'); } catch(_) {}
          client.release();
        }
      }
    };
    migrateYouTubeChannelCharacterVoiceRegistry();

    const migrateHermesAgentOrchestration = async () => {
      let migrationClient;
      try {
        migrationClient = await pool.connect();
        await migrationClient.query(`SELECT pg_advisory_lock(hashtext('makna_hermes_agent_orchestration_v1'));`);
        await migrationClient.query(`
          CREATE TABLE IF NOT EXISTS agent_automation_runs (
            ...
            research_callback_key TEXT,
            research_policy_json JSONB NOT NULL DEFAULT '{}'::jsonb,
            operator_job_id TEXT,
          ...
          ALTER TABLE agent_automation_runs
            ADD COLUMN IF NOT EXISTS research_callback_key TEXT,
            ADD COLUMN IF NOT EXISTS last_error_code TEXT,
            ADD COLUMN IF NOT EXISTS last_error_message TEXT;
          CREATE UNIQUE INDEX IF NOT EXISTS agent_automation_runs_tenant_idempotency_uq
            ON agent_automation_runs(tenant_id, idempotency_key);
          CREATE INDEX IF NOT EXISTS agent_automation_runs_status_idx
            ON agent_automation_runs(tenant_id, status);
          ...
          ALTER TABLE agent_publishing_intents
          ...
          CREATE INDEX IF NOT EXISTS agent_automation_events_run_idx
            ON agent_automation_events(tenant_id, run_id, created_at DESC);
        `);
        console.log('[PostgreSQL] Hermes Agent Orchestration migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Gagal migrasi Hermes Agent Orchestration:', err.message);
      } finally {
        if (migrationClient) {
          await migrationClient.query(`SELECT pg_advisory_unlock(hashtext('makna_hermes_agent_orchestration_v1'));`).catch(() => {});
          migrationClient.release();
        }
      }
    };
    migrateHermesAgentOrchestration();

    const migrateAffiliateContentSchedules = async () => {
      let migrationClient = null;
      try {
        migrationClient = await pool.connect();
        await migrationClient.query(`SELECT pg_advisory_lock(hashtext('makna_affiliate_content_schedules_v1'));`);
        await migrationClient.query(`
          CREATE TABLE IF NOT EXISTS affiliate_content_schedules (
            id TEXT PRIMARY KEY,
            tenant_id TEXT NOT NULL DEFAULT 'default_tenant',
            plan_type TEXT NOT NULL CHECK (plan_type IN ('brand_editorial', 'product_campaign')),
            brand_name TEXT NOT NULL,
            product_id TEXT,
            product_name TEXT,
            cep_code TEXT,
            pillar_name TEXT,
            promotion_context TEXT,
            target_platforms JSONB NOT NULL DEFAULT '["instagram","tiktok"]'::jsonb,
            target_account_ids JSONB NOT NULL DEFAULT '[]'::jsonb,
            scheduled_at TIMESTAMPTZ NOT NULL,
            status TEXT NOT NULL DEFAULT 'planned' 
              CHECK (status IN ('planned', 'in_production', 'ready_to_schedule', 'scheduled', 'published', 'cancelled')),
            content_flow_id TEXT,
            publishing_job_ids JSONB NOT NULL DEFAULT '[]'::jsonb,
            created_by TEXT,
            brand_profile_id TEXT,
            affiliate_program_id TEXT,
            content_planner_id TEXT,
            dispatch_status TEXT DEFAULT 'draft',
          ...
          CREATE INDEX IF NOT EXISTS idx_affiliate_schedules_date 
            ON affiliate_content_schedules(tenant_id, brand_name, scheduled_at);

          CREATE INDEX IF NOT EXISTS idx_affiliate_schedules_brand_profile
            ON affiliate_content_schedules(tenant_id, brand_profile_id);

          CREATE INDEX IF NOT EXISTS idx_affiliate_schedules_status
            ON affiliate_content_schedules(tenant_id, status);
        `);
        console.log('[PostgreSQL] Affiliate Content Schedules migration completed.');
      } catch (err) {
        console.warn('[PostgreSQL Auto-Migration Warning] Gagal migrasi Affiliate Content Schedules:', err.message);
      } finally {
        if (migrationClient) {
          await migrationClient.query(`SELECT pg_advisory_unlock(hashtext('makna_affiliate_content_schedules_v1'));`).catch(() => {});
          migrationClient.release();
        }
      }
    };
    migrateAffiliateContentSchedules();
    }
  }
  return pool;
}
