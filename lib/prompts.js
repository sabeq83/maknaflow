...
export function sanitizeI2vPrompt(prompt) {
  ...
  // 0. Pre-cleaning: Strip negative prompt, Midjourney tags, and brackets
  safe = safe.replace(/\s*NEGATIVE PROMPT:.*$/i, '');
  safe = safe.replace(/--ar\s+\d+:\d+/gi, '');
  safe = safe.replace(/--no\s+\S+/gi, '');
  ...
  // 4. Check if length exceeds 500 characters
  if (safe.length > 500) {
    const blocks = {};
    const blockRegex = /\((Geometric Truth|Biometric Anchor|Wardrobe|Location|Lens|Camera Move|Lighting|Visual Action|Shot on):\s*([^)]+)\)/gi;
    let match;
    const originalSafe = safe;
    
    while ((match = blockRegex.exec(originalSafe)) !== null) {
    ...
    if (blocks.visual_action && blocks.visual_action.length > 0) {
      // Rebuild compact prompt: Prioritize action (up to first 2 actions to keep it short)
      const action = blocks.visual_action.slice(0, 2).join(', ');
      
      let subject = 'hands';
      if (blocks.biometric_anchor) {
        const bio = blocks.biometric_anchor[0];
        if (bio.toLowerCase().includes('muslimah')) {
          subject = 'Southeast Asian Muslimah hands';
        } else {
          subject = bio.split(',')[0] || 'hands';
        }
      }
      
      let object = '';
      if (blocks.geometric_truth) {
        object = ' with ' + (blocks.geometric_truth[0].split(',')[0] || 'product');
      }
      
      let location = '';
      if (blocks.location) {
        location = ' in ' + (blocks.location[0].split(',')[0] || 'kitchen');
      }
      
      let camera = '';
      if (blocks.camera_move) {
        camera = ', ' + blocks.camera_move[0];
      }
      
      let lighting = '';
      if (blocks.lighting) {
        lighting = ', ' + blocks.lighting[0].split(',')[0];
      }
      
      let rebuilt = `${subject}${object}${location}: ${action}${camera}${lighting}`;
      rebuilt = rebuilt.replace(/\s{2,}/g, ' ').trim();
      
      if (rebuilt.length > 30 && rebuilt.length <= 500) {
        safe = rebuilt;
      } else {
        // If still over 500, hard truncate
        safe = rebuilt.substring(0, 500).trim();
      }
    } else {
      safe = safe.substring(0, 500).trim();
    }
  }
