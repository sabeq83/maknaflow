...
import { getMicroPacingTemplate, getMicroPacingSegmentCount } from './words-per-clip-presets.js';
import { buildVisualIdentityPromptSection } from './visual-identity-allocator.js';

import { resolvePromptInstructions } from './prompt-instructions.js';
import { buildCartoonKBList } from './kb-routing.js';
...
export function buildPromptAgentPrompt(kbTexts, visualStoryboard, audioBlueprint, config) {
  ...
  const clipDuration = Number(config?.clip_duration) || 8;
  ...
  const i2vPromptSchema = isGeminiTts
    ...
    : `"(VERTICAL 9:16) --ar 9:16 --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame: [Filename]), (Consistency: MAX). (Geometric Truth: [MANDATE 50 - Shape & Material Extraction]). [LAYER 2: MICRO-PACING & ACTION (MANDATE 49)] ${getMicroPacingTemplate(clipDuration, true, { hasTransitionLock: true })} [LAYER 3: SFX] SFX: [Insert SFX only - DILARANG naskah vokal/spoken words jika minimax!] "`;
  ...
  "t2v_prompts": [
    {
      ...
      "prompt": "(VERTICAL 9:16) --ar 9:16 --no landscape [LAYER 0: VISUAL TRUTH & ANCHORS] (Geometric Truth: [MANDATE 50 - Shape & Material Extraction]), (Biometric Anchor: [MANDATE 29 - 3-Point Character Lock]). [LAYER 1: SCENE & OPTICS] (Location: [MANDATE 33 - Verbatim Scene Lock]), (Lens: [Camera Spec]), (Camera Move: [Insert Kinetic Logic]). [LAYER 2: MICRO-PACING & ACTION (MANDATE 49)] ${getMicroPacingTemplate(clipDuration, false, { hasTransitionLock: true })} [LAYER 3: SFX] SFX: [Insert SFX]"
    }
  ],
...
// ============================================================================
// TIKTOK SHOP COMPLIANCE & SAFE-VOICE MANDATE (SINGLE-PASS GUARDRAIL)
// ============================================================================
export const TIKTOK_COMPLIANCE_MANDATE = `
TIKTOK SHOP COMPLIANCE & SAFE-VOICE MANDATE (WAJIB DIPATUHI PADA SELURUH KLIP):
1. DILARANG menggunakan klaim medis absolut, klaim mengobati/menyembuhkan penyakit kronis (diabetes, wasir, kanker, kolesterol, dsb).
2. DILARANG menggunakan frasa jaminan instan atau klaim absolut seperti "garansi 100%", "dijamin langsing", "hilang permanen".
3. DILARANG menggunakan klaim tanpa efek samping ("tanpa efek samping", "bebas ketergantungan", "aman tanpa efek").
4. DILARANG menggunakan klaim pseudo-medis seperti "detoks usus kotor", "luntur lemak seketika".
5. Gunakan bahasa edukatif, manfaat bahan alami, testimoni personal wajar, dan gaya bercerita yang aman bagi kebijakan TikTok Shop.
`;
...
export function buildReverseEngineeringPrompt(kbTexts, config) {
  ...
  const clipDuration = Number(config?.clip_duration) || 8;
  const pacingCount = getMicroPacingSegmentCount(clipDuration);
  const microPacingSample = getMicroPacingTemplate(clipDuration, isAudioSegmentEnabled);
  const audioSegmentSample = getMicroPacingTemplate(clipDuration, true, { hasTransitionLock: true });
...
6. AUDIO SEGMENT MANDATE [MANDATE 92 — AKTIF]: Rule #5 di atas DIGANTI oleh mandate ini. Kamu WAJIB menyertakan (Audio Segment: "[Kata yang diucapkan]") secara inline di SETIAP segmen waktu LAYER 2. FORMAT WAJIB LAYER 2 (i2v_prompt & t2v_prompt):
   ...
   DILARANG: Menempatkan seluruh naskah dalam 1 beat saja. Harus dibagi merata ke ${pacingCount} beat.${isMascotMode ? `
...
- Setiap klip video (${clipDuration} detik) WAJIB ada tepat 2 karakter berbeda dari CHARACTER VOICE CAST di atas yang saling berdialog/menjawab. Monolog atau klip berisi hanya 1 karakter/narator tunggal dilarang keras!
- Tentukan otonom kapan masing-masing berbicara: Karakter A berbicara selama ${Math.floor(clipDuration / 2)} detik pertama ([00:00-00:${String(Math.floor(clipDuration / 2)).padStart(2, '0')}]), dibalas oleh Karakter B selama ${clipDuration - Math.floor(clipDuration / 2)} detik berikutnya ([00:${String(Math.floor(clipDuration / 2)).padStart(2, '0')}-00:${String(clipDuration).padStart(2, '0')}]).
...
ATURAN DIALOG (SANGAT KETAT):
- Setiap klip video (${clipDuration} detik) WAJIB ada tepat 2 karakter berbeda yang saling berdialog/menjawab. Monolog atau klip berisi hanya 1 karakter/narator tunggal dilarang keras!
- Tentukan otonom kapan masing-masing berbicara: Karakter A berbicara selama ${Math.floor(clipDuration / 2)} detik pertama ([00:00-00:${String(Math.floor(clipDuration / 2)).padStart(2, '0')}]), dibalas oleh Karakter B selama ${clipDuration - Math.floor(clipDuration / 2)} detik berikutnya ([00:${String(Math.floor(clipDuration / 2)).padStart(2, '0')}-00:${String(clipDuration).padStart(2, '0')}]).
...
Karena kampanye ini dikonfigurasi dalam mode "pure_t2v", maka seluruh klip (Klip 1 hingga ${targetClips}) wajib menggunakan Text-to-Video. Oleh karena itu, isi array "t2v_prompt" untuk seluruh klip tersebut. Anda TIDAK perlu mengisi array "t2i_prompt" and "i2v_prompt".`;
  ...
  if (visualMode === 'hybrid_lock') {
    dynamicPlanFields = `      "new_vo": "Naskah voiceover versi baru yang di-upgrade. Wajib dalam bahasa ${languageName} / ${targetLanguageLabel}.",
      ...
      "i2v_prompt": "Prompt visual I2V (In English, plain text linear, no newlines) wajib mengikuti format terstruktur bertingkat (structured layer) ${isWithoutSfx ? '3' : '4'} segmen (${pacingCount} micro-pacing beats @ 2s, total ${clipDuration}s): (VERTICAL 9:16) --ar 9:16 --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame Reference: ...). [LAYER 2: MICRO-PACING & ACTION] ${microPacingSample}${i2vSfxTemplate}"${voiceSegmentsField}`;
...
6. CRITICAL: Seluruh prompt visual (t2i_prompt & i2v_prompt) WAJIB BEBAS dari tag 'NEGATIVE PROMPT' dan tidak boleh mengandung instruksi negatif apa pun. Deskripsikan murni visual positif yang ingin ditampilkan saja.`;
  } else {
    dynamicPlanFields = `      "new_vo": "Naskah voiceover versi baru yang di-upgrade. Wajib dalam bahasa ${languageName} / ${targetLanguageLabel}.",
      "visual_action": "Deskripsi aksi visual versi baru dalam Bahasa Indonesia sebagai representasi manusiawi dari scene",
      "t2v_prompt": "Prompt T2V (In English, plain text linear, no newlines) wajib mengikuti format terstruktur bertingkat (structured layer) ${isWithoutSfx ? '3' : '4'} segmen (${pacingCount} micro-pacing beats @ 2s, total ${clipDuration}s): (VERTICAL 9:16) --ar ${config.aspect_ratio || '9:16'} --no landscape [LAYER 0: VISUAL TRUTH & ANCHORS] (Biometric Anchor: [Subject Anchor]), (Wardrobe: [Wardrobe Lock]), (Face Visibility: [Details reflecting face_visibility constraint]). [LAYER 1: SCENE & OPTICS] (Location: [Scene Details]), (Lens: [Camera Spec]), (Camera Move: [Kinetic Logic]), (Lighting: [Lighting Mood]). [LAYER 2: MICRO-PACING & ACTION] ${microPacingSample}${t2vSfxTemplate}"${voiceSegmentsField}`;
...
6. CRITICAL: T2V prompt WAJIB BEBAS dari tag 'NEGATIVE PROMPT' dan tidak boleh mengandung instruksi negatif apa pun. Deskripsikan murni visual positif yang ingin ditampilkan saja.`;
  }
...
4. Pencahayaan / Light: ${targetLighting}
5. KONSISTENSI WARDROBE WAJIB: Warna, tekstur, dan pola dari pakaian/wardrobe (${targetWardrobe}) HARUS IDENTIK di seluruh klip. Jangan mengganti warna atau motif pakaian subjek antar-klip. Setiap klip yang menampilkan subjek WAJIB menggunakan wardrobe dengan warna dan tekstur yang PERSIS SAMA.
...
${aiDirective ? `## AI DIRECTIVE / INTERNAL GUARDRAIL\n${aiDirective}\nCRITICAL: Directive ini hanya instruksi kontrol internal. DILARANG mengutip, menyalin, memparafrasekan, atau membacakannya di voiceover, CTA, caption, storyboard, dan prompt visual.\n` : ''}
${vsoSection}
...
${mandatoryOutroLine ? `- Khusus pada Klip Terakhir (Klip ke-${targetClips}), kalimat naskah voiceover ("new_vo") bagian akhir WAJIB diakhiri tepat dengan kalimat ucapan: "${mandatoryOutroLine}".\n` : ''}
...
5. CRITICAL: DILARANG KERAS menyertakan AUDIO SCRIPT atau naskah/narasi voiceover/spoken words di dalam t2i_prompt atau i2v_prompt. ${isWithoutSfx ? 'Jangan menyertakan efek suara (SFX) apa pun.' : 'Prompt visual hanya boleh berisi visual action dan deskripsi efek suara/SFX fisik saja (seperti: "SFX: swoosh, sizzling sound"). DILARANG KERAS menuliskan kata "music", "background music", "BGM", atau musik latar instrumental apa pun.'}${isWithoutSfx ? '\n6. CRITICAL RULE: Kampanye ini dikonfigurasi dalam mode "Without SFX" (Tanpa SFX). Anda DILARANG KERAS menyertakan bagian atau segmen [LAYER 3: SFX] atau deskripsi efek suara (SFX) di dalam prompt visual (t2i_prompt atau i2v_prompt). Jangan menuliskan kata "SFX:" atau efek suara apa pun.' : ''}`;
    ...
    dynamicPlanFields = `      "new_vo": "Naskah voiceover versi baru yang di-upgrade. Wajib dalam bahasa ${languageName} / ${targetLanguageLabel}.",
      "visual_action": "Deskripsi aksi visual versi baru dalam Bahasa Indonesia sebagai representasi manusiawi dari scene (misal: Kamera tracking mengikuti tangan menuangkan sirup ke gelas...)",
      "t2v_prompt": "Prompt T2V (In English, plain text linear, no newlines) wajib mengikuti format terstruktur bertingkat (structured layer) ${isWithoutSfx ? '3' : '4'} segmen (${pacingCount} micro-pacing beats @ 2s, total ${clipDuration}s): (VERTICAL 9:16) --ar ${config.aspect_ratio || '9:16'} --no landscape [LAYER 0: VISUAL TRUTH & ANCHORS] (Biometric Anchor: [Subject Anchor]), (Wardrobe: [Wardrobe Lock]), (Face Visibility: [Details reflecting face_visibility constraint]), (Product Truth: [Product Geometry] made of [Material Physics])${reRefFilenameTag}. [LAYER 1: SCENE & OPTICS] (Location: [Scene Details]), (Lens: [Camera Spec]), (Camera Move: [Kinetic Logic]), (Lighting: [Lighting Mood]). [LAYER 2: MICRO-PACING & ACTION] ${microPacingSample}${t2vSfxTemplate}"${voiceSegmentsField}`;
    dynamicImportantRules = `2. ALL visual prompts (t2v_prompt) MUST be written ENTIRELY in English. Naskah voiceover and caption/social metadata in ${targetLanguageLabel}.
3. CRITICAL: All visual prompts (t2v_prompt) MUST be a single plain text paragraph — copy-paste ready.
4. CRITICAL: Semua nilai string HARUS valid JSON. Jangan gunakan *unescaped double quotes* (\\") atau karakter *newline* aktual di dalam *string value* yang dapat merusak struktur JSON.
5. CRITICAL: DILARANG KERAS menyertakan AUDIO SCRIPT atau naskah/narasi voiceover/spoken words di dalam t2v_prompt. ${isWithoutSfx ? 'Jangan menyertakan efek suara (SFX) apa pun.' : 'Prompt visual hanya boleh berisi visual action dan deskripsi efek suara/SFX fisik saja (seperti: \"SFX: swoosh, sizzling sound\"). DILARANG KERAS menuliskan kata \"music\", \"background music\", \"BGM\", atau musik latar instrumental apa pun.'}${isWithoutSfx ? '\n6. CRITICAL RULE: Kampanye ini dikonfigurasi dalam mode \"Without SFX\" (Tanpa SFX). Anda DILARANG KERAS menyertakan bagian atau segmen [LAYER 3: SFX] atau deskripsi efek suara (SFX) di dalam prompt visual (t2v_prompt). Jangan menuliskan kata \"SFX:\" atau efek suara apa pun.' : ''}`;
...
## ATURAN GAYA NARASI (NARRATIVE MODE MANDATE)
Anda wajib menyusun naskah voiceover baru ("new_vo") untuk versi UPGRADE sesuai dengan Narrative Mode: "${config.narrative_mode || 'Storytelling'}" dengan ketentuan berikut:
${mandatoryOutroLine ? `- Khusus pada Klip Terakhir (Klip ke-${targetClips}), naskah voiceover ("new_vo") WAJIB diakhiri tepat dengan kalimat ucapan: "${mandatoryOutroLine}".\n` : ''}
- **Storytelling (Bercerita / Daily-life)**:
  * Klip Pertama (Klip 1): Hook pembuka wajib menggunakan naskah hook dari video referensi/sistem secara utuh.
  * Klip Pertengahan (Klip 2 s/d N-1): Melukiskan suasana rutinitas harian secara visual dan detail prosesnya (kronologis, misalnya: dari ketenangan persiapan kulkas hari Minggu ke hari Senin pagi yang tenang/damai). DILARANG KERAS menggunakan kata ganti orang pertama ("aku/gue/kami/saya") dan tanpa tokoh fiktif ("Andi/Siti") untuk menghindari kesan berbohong. Fokus pada keindahan proses, estetika visual, dan ketenangan pikiran.
  * Klip Terakhir (Klip N): Penutup/kesimpulan yang hangat dari aktivitas rutin tersebut dan CTA yang relevan.
...
export function buildOrganicPillarPrompt(kbTexts, campaignData, productData, brandProfile, vsoData) {
  const { aiDirective, mandatoryOutroLine } = resolvePromptInstructions(campaignData);
  const targetLanguage = campaignData.target_language || 'id-ID';
  const languageName = targetLanguage === 'en-US' ? 'ENGLISH (US)' : 'INDONESIAN';
  const targetLanguageLabel = targetLanguage === 'en-US' ? 'English' : 'Bahasa Indonesia';
  // Determine content world from campaignData (forwarded from planner — Tahap 2)
  const contentWorld = campaignData.content_world || 'real_world';
  const isCartoonUniverse = contentWorld === 'cartoon_universe';
  ...
  // World-specific KB routing (Tahap 2 → Tahap 3: Dynamic Universe KB)
  if (isCartoonUniverse) {
    const knowledgeDomain = campaignData?.knowledge_domain || 'general';
    const universeSlug = campaignData?.universe_profile;
    const cartoonKBs = buildCartoonKBList(knowledgeDomain, universeSlug);
    allowedKBs.push(...cartoonKBs);
    // DO NOT load REALIST_VIRAL_NARRATIVE or VISUAL_STYLE_GUIDE for cartoon universe
  } else {
    allowedKBs.push('REALIST_VIRAL_NARRATIVE', 'VISUAL_STYLE_GUIDE');
  }
  detectFoodAndInjectKB(
    allowedKBs,
    productData?.product_name,
  ...
  const clipDuration = Number(campaignData.clip_duration) || 8;
  const pacingCount = getMicroPacingSegmentCount(clipDuration);
  const audioSegmentSample = getMicroPacingTemplate(clipDuration, true, { hasTransitionLock: true });
...
`}` : '';
  ...
  const brandName = brandProfile?.brand_name || campaignData.account_name || 'Generik';
  const brandContext = brandProfile?.editorial_brand_context || brandProfile?.brand_context || campaignData.brand_context || '';
  const contentGoal = brandProfile?.editorial_content_goal || brandProfile?.content_goal || campaignData.content_goal || '';
  let brandPillars = [];
  try {
    const rawPillars = brandProfile?.editorial_content_pillars_json || brandProfile?.pillars_json || campaignData.pillars_json;
    brandPillars = typeof rawPillars === 'string' ? JSON.parse(rawPillars) : (Array.isArray(rawPillars) ? rawPillars : []);
  } catch (_) {}

  const brandContextSection = (brandContext || contentGoal || brandPillars.length > 0 || (brandName && brandName !== 'Generik' && brandName !== 'account')) ? `
========================================================================
## BRAND IDENTITY & EDITORIAL CONTEXT (MANDATORY)
========================================================================
- Nama Brand/Akun: "${brandName}"
${brandContext ? `- Konteks & Filosofi Alur Berpikir Brand:\n"""\n${brandContext}\n"""\n` : ''}${contentGoal ? `- Tujuan Konten & Misi Literasi:\n"""\n${contentGoal}\n"""\n` : ''}${brandPillars.length > 0 ? `- Pilar Konten Brand: ${brandPillars.join(', ')}\n` : ''}
PENTING: Seluruh alur naskah voiceover, pemilihan sudut pandang, analogi, adegan visual, dan terminologi WAJIB selaras 100% dengan Konteks, Filosofi, dan Tujuan Konten Brand di atas. DILARANG memunculkan topik produk, FMCG, resep kuliner, atau minuman herbal yang tidak relevan dengan Brand Profile.
========================================================================
` : '';
  ...
  const isSchemaV2VisualIdentity = Boolean(
    vsoData && (
      vsoData.schema_version === 'visual_identity_snapshot_v1' ||
      vsoData.structured?.visual_language ||
      vsoData.visual_language ||
      vsoData.mode_routing ||
      (vsoData.identity_ref && vsoData.identity_ref.key && vsoData.identity_ref.key !== 'legacy')
    )
  );
  ...
  if (isSchemaV2VisualIdentity) {
    faceVisibility = 'Faceless';
    vsoSection = buildVisualIdentityPromptSection(vsoData, targetClips);
  } else if (vsoData) {
...
6. MANDAT STRUKTUR TUPLE PROMPT VISUAL (STRICT SEPARATE TAG MANDATE):
   - Di dalam "t2i_prompts" dan "i2v_prompts", Anda WAJIB menuliskan tuple "(Anchor: [Subject Anchor])" dan tuple "(Wardrobe: [Wardrobe Lock])" (memuat spesifikasi warna & tekstur: "${targetWardrobe}") SECARA TERPISAH.
...
`;
  ...
  // === CARTOON UNIVERSE STORY DIRECTIVE (Tahap 2) ===
  let cartoonStoryDirective = '';
  if (isCartoonUniverse) {
    const rowPayload = campaignData._rowPayload || {};
    let universeSnapshot = {};
    try { universeSnapshot = campaignData.universe_snapshot_json ? JSON.parse(campaignData.universe_snapshot_json) : {}; } catch(_) {}
    const universeType = universeSnapshot?.universe_type || 'animal';
    const humanPresenceConfig = universeSnapshot?.human_presence || 'none';
    const depictionPolicyText = universeSnapshot?.depiction_policy || null;
    const historicalPeriodText = universeSnapshot?.historical_period || null;

    const mainChar = rowPayload.main_character || 'Mochi';
    const supportChars = rowPayload.supporting_characters || 'Dr. Paw, Coco';
    const storyPremise = rowPayload.story_premise || '';
    const petProblem = rowPayload.pet_problem || '';
    const productRole = rowPayload.product_role || 'none';
    const revealBeat = rowPayload.product_reveal_beat || 'none';
    const hasProduct = productRole !== 'none' && (campaignData.is_bridging_active === 1 || campaignData.is_bridging_active === true);

    const beatFourFive = hasProduct
      ? `Beat 4 (Klip 4) — SOLUTION INTRODUCTION: Produk diperkenalkan sebagai solusi oleh karakter pendukung. Produk TIDAK boleh terasa muncul tiba-tiba. Produk bukan deus ex machina.
Beat 5 (Klip 5) — PRODUCT DEMONSTRATION: Close-up/macro shot produk. Detail mekanisme dan fungsi produk terlihat jelas. Reaksi ${mainChar} terhadap produk.`
      : `Beat 4 (Klip 4) — STORY PROGRESSION: Cerita berkembang lebih lanjut. Karakter mencoba solusi alami atau saling berinteraksi. JANGAN memunculkan produk fiktif.
Beat 5 (Klip 5) — DEEPER EXPLORATION: Perkembangan emosi dan relasi antar karakter. Situasi mulai membaik secara organik tanpa produk.`;

    const productGeometryBlock = hasProduct ? `
## PRODUCT GEOMETRY LOCK (MANDATORY):
Produk affiliate WAJIB mempertahankan bentuk geometri IDENTIK, warna/material KONSISTEN, ukuran relatif terhadap karakter TERJAGA antar-scene.
Logo merek boleh dihilangkan, bentuk TIDAK BOLEH berubah. Produk HANYA muncul pada Beat ${revealBeat === 'beat_5' ? '5' : '4'} atau setelahnya.
` : '';

    cartoonStoryDirective = `
========================================================================
CARTOON UNIVERSE STORY DIRECTIVE (MANDATORY)
========================================================================
CONTENT WORLD: cartoon_universe | UNIVERSE: ${campaignData.universe_profile || 'pawville'} | TEMPLATE: ${campaignData.story_template || 'pet_problem_solution_7beat'}
UNIVERSE TYPE: ${universeType === 'human' ? 'HUMAN CLAYMATION' : 'ANIMAL / MASCOT'}
HUMAN PRESENCE: ${humanPresenceConfig.toUpperCase()} | VISUAL STYLE: ${universeSnapshot.visual_style || 'cinematic_3d_clay'}
${depictionPolicyText ? `
DEPICTION POLICY — ATURAN KERAS WAJIB DIPATUHI:
${depictionPolicyText}
` : ''}
${historicalPeriodText ? `
HISTORICAL PERIOD: ${historicalPeriodText}
Pastikan seluruh elemen visual (pakaian, arsitektur, benda, teknologi) konsisten dengan periode ini. Hindari anakronisme.
` : ''}

STORY PREMISE: ${storyPremise || '(Tentukan otonom berdasarkan pilar konten)'}
PET PROBLEM: ${petProblem || '(Tentukan masalah observable hewan berdasarkan pilar konten)'}

7-BEAT STORY ARC (WAJIB TEPAT ${campaignData.target_clips_count || 7} KLIP):
Beat 1 (Klip 1) — VISUAL HOOK: Masalah ${mainChar} terlihat langsung. Emosi karakter terasa. Produk BELUM muncul.
Beat 2 (Klip 2) — PROBLEM DEVELOPMENT: Dampak ringan. ${(supportChars.split(',')[0] || '').trim() || 'Karakter pendukung'} mulai menyadari. DILARANG diagnosis medis.
Beat 3 (Klip 3) — DISCOVERY: Karakter pendukung memeriksa situasi, menemukan penyebab. Gunakan ekspresi/gesture visual, BUKAN dialog diagnosis.
${beatFourFive}
Beat 6 (Klip 6) — RESOLUTION: ${mainChar} mulai nyaman/senang. Fokus perubahan emosi/perilaku. DILARANG klaim "menyembuhkan".
Beat 7 (Klip 7) — EMOTIONAL CLOSING + CTA: Hasil akhir nyaman/lucu. CTA hanya via voice-over. CTA DILARANG berupa teks di dunia visual.

CHARACTER IDENTITY LOCK (MANDATORY DI SETIAP PROMPT T2I/I2V):
- ${mainChar}: grey British Shorthair cat, round chubby body, short legs, thick soft grey fur, large amber eyes, small pink nose, green scarf
- Dr. Paw: tan Shiba Inu dog, white doctor coat, small brown medical bag
- Coco: brown-white Corgi dog, tiny sling bag
- Boba: cream-colored hamster, big puffy cheeks
- Tofu: white rabbit, green apron
UKURAN RELATIF antar karakter WAJIB konsisten. DILARANG morphing antar-scene.

MANDAT PROMPT T2I & I2V KARTUN (MANDATORY):
Setiap prompt visual di dalam "t2i_prompts" WAJIB menyertakan paragraf instruksi berikut di awal atau akhir deskripsinya:
"CHARACTER REFERENCE LOCK — MANDATORY
Use the supplied character reference images as identity references. Preserve the exact species, face shape, body proportions, fur color, fur pattern, eye color and permanent accessories. Do not redesign, reinterpret or replace the characters. The references define identity; the text prompt only defines pose, expression, action, camera composition and scene environment."

Dan jika produk target terlihat di adegan tersebut (product_visible: true), tambahkan juga paragraf instruksi berikut:
"PRODUCT REFERENCE LOCK — MANDATORY
Preserve the exact product geometry, color, components, material and relative scale from the supplied product reference."

Setiap prompt visual di dalam "i2v_prompts" WAJIB menyertakan paragraf instruksi berikut di awal deskripsinya:
"Animate only the supplied start frame. Preserve the exact character identity, face, body proportions, fur color, eyes, accessories, product geometry, environment, lighting and visual style. No character redesign. No morphing. No new characters. No wardrobe changes. No anatomy changes. No scene cuts."

MANDAT STRUKTUR STORYBOARD KARTUN (MANDATORY):
Setiap objek di dalam array "storyboard" pada JSON output WAJIB memiliki field terstruktur berikut:
1. "characters": array of strings (ID karakter valid yang benar-benar tampil pada adegan tersebut. Pilihan: ["mochi", "dr_paw", "coco", "boba", "tofu"]). Jika tidak ada karakter tampil, kosongkan array ini. Untuk pilot ini, batasi maksimal 2 karakter utama dalam satu klip.
2. "location": string (ID lokasi valid adegan. Pilihan: "mochi_home", "dr_paw_clinic", "town_square", "park", "market").
3. "product_visible": boolean (apakah produk target terlihat secara fisik di adegan ini. Bernilai true hanya pada Beat 4-5 atau klip lain saat produk terlihat, selain itu wajib false).

LOCATION CONTINUITY LOCK:
Lokasi harus konsisten antar-scene. Tersedia: PawVille Town Square, Mochi's Home, Dr. Paw's Clinic, PawVille Park, PawVille Market.
${productGeometryBlock}
${universeType !== 'human' ?
  'HUMAN-PRESENCE RULE: DILARANG KERAS memunculkan manusia, tangan manusia, wajah manusia di scene manapun.' :
  'HUMAN TYPE UNIVERSE: Karakter adalah manusia bergaya claymation. Patuhi depiction_policy dengan ketat.'}

WORLD-AWARE NEGATIVE PROMPT (DI SETIAP PROMPT VISUAL):
${universeType !== 'human' ?
  'NO human characters, NO human hands, NO human face, NO human figure in any scene' :
  'NO modern clothing, NO anachronistic objects, NO visualization of prophets or forbidden figures'}, NO character morphing, NO extra limbs, NO wardrobe drift, NO style drift, NO text overlay inside animated world, NO watermark, NO photorealistic rendering.

VOICE-OVER RULES: Maks 15 kata/beat. Tone hangat family-friendly. DILARANG klaim medis. GUNAKAN: "membantu", "dirancang untuk", "dapat menjadi pilihan".
Dr. Paw = observer/problem solver, BUKAN diagnoser medis.

ANTHROPOMORPHISM: Boleh berdiri dua kaki. Anatomi dasar spesies WAJIB dipertahankan. DILARANG morphing.
========================================================================
`;
  }
  ...
  const productEndClip = (bridgeDurationClips > 0) ? (bridgeAtClip + bridgeDurationClips - 1) : targetClips;
  const isBridgingActive = Boolean((campaignData.is_bridging_active === 1 || campaignData.is_bridging_active === true || campaignData.is_bridging_active === '1') && productData);

  let productInstruction = "";
  if (isBridgingActive && productData) {
...
`;
  ...
  // Suppress anti-cartoon negatives when content_world is cartoon_universe (Tahap 2)
  const dynamicStyleNeg = (isAnimationOr3d || isCartoonUniverse) ? '' : ', CGI look, plastic skin, anime, cartoon';
  const customNegativePrompt = isCartoonUniverse
    ? ` NEGATIVE PROMPT: human characters, human hands, human face, character morphing, extra limbs, wardrobe drift, style drift, text overlay, watermark, photorealistic rendering, (Constraint: ANIMATE EXISTING LIMBS ONLY).`
    : ` NEGATIVE PROMPT: text, subtitles, watermark, typography, facial features, head portrait, upper body, distorted face in background, extra limbs, third arm, floating thumb, morphing body parts, giant bottle, distorted scale${dynamicStyleNeg}, (Constraint: ANIMATE EXISTING LIMBS ONLY).`;
...
Karena kampanye ini dikonfigurasi dalam mode "pure_t2v", maka seluruh klip (Klip 1 hingga ${targetClips}) wajib menggunakan Text-to-Video. Oleh karena itu, isi array "t2v_prompts" untuk seluruh klip tersebut. Anda TIDAK perlu mengisi array "t2i_prompts" dan "i2v_prompts". Anda DILARANG KERAS menghasilkan key "t2i_prompts" atau "i2v_prompts" di dalam JSON output.`;
  ...
  const opcPacingSample = getMicroPacingTemplate(clipDuration, isAudioSegmentEnabled, { actionPlaceholder: '[Move]', hasTransitionLock: true });
  ...
  if (visualMode === 'hybrid_lock') {
    ...
    {
      "clip": 1,
      "prompt": "(VERTICAL 9:16) --ar ${campaignData.aspect_ratio || '9:16'} --no landscape [STYLE: symbolic_surrealism, modern editorial political illustration] [TEXTURE: printed_paper_grain, matte_editorial_ink, tactile paper fibers] [LAYER 1: SCENE & METAPHOR] (Metaphor Object: floating hourglass above glowing transit railway), (Environment: abstract commuter train carriage, muted amber tone). [LAYER 2: SUBJECT & COMPOSITION] (Subject: faceless commuter silhouette with briefcase, wrists covered in charcoal attire, zero facial features), (Composition: dynamic low-angle tracking shot). [LAYER 3: LIGHT & SHADOW] (Lighting: soft directional editorial backlight, strong geometric shadows)."
    }
  ],
  "i2v_prompts": [
    {
      "clip": 1,
      "prompt": "(VERTICAL 9:16) --ar ${campaignData.aspect_ratio || '9:16'} --no landscape [LAYER 1: INPUT & STYLE LOCK] (Start Frame: [Filename]), (Visual Mode: symbolic_surrealism), (Finish: matte editorial ink). [LAYER 2: MICRO-PACING & KINETICS] ${opcPacingSample}${i2vSfxTemplate}"
    }
  ],`;
    } else {
      dynamicPromptFields = `  "t2i_prompts": [
  ...
  "i2v_prompts": [
    {
      "clip": ${bridgeAtClip},
      "prompt": "(VERTICAL 9:16) --ar ${campaignData.aspect_ratio || '9:16'} --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame: [Filename]), (Consistency: MAX). (Geometric Truth: [MANDATE 50 - Shape & Material Extraction]). [LAYER 2: MICRO-PACING & ACTION (MANDATE 49)] ${opcPacingSample}${i2vSfxTemplate}"
    }
  ],`;
    }
...
3. I2V/T2I prompts MUST be a single plain text paragraph — copy-paste ready.
4. CRITICAL: Semua nilai string HARUS valid JSON. Jangan gunakan *unescaped double quotes* (") atau karakter *newline* aktual di dalam *string value* yang dapat merusak struktur JSON.
...
6. CRITICAL: Seluruh prompt visual (t2i_prompts & i2v_prompts) WAJIB BEBAS dari tag 'NEGATIVE PROMPT' dan tidak boleh mengandung instruksi negatif apa pun. Deskripsikan murni visual positif yang ingin ditampilkan saja.`;
  } else {
    dynamicPromptFields = `  "t2v_prompts": [
    {
      ...
      "scenes_covered": "1",
      "duration": "${clipDuration}s",
      "prompt": "(VERTICAL 9:16) --ar ${campaignData.aspect_ratio || '9:16'} --no landscape [LAYER 0: VISUAL TRUTH & ANCHORS] (Geometric Truth: [MANDATE 50 - Shape & Material Extraction]), (Biometric Anchor: [MANDATE 29 - 3-Point Character Lock]), (Wardrobe: [Wardrobe Lock]). [LAYER 1: SCENE & OPTICS] (Location: [MANDATE 33 - Verbatim Scene Lock]), (Lens: [Camera Spec]), (Camera Move: [Insert Kinetic Logic]), (Lighting: [Lighting Mood]). [LAYER 2: MICRO-PACING & ACTION (MANDATE 49)] ${opcPacingSample}${t2vSfxTemplate}"
    }
  ],`;
...
3. T2V prompts MUST be a single plain text paragraph — copy-paste ready.
4. CRITICAL: Semua nilai string HARUS valid JSON. Jangan gunakan *unescaped double quotes* (") atau karakter *newline* aktual di dalam *string value* yang dapat merusak struktur JSON.
...
- Visual Style: ${isSchemaV2VisualIdentity ? 'Editorial Multi-Mode System (Patuhi tabel alokasi mode visual, texture rendering, dan metaphor engine di seksi VISUAL IDENTITY SYSTEM di atas)' : `${campaignData.visual_style || 'Cinematic'} (Aturan gaya: jika Cinematic, gunakan pencahayaan filmis/dramatis, komposisi estetik, shallow depth of field; jika UGC, gunakan gaya video kasual/handheld Vlog amatir realistis; jika Macrophotography, fokus sangat dekat/extreme close-up menonjolkan tekstur detail produk atau bahan baku secara tajam).`}
...
2. Jabarkan "Pilar Konten" menjadi cerita yang edukatif, menghibur, bernilai wawasan, atau relatable sesuai Narrative Mode.
   🚨 DETAIL MANDAT NARRATIVE MODE ("${campaignData.narrative_mode || 'Storytelling'}"):
    - Jika "Storytelling":
      ...
      * Klip Pertengahan (Klip 2 s/d N-1): Melukiskan suasana, proses, atau dinamika topik secara visual dan kronologis. DILARANG KERAS menggunakan kata ganti orang pertama ("aku/gue/kami/saya") dan tanpa tokoh fiktif ("Andi/Siti") untuk menghindari kesan berbohong. Fokus pada kejelasan pesan, estetika visual, dan kedalaman wawasan.
      * Klip Terakhir (Klip N): Penutup/kesimpulan yang hangat dan reflektif dari topik tersebut beserta CTA yang relevan.
    - Jika "Problem-Solution":
      * Klip Pertama (Klip 1): Hook pembuka berupa keluhan/pertanyaan kritis.
      * Klip Awal Eskalasi (Klip 2): Menekankan rasa lelah, dilema, atau tantangan nyata audiens terkait topik secara terstruktur (pain point).
      * Klip Solusi (Klip 3 s/d N-1): Pivot/penguraian cara pandang baru, mekanisme sistem, atau langkah praktis sebagai penyelesaian masalah secara konkret.
      * Klip Terakhir (Klip N): Kesimpulan manfaat nyata dan CTA rekomendasi tindakan/diskusi.
    - Jika "Educational":
      * Klip Pertama (Klip 1): Hook fakta mengejutkan/paradoks fenomena.
      * Klip Edukasi Teoretis (Klip 2): Penjelasan konsep, dasar pemikiran, atau mekanisme sistemik di balik topik secara teoretis dan mendalam.
      * Klip Detail Analisis (Klip 3 s/d N-1): Penguraian detail mekanisme, fakta, atau implikasi praktis terkait topik secara informatif, logis, objektif, dan kredibel.
      * Klip Terakhir (Klip N): Ringkasan intisari wawasan dan CTA edukasi/eksplorasi wawasan.
...
5. ATURAN SEMANTIC STERILIZATION & SILENT KINETICS: AI Video memiliki filter audio yang ketat. Anda WAJIB merancang seluruh adegan menggunakan prinsip "Silent Kinetics" (gerakan hening) dan "Pure Optics" (fokus optik murni):
   - Fokuskan deskripsi visual pada pencahayaan filmis, pergerakan kamera halus (slow pan/zoom), tekstur permukaan, dan penempatan objek statis.
   - DILARANG menggambarkan interaksi fisik yang menghasilkan suara keras atau benturan (seperti mengetuk saringan kawat, menepuk kemasan, meremas foil logam, menuang air hingga memercik deras).
   - Sebagai gantinya, gunakan gerakan hening non-benturan (seperti "memegang objek secara tenang", "tangan diam di samping objek", "memperhatikan dokumen dengan tenang").
...
Return ONLY valid JSON (no markdown code blocks, no explanation). Use this exact structure:
{
  "analysis_summary": {
    ...
    "sandwich_transition_plan": "${isBridgingActive ? `Rencana transisi softsell klip ${bridgeAtClip}` : 'none (organic editorial campaign - no product bridging)'}"
  },
  "video_dna": {
    "pilar_konten": "Kategori/Pilar konten (sesuai tema: ${campaignData.content_pillar || 'Pilar Konten'})",
    "hook_type": "Tipe hook pembuka (e.g. Pertanyaan Reflektif, Fakta Kritis, Paradoks)",
    "visual_style": "${isSchemaV2VisualIdentity ? 'Editorial Multi-Mode System' : 'Gaya presentasi visual (e.g. Faceless Editorial, Cinematic Graphic, Macro)'}",
    "signature_moment": "Adegan visual paling berkesan/simbolik/estetik yang memperkuat pesan pilar",
    "camera_pace": "Kecepatan pergerakan kamera (e.g. Slow and Steady, Dynamic Tracking, Smooth Pan)",
    "primary_emotion": "Mood dominan (e.g. Kritis dan Reflektif, Tenang, Menggugah Pikiran)",
    "affiliate_integration": "${isBridgingActive ? 'Cara menyisipkan produk (e.g. Natural Usage, Background, Problem Solver)' : 'None (Pure Editorial)'}",
    "affiliate_mention": "${isBridgingActive ? 'Metode penyebutan afiliasi (e.g. Voice Over, Visual Only, Both)' : 'None'}",
    "scene_count": ${targetClips},
    "cta_type": "Tindakan ajakan interaksi (e.g. Diskusi Kolom Komentar, Bagikan Wawasan, Refleksi)"
  },
  "storyboard": [
    {
      "scene": 1,
      "duration": "Estimasi durasi (e.g. ${clipDuration}s)",${isSchemaV2VisualIdentity ? '\n      "visual_mode": "Mode visual adegan (e.g. symbolic_surrealism, editorial_graphic_novel, isometric_society, paper_cutout_documentary sesuai penugasan tabel di atas)",' : ''}
      "visual_description": "Deskripsi visual (sangat detail${isSchemaV2VisualIdentity ? ', deskripsikan metafora visual, tekstur, dan komposisi' : ''})",
  ...
  "voiceover": [
    ...
    {
      "scene": ${targetClips},
      "narration": "Naskah audio penutup klip terakhir.${sanitizeCustomInstruction(mandatoryOutroLine) ? ` MANDATORY: Akhiran/penutup kalimat naskah Klip Terakhir ini WAJIB mengakhiri dengan kalimat ucapan: \\"${sanitizeCustomInstruction(mandatoryOutroLine)}\\"` : ' Buat CTA natural sesuai isi konten tanpa mengucapkan AI Directive.'}",
      "duration": "Estimasi durasi",
      "cta_facebook": "Tuliskan alternatif Call to Action penutup khusus Facebook/Instagram (misal: 'bagikan pendapatmu di kolom komentar!'). Wajib dalam bahasa ${languageName} / ${targetLanguageLabel}.",
      "cta_tiktok": "Tuliskan alternatif Call to Action penutup khusus TikTok (misal: 'tulis pendapatmu di komen ya!'). Wajib dalam bahasa ${languageName} / ${targetLanguageLabel}."
    }
  ],
  ${dynamicPromptFields}
  "social_media_package": {
    "caption": "Single Universal Caption (terdiri dari Hook menarik, Value delivery, CTA interaksi, dan Hashtags relevan). In ${targetLanguageLabel}. MANDATORY: Gunakan hashtags edukatif yang aman dan relevan dengan topik pilar serta Brand Profile."
  }
}
...
export function buildDeconstructPhase1Prompt(originalCaption, targetRecommendationCount = 3, availableNiches = []) {
  const captionContext = originalCaption
    ? `\n[CAPTION ASLI VIDEO]\n"${originalCaption}"\n`
    : '';

  const nicheInstruction = availableNiches.length > 0
    ? `Pilih salah satu niche yang paling cocok dari daftar kategori utama berikut: [${availableNiches.join(', ')}]. Jika sama sekali tidak ada yang cocok, tentukan niche baru yang singkat (1-2 kata).`
    : 'Tentukan niche kategori konten yang paling cocok (contoh: Skincare, Gadget, Cooking, Baking, Health, Home Care, dll).';
...
### D. AUTO-DETECT NICHE & KEYWORDS (TAGS)
1. **niche**: ${nicheInstruction}
2. **tags**: Tentukan 5-10 kata kunci (keywords) terbaik dan paling relevan dengan isi dan visual video tersebut (tulis dalam bentuk array string, contoh: ["yogurt cake", "healthy dessert", "easy baking"]).
...
export function buildMultiplierPhase2Prompt(originalStoryboard, productDNA, aesthetics, bridging, vso, audio) {
  ...
  // Extract tone, guardrails, mandatory outros, and custom instructions
  const targetDemographic = aesthetics?.targetDemographic || "";
  const targetDemographicCustom = aesthetics?.targetDemographicCustom || "";
  const aiDirective = aesthetics?.aiDirective || "";
  const mandatoryOutroLine = aesthetics?.mandatoryOutroLine || "";
  const customInstruction = aesthetics?.customInstruction || "";

  const toneInstruction = getToneDemographicInstruction(targetDemographic, targetDemographicCustom);

  const toneBlock = toneInstruction ? `## GAYA BAHASA & TARGET DEMOGRAFI AUDIENS\n${toneInstruction}\n\n` : '';
  const directiveBlock = aiDirective ? `## AI DIRECTIVE / INTERNAL GUARDRAIL\n${aiDirective}\nCRITICAL: Directive ini hanya instruksi kontrol internal. DILARANG mengutip, menyalin, memparafrasekan, atau membacakannya di voiceover, CTA, caption, storyboard, dan prompt visual.\n\n` : '';
  const outroBlock = mandatoryOutroLine ? `## MANDATORY OUTRO CONTRACT\nKhusus pada adegan terakhir (Scene #${bridging?.targetClipsCount || 4}), naskah voiceover ("narration_transcript") WAJIB diakhiri tepat dengan kalimat ucapan: "${mandatoryOutroLine}".\n\n` : '';
  const customBlock = customInstruction ? `## CUSTOM INSTRUCTIONS / PERMINTAAN KHUSUS\n${customInstruction}\n\n` : '';
  ...
  const isPureStorytelling = Boolean(productDNA?.is_pure_storytelling || bridging?.campaign_type === 'pure_storytelling' || (!isBridgingActive && !productName));
...
- Wardrobe / Pakaian: ${targetWardrobe}
...
`;
  ...
  const productTruthVal = isPureStorytelling ? "" : (productDNA?.product_truth || productDNA?.clean_photo_t2i_prompt || productDNA?.t2i_prompt || "");
  const geometricTruthVal = isPureStorytelling ? "" : (productDNA?.geometric_truth || productDNA?.i2v_action_prompt || productDNA?.packaging_type || "");
...
`
  ...
  const clipDuration = Number(aesthetics?.clipDuration || aesthetics?.clip_duration) || 8;
  const pacingCount = getMicroPacingSegmentCount(clipDuration);
  const kineticPacing = getMicroPacingTemplate(clipDuration, false, { actionPlaceholder: '[Kinetic Move]', hasTransitionLock: true });
  const actionPacing = getMicroPacingTemplate(clipDuration, false, { actionPlaceholder: '[Action]', hasTransitionLock: true });
  const genericPacing = getMicroPacingTemplate(clipDuration, false, { actionPlaceholder: '...', hasTransitionLock: false });
...
Karena kampanye ini dikonfigurasi dalam mode "hybrid_lock", maka seluruh klip (Klip 1 hingga Klip ${targetClipsCount}) wajib menggunakan model Double-Pass (T2I + I2V) demi konsistensi detail visual yang sangat tinggi.
Oleh karena itu:
1. Anda WAJIB mengisi "t2i_prompt" (start frame image) dan "i2v_prompt" (camera/subject motion) untuk SETIAP klip (Klip 1 s.d. Klip ${targetClipsCount}).
2. Anda DILARANG mengisi "t2v_prompt" (kosongkan key tersebut / isi dengan "").

${!isPureStorytelling ? `Untuk klip dalam rentang pembahasan produk target (Klip ke-${bridgeAtClip} hingga Klip ke-${Math.min(productEndClip, targetClipsCount)}):
- Use the declared filename "${productFilenameDeclare}" inside your "t2i_prompt" to refer to the product reference photo.
...
- **i2v_prompt** MUST follow this structured layered format (written in English as a single line, NO newlines):
  (VERTICAL 9:16) --ar ${targetAr} --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame: [Generated start frame image]), (Consistency: MAX). (Geometric Truth: ${geometricTruthVal ? geometricTruthVal : '[Consistency of subject/product]'}). [LAYER 2: MICRO-PACING & ACTION] ${kineticPacing} [LAYER 3: SFX] SFX: [SFX description, e.g. whoosh, ambient sizzle].

Untuk klip di luar rentang produk (Klip sebelum ke-${bridgeAtClip} atau sesudah ke-${productEndClip}):` : 'Untuk seluruh adegan (Klip 1 s.d. ' + targetClipsCount + '):'}
- **t2i_prompt** MUST follow this structured layered format (written in English as a single line, NO newlines):
  (VERTICAL 9:16) --ar ${targetAr} --no landscape [LAYER 1: OPTICS] (Shot on [Camera Specs], [Lens Specs]), (Texture: [Film/Digital Physics]). [LAYER 2: SUBJECT & VISUAL TRUTH] (Geometric Truth: strictly no product shown), (Anchor: [Subject Demographic]), (Wardrobe: [Hijab/Wardrobe Style]). [LAYER 3: SCENE & LIGHT] (Environment: [Location/Setting]), (Lighting: [Lighting Mood/Ambiance]). [LAYER 4: KINETIC IMPLICATION] (Frozen Action: Subject is poised to [Action Verb/Pose]), (Micro-Expression: [Key Emotion/Vibe]).
- **i2v_prompt** MUST follow this structured layered format (written in English as a single line, NO newlines):
  (VERTICAL 9:16) --ar ${targetAr} --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame: [Generated start frame image]), (Consistency: MAX). (Geometric Truth: strictly no product shown). [LAYER 2: MICRO-PACING & ACTION] ${kineticPacing} [LAYER 3: SFX] SFX: [SFX description, e.g. whoosh, ambient sizzle].
`
    : `
...
- **t2v_prompt** MUST follow this structured layered format (written in English as a single line, NO newlines):
  (VERTICAL 9:16) --ar ${targetAr} --no landscape [LAYER 0: VISUAL TRUTH & ANCHORS] (Geometric Truth: [Shape consistency]), (Biometric Anchor: [Subject Demographic]), (Wardrobe: [Hijab/Wardrobe Style]). [LAYER 1: SCENE & OPTICS] (Location: [Setting]), (Lens: [Camera Spec]), (Camera Move: [Kinetic Move]), (Lighting: [Lighting Mood/Ambiance]). [LAYER 2: MICRO-PACING & ACTION] ${actionPacing} [LAYER 3: SFX] SFX: [SFX description].
`;
  ...
  const narrativeRules = isPureStorytelling
    ? `
🚨 ATURAN GAYA NARASI & FORMULA VIRAL (NARRATIVE MODE MANDATE - TANPA PRODUK):
Anda wajib menyusun naskah voiceover baru ("narration_transcript") untuk versi REMAKE sesuai dengan Narrative Mode: "${narrativeMode}" dengan ketentuan:
- **Storytelling (Bercerita / Daily-life / Refleksi)**:
  * Klip 1 (Hook Pembuka): Adaptasi hook pembuka dari video blueprint referensi secara utuh untuk menarik atensi audiens di 3 detik pertama.
  * Klip 2 s/d ${targetClipsCount - 1} (Pengembangan Cerita): Melukiskan suasana rutinitas harian secara visual dan detail prosesnya (kronologis). DILARANG KERAS menggunakan kata ganti orang pertama ("aku/gue/kami/saya") dan tanpa tokoh fiktif ("Andi/Siti") untuk menjaga kesan alami dan berbobot. Fokus pada keindahan proses, estetika visual, dan ketenangan pikiran.
  * Klip ${targetClipsCount} (Penutup & Closing): Penutup/kesimpulan yang hangat dari aktivitas rutin/refleksi tersebut dan CTA interaksi (save/share) yang relevan.
- **Problem-Solution (Masalah & Solusi)**:
  * Klip 1: Hook pembuka berupa keluhan/pertanyaan relatable penonton.
  * Klip 2: Eskalasi emosional pain point ("Capek banget kan...", "Pasti sering ngerasa...", "Berapa kali lo ngerasa...").
  * Klip 3 s/d ${targetClipsCount - 1}: Pivot/perkenalan kebiasaan baru atau langkah praktis sebagai penyelesaian masalah secara konkret. Fokus pada efisiensi waktu, kemudahan, dan nilai praktis.
  * Klip ${targetClipsCount}: Kesimpulan manfaat praktis dan CTA rekomendasi tindakan.
- **Educational (Tutorial / Penjelasan Faktual)**:
  * Klip 1: Hook fakta mengejutkan atau mitos salah kaprah.
  * Klip 2: Penjelasan konsep/fakta mendalam di balik topik.
  * Klip 3 s/d ${targetClipsCount - 1}: Tips teknis dan implementasi praktis terkait topik. Gunakan bahasa informatif, logis, objektif, dan kredibel.
  * Klip ${targetClipsCount}: Ringkasan manfaat pengetahuan dan CTA edukasi lanjutan.
`
    : (isBridgingActive
      ? `
...
`;
  ...
  const roleIntro = isPureStorytelling
    ? `Kamu adalah seorang Creative Director & Senior Copywriter di MAKNA Engine.
Tugas kamu adalah mengambil sebuah **Original Storyboard** (hasil dekonstruksi video viral) dan merekayasa ulang (remake) storyboard tersebut menjadi konten narasi/edukasi/storytelling yang segar, berbobot, dan bernilai tinggi TANPA promosi produk jualan.`
    : `Kamu adalah seorang Creative Director & Senior Copywriter di MAKNA Engine.
Tugas kamu adalah mengambil sebuah **Original Storyboard** (hasil dekonstruksi video viral) dan merekayasa ulang (remake) storyboard tersebut untuk mempromosikan **Produk Target Kami**.`;
  ...
  const angleTitle = bridging?.angleTitle || bridging?.angle_title || '';
  const hookType = bridging?.hookType || bridging?.hook_type || '';
  const hookText = bridging?.hookText || bridging?.hook_text || '';
  const targetPersona = bridging?.targetPersona || bridging?.target_persona || '';
  const emotionalTrigger = bridging?.emotionalTrigger || bridging?.emotional_trigger || '';
  const ctaType = bridging?.ctaType || bridging?.cta_type || '';
  const nicheKbSnippet = bridging?.nicheKbContent || bridging?.niche_kb_content || '';

  const dataSection = isPureStorytelling
    ? `---
### TOPIK & SUDUT PANDANG CERITA (ORGANIC MONETIZATION - NON-PRODUK):
- Judul / Angle Spesifik: "${angleTitle || productDesc || 'Refleksi mendalam & pesan bernilai tinggi'}"
${targetPersona ? `- Target Persona / Audiens: "${targetPersona}"\n` : ''}${hookType ? `- Tipe Hook Pembuka: "${hookType}"\n` : ''}${hookText ? `- Panduan Hook 0-3s: "${hookText}" (Gunakan atau variasikan hook ini untuk Scene 1)\n` : ''}${emotionalTrigger ? `- Trigger Emosi Utama: "${emotionalTrigger}"\n` : ''}${ctaType ? `- Rekomendasi CTA Interaksi: "${ctaType}"\n` : ''}- Pesan Penutup / Outro: "${productUsp || mandatoryOutroLine || 'Pesan moral penutup / CTA interaksi'}"
${nicheKbSnippet ? `\n### PANDUAN DOMAIN KHUSUS (NICHE KB):\n${nicheKbSnippet}\n` : ''}`
    : `---
### DATA PRODUK TARGET KAMI:
- Nama Produk: "${productName}"
...
${toneBlock}${directiveBlock}${outroBlock}${customBlock}${dataSection}
...
Kembalikan HANYA JSON valid dengan struktur berikut, tanpa dibungkus markdown code fences. Pastikan seluruh nilai prompt visual ("t2v_prompt", "t2i_prompt", "i2v_prompt") ditulis dalam SATU BARIS tunggal tanpa karakter newline/pindah baris:
{
  ...
  "prompts": [
    {
      "scene": 1,
      "t2v_prompt": "(VERTICAL 9:16) --ar ${targetAr} --no landscape [LAYER 0: VISUAL TRUTH & ANCHORS] (Geometric Truth: ...), (Biometric Anchor: ...), (Wardrobe: ...). [LAYER 1: SCENE & OPTICS] (Location: ...), (Lens: ...), (Lighting: ...). [LAYER 2: MICRO-PACING & ACTION] ${genericPacing} [LAYER 3: SFX] SFX: ...",
      ...
      "i2v_prompt": "(VERTICAL 9:16) --ar ${targetAr} --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame: ...). [LAYER 2: MICRO-PACING & ACTION] ${genericPacing} [LAYER 3: SFX] SFX: ..."
    }
  ],
...
export function sanitizeI2vPrompt(prompt) {
  ...
  // 0. Pre-cleaning Selektif: Bersihkan rasio, layer brackets, dan vertical tag. 
  // NEGATIF PROMPT dan keyword negatif "--no" sengaja dipertahankan agar tidak terhapus.
  safe = safe.replace(/--ar\s+\d+:\d+/gi, '');
  safe = safe.replace(/\[LAYER\s+\d+:[^\]]*\]/gi, '');
  safe = safe.replace(/\(VERTICAL\s+\d+:\d+\)/gi, '');
  safe = safe.replace(/\(Constraint:\s*[^)]*\)/gi, '');
  ...
  const synonyms = [
    ...
    { pattern: /\bcrash\s+zoom\b/gi, replacement: 'rapid zoom' },
    { pattern: /\bcrash\b/gi, replacement: 'rapid' },
    { pattern: /\bmuscle\s+tension\b/gi, replacement: 'firm pressure' }
  ];
  ...
  if (safe.replace(/\W/g, '').length < 30) {
    safe = 'Slow Zoom In on product label, followed by Orbital Pan around the product, ending with Rack Focus from foreground to product center. Cinematic, professional studio lighting.';
  }
...
4. **Deskripsi I2V (\`clip2_i2v_prompt\`)**:
   - Tulis dalam bahasa Inggris.
   - Wajib mengikuti format terstruktur bertingkat (structured layer) ${pacingCount} segmen micro-pacing (2 detik per segmen) berikut:
     "(VERTICAL 9:16) --ar 9:16 --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame: clip2_t2i.png), (Consistency: MAX). [LAYER 2: MICRO-PACING & ACTION] ${injectorPacing} [LAYER 3: SFX] SFX: [suara efek suara yang cocok]"
...
export function buildStrategicCampaignPrompt(campaign, item, workflow, creativeKb, vsoDirectives, isolationMandate, bridgeRangeText) {
  ...
  const clipDuration = Number(workflow.clip_duration || campaign.clip_duration) || 8;
  const pacingCount = getMicroPacingSegmentCount(clipDuration);
  const strategicPacing = getMicroPacingTemplate(clipDuration, false, { actionPlaceholder: '[Micro motion]' });
...
- Jumlah Klip Target: ${clipCount} klip (setiap klip berdurasi ~${clipDuration} detik)
...
4. KETENTUAN ANATOMI FORMAT PROMPT VISUAL (MANDATORI 100% BAHASA INGGRIS / ENGLISH ONLY):
   ...
   - "i2v_prompt": WAJIB 100% BERBAHASA INGGRIS (ENGLISH ONLY - DILARANG KERAS MENGGUNAKAN BAHASA INDONESIA PADA PROMPT I2V. DILARANG KERAS MENGHASILKAN OBJEK JSON / NESTED OBJECT). WAJIB berfokus pada gerakan mikro kamera halus (cinematic slow dolly zoom, subtle hand balance, steady ambient lighting) yang mengikat presisi pada Start Frame T2I. DILARANG KERAS (FORBIDDEN) meminta aksi fisik ekstrem yang merubah bentuk fisik produk (seperti tangan baru masuk dari luar layar atau membuka kemasan utuh):
     (VERTICAL 9:16) --ar 9:16 --no landscape [LAYER 1: INPUT & TRUTH LOCK] (Start Frame Reference: CLIP_N_START_FRAME.png), (Consistency: MAX). [LAYER 2: MICRO-PACING & ACTION] ${strategicPacing}${workflow.sfx_setting === 'with_sfx' ? ' [LAYER 3: SFX] [SFX: English sound effect details]' : ''}
