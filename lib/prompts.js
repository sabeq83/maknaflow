...
`
  ...
  const clipDuration = Number(aesthetics?.clipDuration || aesthetics?.clip_duration || options?.clip_duration) || 8;
  const pacingCount = getMicroPacingSegmentCount(clipDuration);
...
Untuk klip dalam rentang pembahasan produk target (Klip ke-${bridgeAtClip} hingga Klip ke-${Math.min(productEndClip, targetClipsCount)}):
...
Untuk klip di luar rentang produk (Klip sebelum ke-${bridgeAtClip} atau sesudah ke-${productEndClip}):
- **t2i_prompt** MUST follow this structured layered format (written in English as a single line, NO newlines):
...
${toneBlock}${directiveBlock}${outroBlock}${customBlock}---
### DATA PRODUK TARGET KAMI:
- Nama Produk: "${productName}"
