...
export function buildOrganicPillarPrompt(kbTexts, campaignData, productData, brandProfile, vsoData) {
  ...
  if (isCartoonUniverse) {
    allowedKBs.push('PET_CONTENT', 'CARTOON_UNIVERSE_STORY_ENGINE', 'CARTOON_VISUAL_CONTINUITY');
    // Dynamic: load universe-specific profile KB based on slug
    const universeSlug = campaignData?.universe_profile || 'pawville';
    const profileKBName = universeSlug.toUpperCase() + '_UNIVERSE_PROFILE';
    allowedKBs.push(profileKBName);
    // Also allow the generic fallback in case slug doesn't match any KB file
    if (profileKBName !== 'PAWVILLE_UNIVERSE_PROFILE') {
      allowedKBs.push('PAWVILLE_UNIVERSE_PROFILE');
    }
