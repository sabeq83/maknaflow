...
export function buildReverseEngineeringPrompt(kbTexts, config) {
  ...
  const mandatoryOutroLine = config.mandatory_outro_line || '';
...
${config.custom_instruction && !config.ai_directive ? `## CUSTOM INSTRUCTIONS FROM USER\n${config.custom_instruction}\n` : ''}
${vsoSection}
...
export function buildOrganicPillarPrompt(kbTexts, campaignData, productData, brandProfile, vsoData) {
  const aiDirective = campaignData.ai_directive || campaignData.custom_instruction || '';
  const mandatoryOutroLine = campaignData.mandatory_outro_line || '';
