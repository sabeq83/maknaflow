export function resolveEpisodeStages({ episode, research, blueprint, script, productionPackage }) {
  if (!episode) return [];
  ...
  const stages = [
    ...
    },
    {
      ...
      enabled: productionPackage && ['preview_ready', 'final_rendering', 'completed'].includes(productionPackage.status),
      status: !(productionPackage && ['preview_ready', 'final_rendering', 'completed'].includes(productionPackage.status)) ? 'blocked' :
