...
export async function approvePillarCampaignItem(itemId, changes = {}) {
  ...
  const requestedRevision = changes.review_revision || null;
  if (item.review_revision && requestedRevision !== item.review_revision) {
...
export async function transitionPillarReview({ itemId, action, reviewRevision, reason = null, actorId = null, idempotencyKey = null, changes = null }) {
  ...
  if (!reviewRevision) throw new PillarCampaignApprovalError('review_revision wajib diisi.', 400, 'PILLAR_REVIEW_REVISION_REQUIRED');
  ...
  const key = idempotencyKey || `review:${tenantId}:${itemId}:${reviewRevision}:${action}`;
  ...
  const inserted = await pgQuery(
    ...
    [actionId, tenantId, String(itemId), reviewRevision, action, key, actorId, reason]
  );
  ...
  try {
    ...
    if (!item) throw new PillarCampaignApprovalError('Campaign item not found', 404, 'PILLAR_ITEM_NOT_FOUND');
    if (item.review_revision !== reviewRevision) throw new PillarCampaignApprovalError('Revision review berubah. Muat ulang sebelum melanjutkan.', 409, 'PILLAR_APPROVAL_STALE_REVISION');
    let result;
    if (action === 'approve') {
      ...
      result = changes
        ? await approvePillarCampaignItem(itemId, { ...changes, review_revision: reviewRevision, actor_id: actorId })
        : await approvePillarCampaignItemUnchanged(itemId, { review_revision: reviewRevision, actor_id: actorId });
