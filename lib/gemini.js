...
export const GEMINI_MODELS = {
  ...
  FALLBACK_1: 'gemini-3.6-flash',
  FALLBACK_2: 'gemini-flash-latest',
  TTS: 'gemini-2.5-flash-preview-tts',
  SCRAPER: 'gemini-3.7-flash',
...
/**
 * Wraps a model instance to handle errors and fall back to fallback models
 */
function makeModelResilient(model, apiKey, modelName = GEMINI_MODELS.PRIMARY, isExplicitKey = false, explicitFallback = null) {
  ...
  model.generateContent = async function(request) {
    const primaryModel = modelName || GEMINI_MODELS.PRIMARY;
    const fallbackModel = explicitFallback || (primaryModel === GEMINI_MODELS.PRIMARY 
      ...
      while (true) {
        try {
          if (currentModel === primaryModel) {
            return await callWithTimeout(originalGenerateContent(request));
          } else {
            ...
            return await callWithTimeout(fallbackModelObj.generateContent(request));
          }
        } catch (error) {
...
export async function getGeminiModel(apiKey = null, systemInstruction = null) {
  ...
  return makeModelResilient(model, key, models.PRIMARY, apiKey !== null, models.FALLBACK_1);
}
...
export async function testGeminiConnection(apiKey, modelToTest = null) {
  try {
    ...
    const resilientModel = makeModelResilient(model, apiKey, activeModel, true, models.FALLBACK_1);
