...
export async function getModelWaterfallChain(startingModel = null) {
  ...
  // If custom model ID, try custom model first, then cascade down standard order
...
/**
 * Wraps a model instance to handle 503 high demand errors using a waterfall sequential cascade
 */
  ...
  model.generateContent = async function(request) {
    ...
    const executeWaterfall = async (activeKey) => {
      ...
      for (let i = 0; i < waterfallChain.length; i++) {
        ...
        // 1 initial attempt + 1 quick retry if 503 high demand
        for (let attempt = 0; attempt < 2; attempt++) {
          try {
            ...
            if (is503) {
              if (attempt === 0) {
                console.warn(`[Gemini-Waterfall] ${currentModel} returned 503 (high demand). Quick retry in 2s...`);
                await new Promise(r => setTimeout(r, 2000));
                continue;
              } else if (nextModel) {
                console.warn(`[Gemini-Waterfall] ${currentModel} still 503. Cascading down to ${nextModel}...`);
                break; // Move to next model in the waterfall chain
              }
            }

            // Propagate non-503 errors (e.g. 429 quota or auth) to key pool handler
            throw error;
          }
        }
      }
