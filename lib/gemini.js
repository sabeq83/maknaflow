...
export function getCascadeAction(error, attempt, hasNextModel) {
  ...
  if (is503) {
    if (attempt === 0) {
      return { action: 'retry', delayMs: 2000, reason: '503 High Demand' };
    }
    if (hasNextModel) {
  ...
  if (is429) {
    if (attempt === 0) {
      return { action: 'retry', delayMs: 2000, reason: '429 Rate Limit' };
    }
    if (hasNextModel) {
  ...
  model.generateContent = async function(request) {
    ...
    const executeWaterfall = async (activeKey) => {
      ...
      for (let i = 0; i < waterfallChain.length; i++) {
        ...
        // 1 initial attempt + 1 quick retry if retryable
        for (let attempt = 0; attempt < 2; attempt++) {
          try {
            ...
            if (cascadeAction.action === 'retry') {
              console.warn(`[Gemini-Waterfall] ${currentModel} returned ${cascadeAction.reason}. Quick retry in ${cascadeAction.delayMs || 2000}ms...`);
              await new Promise(r => setTimeout(r, cascadeAction.delayMs || 2000));
              continue;
            ...
            // Propagate non-cascadeable errors
            throw error;
          }
        }
      }
