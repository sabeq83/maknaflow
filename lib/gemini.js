...
export const MODULE_MODEL_MATRIX = {
  ...
  CAMPAIGN_PLANNER: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
  PILLAR_CAMPAIGN: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
  RE_CAMPAIGN: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
  RECIPE_LABS: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
  INSTANT_FACTORY: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
  MULTIPLIER_LAB: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
  BRIDGE_INJECT: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
  SHEETS_AUTOPILOT: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
  CONTENT_AUTOMATION: { paid: 'gemini-3.6-flash', free: 'gemini-3.8-flash', useKB: true },
...
export async function resolveModelForTask(taskType = 'CAMPAIGN_PLANNER', modelOverride = null) {
  ...
  return savedPrimary || (tier === 'paid' ? 'gemini-3.6-flash' : 'gemini-3.8-flash');
}
...
export async function getModelWaterfallChain(startingModel = null, explicitTier = null) {
  ...
  const fullCascade = tier === 'paid' ? GEMINI_CASCADE_ORDER_PAID : GEMINI_CASCADE_ORDER_FREE;
  ...
  start = start || fullCascade[0];

  const startIndex = fullCascade.indexOf(start);
  if (startIndex !== -1) {
    return fullCascade.slice(startIndex);
  }
  // If custom or not in current list, try custom model first, then cascade down full order
  return [start, ...fullCascade.filter(m => m !== start)];
}
...
export async function getResolvedGeminiModels({ taskType = null, explicitTier = null } = {}) {
  ...
  let primary = tier === 'paid' ? 'gemini-3.6-flash' : 'gemini-3.8-flash';
