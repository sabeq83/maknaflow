...
export async function handleCallback(code, redirectUri) {
  ...
  await setSetting('google_tokens', JSON.stringify(mergedTokens));
  ...
  if (data.email) {
    await setSetting('google_email', data.email);
  }
...
/**
 * Get authorized client with valid tokens (auto-refresh bound to active tenant)
 */
export function getAuthorizedClient() {
  ...
  // Set up token refresh callback — preserve refresh_token and bind to active tenant
  client.on('tokens', (newTokens) => {
    tenantContext.run(activeTenantId, async () => {
      let current = {};
      try {
      ...
      await setSetting('google_tokens', JSON.stringify(merged));
      console.log(`[Google Auth] Refreshed tokens automatically persisted for tenant: ${activeTenantId}`);
    }).catch(err => {
      console.error(`[Google Auth] Failed to persist refreshed tokens for tenant ${activeTenantId}:`, err.message);
    });
  });
