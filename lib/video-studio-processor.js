...
export async function processYouTubeTimelineMux(config) {
  ...
  const onePassVideoLabel = resolvedVideos.length === 1 ? 'v_std_0' : 'v_one_pass';
  if (resolvedVideos.length > 1) {
    const videoConcatInputs = resolvedVideos.map((_, i) => `v_std_${i}`);
    filterGraph.push({
      filter: 'concat',
      options: { n: resolvedVideos.length, v: 1, a: 0 },
      inputs: videoConcatInputs,
      outputs: 'v_one_pass'
    });
  }
  ...
  if (useAtempo && Math.abs(speedFactor - 1.0) > 0.001) {
    // In fluent-ffmpeg context: speed factor for atempo is reciprocal of duration change factor
  ...
  // Always pad silence to make audio match planned duration
  filterGraph.push({
    filter: 'apad',
