...
// Ensure ffmpeg path is also set correctly if environment variable or Homebrew path exists
...
export function resolveLocalPath(filePath) {
  ...
  // Normalize paths to ensure leading slash for relative segment checks
  const normalizedPath = filePath.startsWith('/') ? filePath : '/' + filePath;

  if (normalizedPath.startsWith('/temp/')) {
    return path.join(process.cwd(), 'public', normalizedPath);
  }
  if (normalizedPath.startsWith('/rendered/')) {
    return path.join(process.cwd(), 'public', normalizedPath);
  }
...
export async function processTtsStudioMuxing(config) {
  ...
  let videoPaths = [];
  try {
    const parsed = typeof videoPath === 'string' ? JSON.parse(videoPath) : videoPath;
    videoPaths = Array.isArray(parsed) ? parsed.filter(Boolean) : (videoPath ? [videoPath] : []);
  } catch {
    videoPaths = videoPath ? [videoPath] : [];
  }

  if (videoPaths.length === 0) {
    throw new Error(`Daftar path klip video kosong atau tidak valid untuk batch '${batchId}'.`);
  }

  // Load completed clips from batch
  const { getTtsBatchWithClips } = await import('./db.js');
  const batchData = await getTtsBatchWithClips(batchId);
  ...
  if (videoPaths.length !== completedClips.length) {
    throw new Error(`Jumlah video (${videoPaths.length}) tidak sama dengan audio TTS (${completedClips.length}) untuk batch '${batchId}'.`);
  }
  const pairCount = videoPaths.length;
...
// Automatically trigger startup sweep when module is loaded, unless this
// environment explicitly disables background services (local staging).
if (process.env.ENABLE_BACKGROUND_SERVICES !== 'false') {
  runStartupGC();
}

export async function processYouTubeTimelineMux(config) {
  const { videoPaths, audioPath, plannedDurationSeconds, durationPolicy, outputPath, narrationProfileKey } = config;

  const resolvedVideos = videoPaths.map(resolveLocalPath);
  const resolvedOutput = resolveLocalPath(outputPath);
  let resolvedAudio = resolveLocalPath(audioPath);

  const profile = getNarrationProfile(narrationProfileKey || 'general_id');

  // Measure audio duration
  let audioDuration = 0;
  if (resolvedAudio && fs.existsSync(resolvedAudio)) {
    audioDuration = await getMediaDuration(resolvedAudio);
  }

  // Calculate pacing profile ratio
  const ratio = plannedDurationSeconds > 0 ? (audioDuration / plannedDurationSeconds) : 1.0;
  let useAtempo = false;
  let speedFactor = 1.0;

  const maxLimit = profile.tempoCorrection?.max || 1.08;
  const minLimit = profile.tempoCorrection?.min || 0.92;

  if (ratio > maxLimit) {
    throw new Error(`DurationMismatchError: Audio duration (${audioDuration.toFixed(1)}s) is too long for the planned timeline (${plannedDurationSeconds}s). revision_required.`);
  } else if (ratio >= minLimit && ratio <= maxLimit) {
    useAtempo = true;
    speedFactor = ratio; // speed up/down within limits
  }

  // Set up silent audio if missing or extremely short
  if (!resolvedAudio || !fs.existsSync(resolvedAudio)) {
    const tempDir = path.join(process.cwd(), 'public', 'temp');
    fs.mkdirSync(tempDir, { recursive: true });
    resolvedAudio = path.join(tempDir, `silent_${Date.now()}_${Math.random().toString(36).substring(7)}.wav`);
    const { execSync } = require('child_process');
    execSync(`ffmpeg -y -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -t ${Math.ceil(plannedDurationSeconds || 5)} "${resolvedAudio}"`);
  }
  ...
  // Standardize videos & generate or format audio tracks
  for (let i = 0; i < resolvedVideos.length; i++) {
    const vp = resolvedVideos[i];
    filterGraph.push({
      filter: 'scale',
      options: `${stdWidth}:${stdHeight}`,
      inputs: `${i}:v`,
    ...
    // Audio check & fallback for video clips
    let hasAudio = false;
    try {
      hasAudio = await new Promise((resolve) => {
        ffmpeg.ffprobe(vp, (err, metadata) => {
    ...
    if (hasAudio) {
      ...
      filterGraph.push({
        filter: 'anullsrc',
        options: 'cl=stereo:r=48000',
        outputs: `v_silent_src_${i}`
      });
      filterGraph.push({
        filter: 'atrim',
        options: { duration: duration },
        inputs: `v_silent_src_${i}`,
        outputs: `v_std_a_${i}`
      });
    }
  }
  ...
  const onePassVideoLabel = resolvedVideos.length === 1 ? 'v_std_0' : 'v_one_pass';
  const onePassAudioLabel = resolvedVideos.length === 1 ? 'v_std_a_0' : 'a_sfx_unsplit';

  if (resolvedVideos.length > 1) {
    const concatInputs = [];
    for (let i = 0; i < resolvedVideos.length; i++) {
      concatInputs.push(`v_std_${i}`);
      concatInputs.push(`v_std_a_${i}`);
    }
    filterGraph.push({
      filter: 'concat',
      options: { n: resolvedVideos.length, v: 1, a: 1 },
      inputs: concatInputs,
      outputs: ['v_one_pass', 'a_sfx_unsplit']
    });
  }

  // Measure total input video duration
  let totalVideoIn = 0;
  for (const vp of resolvedVideos) {
    totalVideoIn += await getMediaDuration(vp);
  }

  let sfxAudioOut = 'a_sfx_sync';

  if (totalVideoIn < plannedDurationSeconds) {
    const pad = plannedDurationSeconds - totalVideoIn;
    filterGraph.push({
      filter: 'tpad',
      options: { stop_mode: 'clone', stop_duration: pad },
      inputs: onePassVideoLabel,
      outputs: 'v_sync'
    });
    filterGraph.push({
      filter: 'apad',
      options: { whole_dur: plannedDurationSeconds },
      inputs: onePassAudioLabel,
      outputs: 'a_sfx_sync'
    });
  } else {
    filterGraph.push({
      filter: 'trim',
      options: { duration: plannedDurationSeconds },
      inputs: onePassVideoLabel,
      outputs: 'v_sync'
    });
    filterGraph.push({
  ...
  // Audio filter: apply speed factor and pad with silence to fit planned duration
  const audioInputIdx = resolvedVideos.length;
  let audioOut = `${audioInputIdx}:a`;

  if (useAtempo && Math.abs(speedFactor - 1.0) > 0.001) {
    const tempoVal = Number((1 / speedFactor).toFixed(3));
    if (tempoVal >= 0.5 && tempoVal <= 2.0) {
      filterGraph.push({
        filter: 'atempo',
        options: String(tempoVal),
        inputs: `${audioInputIdx}:a`,
        outputs: 'a_tempo'
      });
      audioOut = 'a_tempo';
    }
  ...
  // Always pad silence to make VO audio match planned duration
  filterGraph.push({
    filter: 'apad',
    options: { whole_dur: plannedDurationSeconds },
    inputs: audioOut,
    outputs: 'a_vo_padded_raw'
  });

  // Split the VO stream so it can be consumed by both sidechain compressor and amix
  filterGraph.push({
    filter: 'asplit',
    options: '2',
    inputs: 'a_vo_padded_raw',
    outputs: ['a_vo_padded_1', 'a_vo_padded_2']
  });

  // Audio Ducking sidechain compressor: compress SFX when VO is loud
  filterGraph.push({
    ...
    inputs: [sfxAudioOut, 'a_vo_padded_1'],
    outputs: 'a_sfx_ducked'
  });

  // Mix ducked SFX and VO
  filterGraph.push({
    filter: 'amix',
    options: { inputs: 2, weights: '1 1' },
    inputs: ['a_sfx_ducked', 'a_vo_padded_2'],
    outputs: 'a_final_mixed'
  });
  command.complexFilter(filterGraph);
  ...
  return new Promise((resolve, reject) => {
    command
      .on('start', (cmdLine) => console.log('Spawned FFmpeg command:', cmdLine))
      .save(resolvedOutput)
