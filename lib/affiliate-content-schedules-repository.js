...
export async function listAffiliateSchedules({
  ...
  if (brandProfileId && brandName && brandName !== 'all') {
    conditions.push(`(s.brand_profile_id = $${paramIdx++} OR LOWER(s.brand_name) = LOWER($${paramIdx++}))`);
    params.push(brandProfileId, brandName);
  } else if (brandProfileId) {
    conditions.push(`s.brand_profile_id = $${paramIdx++}`);
    params.push(brandProfileId);
  } else if (brandName && brandName !== 'all') {
    conditions.push(`LOWER(s.brand_name) = LOWER($${paramIdx++})`);
  ...
  const sql = `
    SELECT 
      ...
      p.clean_photo_url AS db_clean_photo_url,
...
export async function createAffiliatePlanSchedules({
  ...
  if (!brandName && !brandProfileId) throw new Error('Brand identifier wajib diisi.');
    ...
    for (const item of items) {
      ...
      const insertRes = await client.query(`
        INSERT INTO affiliate_content_schedules (
          id, tenant_id, brand_profile_id, plan_type, brand_name, product_id, product_name,
