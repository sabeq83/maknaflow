...
import { listBrandProducts } from './brand-product-repository.js';
...
export async function listEligibleAutomationProducts({ brandProfileId, search = '' }) {
  ...
  const rows = await listBrandProducts({ brandProfileId, includeUnlinked: true, search });
  const products = rows.map(row => ({
    ...row,
    product_name: row.product_name_override || row.product_name,
    image_url: row.clean_photo_url || row.raw_photo_url || null,
    is_linked: Boolean(row.brand_product_id && row.is_active),
    binding_active: Boolean(row.is_active),
    completeness: {
      description: Boolean(row.product_description), usp: Boolean(row.unique_selling_point),
      image: Boolean(row.clean_photo_url || row.raw_photo_url), target_audience: Boolean(row.target_audience),
      product_url: Boolean(row.source_url || row.landing_page_url), affiliate: Boolean(row.affiliate_link)
    }
  }));
  return { products, summary: { total: products.length, linked: products.filter(row => row.is_linked).length, unlinked: products.filter(row => !row.is_linked).length } };
}
...
export async function captureProductSnapshot({ brandProfileId, productId, brandProductId }) {
  ...
  const result = await pgQuery(`
    SELECT p.id AS product_id,bp.id AS brand_product_id,b.id AS brand_profile_id,b.brand_name,
      ...
      bp.affiliate_link,bp.tracking_code,bp.landing_page_url,bp.product_name_override,bp.cta_override
  ...
  const snapshot = {
    ...
    product_name: row.product_name_override || row.product_name, product_description: row.product_description,
    ...
    tracking_code: row.tracking_code || '', cta_override: row.cta_override || '', product_truth: row.product_truth || '',
