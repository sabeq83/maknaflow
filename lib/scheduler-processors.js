...
export async function processDeconstruct(payload, job) {
  ...
  try {
    ...
    console.log(`[DECONSTRUCT] ${label} Downloading video: ${asset.source_url} (tenant: ${tenantId})`);
    ...
    const responseText = await executeWithKeyPool(1, async (apiKey) => {
      ...
      console.log(`[DECONSTRUCT] ${label} Upload success to Gemini: ${uploadResult.fileUri}`);
      ...
      const promptText = buildDeconstructPhase1Prompt(targetRecommendationCount);
