    ...
    if (shouldRunT2i) {
      ...
      try {
        ...
        const submitStartFrame = async (clipIndex, prompt) => {
          ...
          let extraReferences = [];
          if (isCartoonWorld) {
            // Cartoon universe: gunakan cartoon-reference-resolver untuk referensi karakter + produk
            ...
            // [FIX] Real-world: teruskan productBase64 ke bridge clips
            // Sebelumnya productBase64 di-resolve (baris 5246) tapi tidak pernah diteruskan
            // ke buildOpcStartFrameRequest, menyebabkan reference_images kosong ke G-Labs.
            // Fix ini menyejajarkan perilaku Phase 1 dengan regenerate-t2i/route.js baris 154-156.
            if (isBridge && productBase64) {
              extraReferences = [productBase64];
              console.log(`[OPC Analyzer T2I] ✅ Bridge clip ${cNum} (${bridgeAtClip}–${productEndClip}): product reference image akan dikirim ke G-Labs.`);
            }
          }
          const built = await buildOpcStartFrameRequest({ campaign: tempCampaign, item: { ...item, new_video_plan_json: JSON.stringify(newVideoPlan) }, clipIndex, prompt, origin: 'phase_1_initial', extraReferences });
    ...
    for (let c = 1; c <= targetClips; c++) {
      ...
      if (isHybridLockClip) {
        ...
        if (startFrameExists) {
          ...
          const rowPayload = item.row_creative_payload ? JSON.parse(item.row_creative_payload) : {};
          let productBase64 = await resolveProductBase64(campaign, productData, rowPayload);
          ...
          const runT2I = async (refs) => {
            const { buildOpcStartFrameRequest } = await import('./opc-start-frame-request.js');
            const { recordStartFrameRequestAudit } = await import('./opc-start-frame-audit.js');
            const built = await buildOpcStartFrameRequest({ campaign, item, clipIndex: c, prompt: t2iPromptText, origin: 'production_recovery', extraReferences: isCartoon ? (refs || []) : [] });
