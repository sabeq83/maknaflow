...
export async function processDeconstruct(payload, job) {
  ...
  const { uploadVideoToGemini, generateWithVideo, getGeminiModel } = await import('./gemini');
  ...
  try {
    ...
    const uploadResult = await uploadVideoToGemini(tempFilePath, mimeType);
    console.log(`[DECONSTRUCT] Upload success to Gemini: ${uploadResult.name}`);
    ...
    // Choose visual reasoning model (Flash/Pro)
    const model = getGeminiModel();
    const responseText = await generateWithVideo(uploadResult.name, mimeType, promptText, model);
    const parsed = parseGeminiJSON(responseText);
