...
import { executeWithKeyPool, GEMINI_MODELS, makeModelResilient, getResolvedGeminiModels } from './gemini.js';
...
export async function resolveProductBase64(campaign, productData = null, rowPayload = null) {
  try {
    const fs = require('fs');
    ...
    if (campaign?.target_product_id || campaign?.product_id) {
      try {
        ...
        const p = await getProductById(campaign.target_product_id || campaign.product_id);
        if (p) {
          const studioPhoto = p.clean_photo_url || p.cleaned_photo_url || p.raw_photo_url || p.photo_url;
          const b64 = fileToBase64(studioPhoto);
          if (b64) {
            console.log(`[resolveProductBase64] ✅ Resolved from DB product (id=${campaign.target_product_id || campaign.product_id}): ${studioPhoto}`);
            return b64;
          }
        }
      } catch (e) {
        console.warn('[resolveProductBase64] DB product lookup failed:', e.message);
      }
    }
...
export async function processScraper(payload, job) {
  ...
  const { downloadFromUrl, moveToLibrary, generateThumbnail, getFileSize } = await import('./video-downloader.js');
  const { createVideoLibraryEntry } = await import('./db.js');
  const { RE_PIPELINE_HEADERS } = await import('./export-builder.js');
...
export async function processAnalyzer(payload, job) {
  ...
  const { getAllKnowledgeBasesWithContent, createReverseResult, getSetting: getS } = await import('./db.js');
  const { buildReverseEngineeringPrompt } = await import('./prompts.js');
  const { uploadVideoToGemini, generateWithVideo } = await import('./gemini.js');
  const { v4: uuidv4 } = await import('uuid');
  const {
  ...
  } = await import('./export-builder.js');
  ...
  try {
    ...
    const { getVideoById, getAllVideoLibrary } = await import('./db.js');
    ...
    const result = await executeWithKeyPool(1, async (apiKey) => {
      const { getMimeType } = await import('./video-downloader.js');
    ...
    try {
      ...
      try {
        ...
        const { getReCampaign } = await import('./db.js');
        ...
        if (storageProvider === 'nextcloud') {
          ...
          const { getOrCreateCampaignFolder, getOrCreateFolderInFolder, uploadMarkdownToCampaignFolder } = await import('./drive-uploader.js');
...
export async function processIdeation(payload, job) {
  const { parseGeminiJSON } = await import('./json-parser.js');
  const { getAllKnowledgeBasesWithContent, getAllProductExtractions, createIdea } = await import('./db.js');
  const { buildIdeationPrompt } = await import('./prompts.js');
  const { v4: uuidv4 } = await import('uuid');
  const { generateBatchId, PIPELINE_HEADERS, buildIdeationRow } = await import('./export-builder.js');
  ...
  const { getAllIdeas, getProductExtraction } = await import('./db.js');
...
export async function processProduction(payload, job) {
  const { parseGeminiJSON } = await import('./json-parser.js');
  const { getAllKnowledgeBasesWithContent, getIdea, createAsset, updateIdeaStatus } = await import('./db.js');
  const { buildProductionPrompt } = await import('./prompts.js');
  const { v4: uuidv4 } = await import('uuid');
  const { buildProductionRows, buildCaptionsRow } = await import('./export-builder.js');
...
export async function processReScraper(payload, job) {
  const { downloadFromUrl, moveToLibrary, generateThumbnail, getFileSize } = await import('./video-downloader.js');
  const { getNextPendingScrapeItem, updateReCampaignItem } = await import('./db.js');
  const path = await import('path');
  const { v4: uuidv4 } = await import('uuid');
  ...
  try {
    ...
    const { logSystemError } = await import('./error-logger.js');
...
export async function processReAnalyzer(payload, job) {
  ...
  const { buildReverseEngineeringPrompt, WARDROBE_PRESETS, DEMOGRAPHIC_PRESETS, LIGHTING_PRESETS } = await import('./prompts.js');
  const { uploadVideoToGemini, generateWithVideo } = await import('./gemini.js');
  const {
    generateREBatchId, buildREResultRow, buildREStoryboardRows,
    buildREVoiceoverRows, buildREPromptRows, buildRECaptionsRow, buildMarkdownContent,
  ...
  const { createSpreadsheet, uploadREMarkdownToDrive } = await import('./drive-uploader.js');
  const { v4: uuidv4 } = await import('uuid');
  ...
  try {
    ...
    if (campaign.visual_overrides_json) {
      try {
        ...
        console.warn('[RE Analyzer] Failed to parse and resolve visual_overrides_json:', e.message);
      }
    }
    ...
    const result = await executeWithKeyPool(1, async (apiKey) => {
      ...
      if (campaign.is_bridging_active) {
        ...
        const { buildReverseEngineeringBridgePrompt } = await import('./prompts.js');
        prompt = buildReverseEngineeringBridgePrompt(kbs, {
          ...
          clip_duration: campaign.clip_duration || 8,
      ...
      const { generateContentFlexible } = await import('./gemini.js');
    ...
    if (campaign.enable_audio_segment === 1) {
      ...
      const { registerJitVoiceCast } = await import('./minimax-tts.js');
    ...
    try {
      let batchId = '';
      if (uploadSpreadsheet && sheets && spreadsheetId) {
        const { ensureTabAndAppendRows, updateCell, getColumnLetter } = await import('./sheets-autopilot-worker.js');
...
export async function processGLabsCampaign(payload, job) {
  ...
  const { listGlabsCampaigns, updateGlabsCampaign } = await import('./db.js');
  const { uploadVideoToFolder } = await import('./drive-uploader.js');
  const path = await import('path');
  const fs = await import('fs');
          ...
          try {
            ...
            if (videoFile) {
              ...
              if (response.ok) {
                ...
                if (storageProvider === 'nextcloud') {
                  const { uploadBufferToNextcloud } = await import('./nextcloud-helper.js');
...
export async function processGLabs(payload, job) {
  ...
  const { generateVideo, getFileUrl } = await import('./webhook-client.js');
...
export async function resolveProductData(campaignRow) {
  ...
  if (bridging_mode === 'url_extract') {
    ...
    const { scrapeUrl } = await import('./url-scraper.js');
    const { buildProductAgentPrompt } = await import('./prompts.js');
    const { getGeminiModel } = await import('./gemini.js');
    const { parseGeminiJSON } = await import('./json-parser.js');
    ...
    if (campaignRow.visual_mode === 'hybrid_lock' && scrapedImageUrl) {
      const path = await import('path');
      const fs = await import('fs');
      const { downloadECommerceImage } = await import('./url-scraper.js');
...
export async function processReTts(payload, job) {
  const { getNextPendingReTtsItem, updateReCampaignItem, getReCampaign, createTtsBatch, createTtsClip, updateTtsClip, getDb, getSetting } = await import('./db.js');
  const db = getDb();
  const { generateMinimaxVO } = await import('./minimax-tts.js');
  const { callGeminiTtsApi } = await import('./gemini.js');
  const { convertPcmToWav } = await import('./audio-helper.js');
  const path = await import('path');
  const fs = await import('fs');
  ...
  try {
    ...
    if (provider !== 'minimax' && voiceover.length > 1 && campaign.enable_audio_segment !== 1) {
      try {
        const { generateGeminiBatchTTS } = await import('./gemini-tts-batch.js');
...
export async function processReGlabs(payload, job) {
  const { getNextPendingReGlabsItem, updateReCampaignItem, getSetting, setSetting, getDb, insertGlabsTask, updateGlabsTaskStatus } = await import('./db.js');
  const { generateVideo, generateImage, getTaskStatus, getFileUrl } = await import('./webhook-client.js');
  const path = await import('path');
  const fs = await import('fs');
  ...
  if (payload?.target_type === 'angle_variant') {
    ...
    if (enableTts && variant.tts_status !== 'completed') {
      ...
      try {
        ...
        const { createTtsBatch, createTtsClip, updateTtsClip } = await import('./db.js');
        const { generateMinimaxVO } = await import('./minimax-tts.js');
        const { generateGeminiExact8sAudio } = await import('./gemini-8s-tts.js');
    ...
    if (enableFfmpeg) {
      ...
      try {
        const { processTtsStudioMuxing } = await import('./video-studio-processor.js');
    ...
    try {
      ...
      if (storageProvider === 'nextcloud') {
        ...
        const { uploadVideoToFolder, uploadLocalFileToFolder } = await import('./drive-uploader.js');
  ...
  try {
    for (let c = 1; c <= targetClips; c++) {
      ...
      if (isHybridLockClip) {
        ...
        let effectiveStartFrameRelativePath = null;
        let effectiveStartFrameLocalPath = null;
        let startFrameExists = false;

        const { resolvePublicFilePath, findExistingStartFrameFile } = await import('./file-path-resolver.js');

        // Layer 1: Cek t2i_images_json
        const t2iImages = item.t2i_images_json ? (typeof item.t2i_images_json === 'string' ? JSON.parse(item.t2i_images_json) : item.t2i_images_json) : [];
        if (Array.isArray(t2iImages) && t2iImages[c - 1]) {
          const registered = t2iImages[c - 1];
          const resolved = resolvePublicFilePath(registered);
          if (resolved) {
            effectiveStartFrameRelativePath = registered.startsWith('/') ? registered : `/${registered}`;
            effectiveStartFrameLocalPath = resolved;
            startFrameExists = true;
          }
        }

        // Layer 2: Cek re_campaign_item_assets
        if (!startFrameExists) {
          try {
            const { pgQuery } = await import('./db-pg.js');
            const assetRow = await pgQuery(
              `SELECT local_path FROM re_campaign_item_assets 
               WHERE campaign_item_id = $1 AND clip_index = $2 AND asset_type = 'start_frame' AND status = 'completed'
               ORDER BY revision DESC LIMIT 1`,
              [String(item.id), c]
            );
            if (assetRow.rows.length > 0 && assetRow.rows[0].local_path) {
              const regAsset = assetRow.rows[0].local_path;
              const resolved = resolvePublicFilePath(regAsset);
              if (resolved) {
                effectiveStartFrameRelativePath = regAsset.startsWith('/') ? regAsset : `/${regAsset}`;
                effectiveStartFrameLocalPath = resolved;
                startFrameExists = true;
              }
            }
        ...
        // Layer 3: Cek disk pattern scanner
        if (!startFrameExists) {
          const diskMatch = findExistingStartFrameFile({ itemId: item.id, clipIndex: c, prefix: 'start_frame' });
          if (diskMatch) {
            effectiveStartFrameRelativePath = diskMatch.webRelativePath;
            effectiveStartFrameLocalPath = diskMatch.absolutePath;
            startFrameExists = true;
          }
        }

        if (startFrameExists) {
          console.log(`[RE Glabs] Reusing existing start frame for clip ${c}: ${effectiveStartFrameRelativePath}. Skipping T2I stage.`);
        } else {
          const productBase64 = await resolveProductBase64(campaign, productData);
...
export async function processReFfmpeg(payload, job) {
  const { getNextPendingReFfmpegItem, updateReCampaignItem, getReCampaign, getSetting, setSetting, getDb } = await import('./db.js');
  const { processTtsStudioMuxing } = await import('./video-studio-processor.js');
  const { google } = await import('googleapis');
  const { getAuthorizedClient } = await import('./google-auth.js');
  const path = await import('path');
    ...
    if (campaign.enable_tts === 0 && campaign.enable_social_post === 0) {
      ...
      try {
        ...
        if (item.tts_batch_id && item.tts_batch_id !== 'skipped') {
          try {
            const { uploadLocalFileToFolder } = await import('./drive-uploader.js');
  ...
  try {
    ...
    if (hasNextcloud) {
      try {
        ...
        if (storageProvider === 'nextcloud') {
          const { getOrCreatePublicShareLink } = await import('./nextcloud-helper.js');
    ...
    try {
      const { syncCampaignToContentFlow } = await import('./contentflow-ingest.js');
...
export async function processReSocialPoster(payload, job) {
  const { getNextPendingSocialPostItem, updateReCampaignItem, getReCampaign } = await import('./db.js');
  const { google } = await import('googleapis');
  const { getAuthorizedClient } = await import('./google-auth.js');
  const path = await import('path');
...
export async function processItemMultiAngle(itemId, angleCount) {
  const { getDb, updateReCampaignItem } = await import('./db.js');
  const { callGeminiAPI } = await import('./gemini.js');
  const { buildMultiAngleMultiplierPrompt } = await import('./prompts.js');
  const { parseGeminiJSON } = await import('./json-parser.js');
...
export async function processRowProductSourcing(payload, job) {
  const { getPillarCampaignItem, updatePillarCampaignItem, getDb } = await import('./db.js');
  const { scrapeUrl, downloadECommerceImage } = await import('./url-scraper.js');
  const { buildProductSourcingPrompt } = await import('./prompts.js');
  ...
  try {
    ...
    const extractedData = await executeWithKeyPool(1, async (apiKey) => {
      ...
      const resilientModel = makeModelResilient(model, apiKey, models.PRIMARY, true);
      const result = await resilientModel.generateContent(prompt);
...
export async function processPillarGenerator(payload, job) {
  const { parseGeminiJSON } = await import('./json-parser.js');
  const { getPillarCampaignItem, updatePillarCampaignItem, getPillarCampaign, getSetting, getNextPendingPillarGeneratorItem, getDb } = await import('./db.js');
  const { buildOrganicPillarPrompt, WARDROBE_PRESETS, DEMOGRAPHIC_PRESETS, LIGHTING_PRESETS } = await import('./prompts.js');
  const { executeWithKeyPool } = await import('./gemini.js');
  const { GoogleGenerativeAI } = await import('@google/generative-ai');
  const path = await import('path');
  ...
  let rowPayload = {};
  try {
    rowPayload = typeof item.row_creative_payload === 'string'
  ...
  if (item.content_kind === 'recipe_campaign' || rowPayload.content_kind === 'recipe_campaign') {
    const { processRecipeCampaignCreative } = await import('./recipe-production-adapter.js');
    return processRecipeCampaignCreative({ item, campaign, job });
  }
  ...
  try {
    ...
    if (effectiveContentWorld === 'cartoon_universe') {
      try {
        const { getWorldAwareKB } = await import('./kb-loader.js');
        const worldContext = {
          contentWorld: effectiveContentWorld,
          knowledgeDomain: rowPayload?.knowledge_domain || campaign.knowledge_domain || 'general',
          universeProfile: rowPayload?.universe_profile || campaign.universe_profile
        };
        const worldAwareKbContent = getWorldAwareKB(worldContext);
        if (worldAwareKbContent) {
          kbs.push({ name: 'WORLD_AWARE_KBS', content: worldAwareKbContent });
          console.log(`[OPC Cartoon] Loaded centralized cartoon KBs for item ${item.id}`);
        }
      } catch (kbErr) {
    ...
    if (isMassProd) {
      ...
      // Product reference image tidak di-resolve atau dimutasi di tahap creative.
      // Shared start-frame builder memilih foto Clean berdasarkan target_product_id.
    } else {
      productData = await resolveProductData(campaign);
    }
    ...
    let newVideoPlan = storyboardList.map((s, idx) => {
      ...
      return {
        clip_index: clipNum,
        product_visible: s.product_visible === true || s.requires_product_reference === true || (Number(clipNum) >= Number(campaign.bridge_at_clip || 2) && Number(clipNum) < Number(campaign.bridge_at_clip || 2) + Math.max(1, Number(campaign.bridge_duration_clips || 1))),
        requires_product_reference: s.requires_product_reference === true || (Number(clipNum) >= Number(campaign.bridge_at_clip || 2) && Number(clipNum) < Number(campaign.bridge_at_clip || 2) + Math.max(1, Number(campaign.bridge_duration_clips || 1))),
    ...
    // 1. Run VO compliance checker if enabled
    let voiceover = newVideoPlan.map((p, idx) => ({
      scene: p.clip_index || (idx + 1),
      narration: p.new_vo || "",
      duration: "8s",
      cta_facebook: p.cta_facebook || "",
    ...
    const isManualReview = (campaign.execution_mode === 'manual_review' || campaign.approval_mode === 'creative' || campaign.approval_mode === 'storyboard' || campaign.scheduler_pause_at === 'tts' || campaign.scheduler_pause_at === 'start_frames');
    const shouldRunT2i = campaign.visual_mode === 'hybrid_lock' && !isManualReview;

    if (isManualReview) {
      console.log(`[OPC Generator] Item #${item.id}: Mode Manual Review aktif (${campaign.execution_mode || campaign.approval_mode}). Fase 1 selesai (Storyboard & VO). Menjeda sebelum generasi Start Frame T2I.`);
    } else if (shouldRunT2i) {
      console.log(`[OPC Analyzer T2I] Mode Autopilot hybrid_lock aktif. Memulai pre-rendering T2I untuk seluruh ${targetClips} klip...`);
      await updatePillarCampaignItem(item.id, { generation_progress: `Pre-rendering Start Frames (0/${targetClips})` }).catch(() => {});
      try {
        const { generateImage, getTaskStatus, getFileUrl, GLABS_IMAGE_POLL_INTERVAL_MS } = await import('./webhook-client.js');
        const pollInterval = GLABS_IMAGE_POLL_INTERVAL_MS || 4000;
        ...
        const { buildOpcStartFrameRequest } = await import('./opc-start-frame-request.js');
        const { recordStartFrameRequestAudit } = await import('./opc-start-frame-audit.js');
        // [FIX v_patch] Resolve referensi kartun untuk content_world cartoon_universe
        const isCartoonWorld = tempCampaign.content_world === 'cartoon_universe' || rowPayload?.content_world === 'cartoon_universe';
        let universeSnapshotForT2i = null;
        let visualIdentitySnapshotForT2i = null;
        if (isCartoonWorld) {
          try { universeSnapshotForT2i = tempCampaign.universe_snapshot_json ? JSON.parse(tempCampaign.universe_snapshot_json) : null; } catch (_) {}
          try { visualIdentitySnapshotForT2i = tempCampaign.visual_overrides_json ? JSON.parse(tempCampaign.visual_overrides_json) : null; } catch (_) {}
        }
        const submitStartFrame = async (clipIndex, prompt) => {
          const cNum = Number(clipIndex);
          let contextReferences = [];
          if (isCartoonWorld) {
            // Product reference dimiliki shared builder; resolver ini hanya memasok character/style context.
            const planEntry = newVideoPlan.find(p => Number(p.clip_index) === cNum) || {};
            const clipCharacters = Array.isArray(planEntry.characters) ? planEntry.characters : [];
            const { normalizeCharacterId } = await import('./universe-manifests.js');
            const { resolveClipReferenceImages } = await import('./cartoon-reference-resolver.js');
            ...
            const resolvedRefs = resolveClipReferenceImages({
              ...
              universeProfile: tempCampaign.universe_profile || rowPayload?.universe_profile || 'pawville',
              universeSnapshot: universeSnapshotForT2i,
              visualIdentitySnapshot: visualIdentitySnapshotForT2i,
              clip: clipIndex,
              productReference: null,
              productRevealBeat: rowPayload?.product_reveal_beat || tempCampaign.product_reveal_beat || 'none',
              clipCharacters: normalizedClipChars
            });
            contextReferences = resolvedRefs.allReferences || [];
          }
          const built = await buildOpcStartFrameRequest({ campaign: tempCampaign, item: { ...item, new_video_plan_json: JSON.stringify(newVideoPlan) }, clipIndex, prompt, origin: 'phase_1_initial', contextReferences });
          const result = await generateImage(built.providerRequest);
          if (result?.task_id) await recordStartFrameRequestAudit(built.audit, result.task_id);
          return result;
        };

        const { saveRevisionedStartFrame } = await import('./start-frame-storage.js');
        const { withGlabsImageLane, attachProviderTask } = await import('./glabs-image-lane-service.js');
        const { recordCompletedStartFrameAsset } = await import('./pillar-start-frame-service.js');

        const downloadAndSaveStartFrame = async (clipIndex, taskId, files) => {
          let imageFile = files.find(f => f.endsWith('.png') || f.endsWith('.jpg') || f.endsWith('.jpeg') || f.endsWith('.webp')) || files[0];
          if (imageFile && (imageFile.startsWith('http://') || imageFile.startsWith('https://'))) {
            imageFile = imageFile.split('/').pop();
          }
          if (!imageFile) return false;
          const completedUrl = getFileUrl(imageFile, taskId);
          try {
            const imgResponse = await fetch(completedUrl);
            if (imgResponse.ok) {
              const imgBuffer = Buffer.from(await imgResponse.arrayBuffer());
              ...
              t2iImagePaths[clipIndex - 1] = saved.relativePath;
              console.log(`[OPC Analyzer T2I] Saved image for clip ${clipIndex}: ${saved.relativePath}`);
              await recordCompletedStartFrameAsset(item.id, {
                clipIndex,
                localPath: saved.relativePath,
                revision: 1
              }).catch(() => {});
              const savedCount = t2iImagePaths.filter(Boolean).length;
              await updatePillarCampaignItem(item.id, { generation_progress: `T2I Tersimpan ${savedCount}/${targetClips}` }).catch(() => {});
              return true;
            } else {
              console.warn(`[OPC Analyzer T2I] Failed to download image for clip ${clipIndex}: status ${imgResponse.status}`);
            }
          } catch (err) {
            console.warn(`[OPC Analyzer T2I] Failed to download image for clip ${clipIndex}:`, err.message);
          }
          return false;
        };

        const executeSingleClipIsolated = async (clipIndex, promptText) => {
          try {
            await withGlabsImageLane({
              tenantId: tempCampaign.tenant_id,
              webhookOverride: null,
              mode: 'exclusive',
              ownerKind: 'pillar_initial_exclusive',
              ownerId: `${item.id}:${clipIndex}:init`,
              campaignId: String(tempCampaign.id),
              campaignItemId: String(item.id),
              leaseSeconds: 180
            }, async (lane) => {
              console.log(`[OPC Analyzer T2I] [Isolated] Acquired exclusive lane ${lane.lane_key}. Submitting T2I task for clip ${clipIndex}...`);
              await updatePillarCampaignItem(item.id, { generation_progress: `T2I Klip ${clipIndex} (Product Bridge Locked)` }).catch(() => {});
              const t2iResult = await submitStartFrame(clipIndex, promptText);
              if (!t2iResult?.task_id) return;
              const t2iTaskId = t2iResult.task_id;
              await attachProviderTask(lane.id, t2iTaskId);
              console.log(`[OPC Analyzer T2I] [Isolated] Task ${t2iTaskId} submitted. Polling status with ${pollInterval}ms interval...`);

              const maxAttempts = 40; // 160s at 4s
              for (let attempt = 0; attempt < maxAttempts; attempt++) {
                ...
                if (taskStatus === 'completed') {
                  const files = statusResult.results || statusResult.files || [];
                  await downloadAndSaveStartFrame(clipIndex, t2iTaskId, files);
                  break;
                } else if (taskStatus === 'failed') {
                  console.error(`[OPC Analyzer T2I] [Isolated] Task ${t2iTaskId} failed for clip ${clipIndex}`);
                  break;
                }
              }
            });
          } catch (err) {
            console.error(`[OPC Analyzer T2I] [Isolated] Error processing clip ${clipIndex}:`, err.message);
          }
        };

        const executeThreadedBatch = async (clipBatch) => {
          if (!clipBatch.length) return;
          const clipLabels = clipBatch.map(c => c.clipIndex).join(', ');
          ...
          try {
            await withGlabsImageLane({
              ...
              webhookOverride: null,
              mode: 'shared',
              ownerKind: 'pillar_initial_shared',
              ownerId: `${item.id}:${clipBatch.map(c => c.clipIndex).join('_')}:init`,
              campaignId: String(tempCampaign.id),
              campaignItemId: String(item.id),
              leaseSeconds: 240
            }, async (lane) => {
              console.log(`[OPC Analyzer T2I] [Threading] Acquired shared lane ${lane.lane_key} for clips ${clipLabels}...`);
              await updatePillarCampaignItem(item.id, { generation_progress: `T2I Klip ${clipLabels}` }).catch(() => {});
              const submittedTasks = [];

              for (let i = 0; i < clipBatch.length; i++) {
                const { clipIndex, promptText } = clipBatch[i];
                try {
                  console.log(`[OPC Analyzer T2I] [Threading] Submitting T2I task for clip ${clipIndex}...`);
                  const t2iResult = await submitStartFrame(clipIndex, promptText);
                  if (t2iResult?.task_id) {
                    submittedTasks.push({ clipIndex, taskId: t2iResult.task_id, completed: false, files: null, failed: false });
                  }
                } catch (err) {
                  console.warn(`[OPC Analyzer T2I] [Threading] Failed to submit T2I for clip ${clipIndex}:`, err.message);
                }
              }

              if (submittedTasks.length > 0) {
                console.log(`[OPC Analyzer T2I] [Threading] Polling ${submittedTasks.length} tasks with ${pollInterval}ms interval...`);
                ...
                for (let attempt = 0; attempt < maxAttempts; attempt++) {
                  ...
                  for (const task of submittedTasks) {
                    ...
                    try {
                      ...
                      const taskStatus = (statusResult?.status || '').toLowerCase();
                      if (taskStatus === 'completed') {
                        task.files = statusResult.results || statusResult.files || [];
                        task.completed = true;
                      } else if (taskStatus === 'failed') {
                        task.failed = true;
                        console.warn(`[OPC Analyzer T2I] [Threading] Task ${task.taskId} for clip ${task.clipIndex} failed.`);
                      }
                    } catch (err) {
                ...
                for (const task of submittedTasks) {
                  if (task.completed && task.files) {
                    await downloadAndSaveStartFrame(task.clipIndex, task.taskId, task.files);
                  }
                }
              }
            });
          } catch (err) {
            console.error(`[OPC Analyzer T2I] [Threading] Error in batch ${clipLabels}:`, err.message);
          }
        };
        ...
        if (t2iPattern === 'sequential') {
          ...
          for (let c = 1; c <= targetClips; c++) {
            const t2iPromptText = newVideoPlan.find(p => Number(p.clip_index) === c)?.t2i_prompt || "";
            if (t2iPromptText) {
              await executeSingleClipIsolated(c, t2iPromptText);
            }
          }
        } else {
          // --- THREADING PATTERN WITH PRODUCT BRIDGE ISOLATION ---
          const isBridging = Boolean(tempCampaign.is_bridging_active && tempCampaign.is_bridging_active !== '0' && tempCampaign.is_bridging_active !== 0);
          const isProductCampaign = Boolean(tempCampaign.target_product_id || tempCampaign.product_id);
          const bridgeStart = isBridging ? Math.max(1, Number(tempCampaign.bridge_at_clip) || 2) : -1;
          const rawBridgeDuration = isBridging ? (tempCampaign.bridge_duration_clips !== undefined ? Number(tempCampaign.bridge_duration_clips) : 1) : 0;
          const bridgeEnd = isBridging
            ? (rawBridgeDuration > 0 ? (bridgeStart + rawBridgeDuration - 1) : targetClips)
            : -1;
          ...
          for (let c = 1; c <= targetClips; c++) {
            const promptText = newVideoPlan.find(p => Number(p.clip_index) === c)?.t2i_prompt || "";
            if (!promptText) continue;
            const isBridgeClip = isBridging && isProductCampaign && c >= bridgeStart && c <= bridgeEnd;
            if (isBridgeClip) {
              bridgeClips.push({ clipIndex: c, promptText });
            } else if (isBridging && isProductCampaign && c < bridgeStart) {
              beforeBridge.push({ clipIndex: c, promptText });
            } else {
              afterBridge.push({ clipIndex: c, promptText });
            }
          }

          // 1. Threaded before-bridge tasks -> drain until terminal
          if (beforeBridge.length > 0) {
            console.log(`[OPC Analyzer T2I] Executing ${beforeBridge.length} before-bridge tasks...`);
            await executeThreadedBatch(beforeBridge);
          }

          // 2. Reference-critical bridge task(s) -> submit + poll + download isolated one by one
          for (const bridgeClip of bridgeClips) {
            console.log(`[OPC Analyzer T2I] Executing reference-critical bridge clip ${bridgeClip.clipIndex} isolated...`);
            await executeSingleClipIsolated(bridgeClip.clipIndex, bridgeClip.promptText);
          }

          // 3. Threaded after-bridge tasks -> submit after bridge terminal
          if (afterBridge.length > 0) {
            console.log(`[OPC Analyzer T2I] Executing ${afterBridge.length} after-bridge tasks...`);
            await executeThreadedBatch(afterBridge);
          }
        }
      } catch (err) {
        console.error('[OPC Analyzer T2I] Error during pre-rendering T2I:', err);
        await updatePillarCampaignItem(item.id, { generation_progress: null }).catch(() => {});
      }
    }
    ...
    await updatePillarCampaignItem(item.id, {
      ...
      video_dna_json: JSON.stringify(videoDna),
      t2i_images_json: JSON.stringify(t2iImagePaths),
      workflow_status: campaign.execution_mode === 'full_autopilot' ? 'production_processing' : 'creative_processing'
    });
    const { finalizeStartFrameCheckpoint } = await import('./pillar-start-frame-service.js');
    await finalizeStartFrameCheckpoint(item.id, { paths: t2iImagePaths });
    ...
    if (uploadSpreadsheet || uploadMarkdown) {
      try {
        const { google } = await import('googleapis');
        const { getAuthorizedClient } = await import('./google-auth.js');
        const { createSpreadsheet, getOrCreateCampaignFolder, uploadMarkdownToCampaignFolder } = await import('./drive-uploader.js');
        const {
        ...
        if (uploadMarkdown) {
          ...
          if (storageProvider === 'nextcloud') {
            ...
            const { getOrCreateCampaignFolder, getOrCreateFolderInFolder } = await import('./drive-uploader.js');
            const campaignFolderId = await getOrCreateCampaignFolder(parentFolderName, '');
            const batchFolderId = await getOrCreateFolderInFolder(batchId, campaignFolderId);
            folderUrl = `https://drive.google.com/drive/folders/${batchFolderId}`;
          ...
          const { updatePillarCampaignItem } = await import('./db.js');
          updatePillarCampaignItem(item.id, { target_markdown_url: driveUrl });
        }

        if (uploadSpreadsheet) {
          const { ensureTabAndAppendRows, updateCell } = await import('./sheets-autopilot-worker.js');
          if (!targetSpreadsheetId) {
            ...
            const { updatePillarCampaign } = await import('./db.js');
    ...
    // QUOTA_EXHAUSTED = semua model Gemini sedang 503 sementara.
    // Jangan tandai 'failed' — reset ke 'pending' agar scheduler mencoba ulang di putaran berikutnya
    // tanpa memicu infinite loop dari mekanisme auto-recovery 'processing' scheduler.
    const isAllModelsDown = (error.message || '').includes('QUOTA_EXHAUSTED');
    if (isAllModelsDown) {
      console.warn(`[Pillar Generator] ⏳ Semua model Gemini overloaded untuk item #${item.id}. Status direset ke 'pending' untuk retry otomatis di putaran berikutnya.`);
      await updatePillarCampaignItem(item.id, { generation_status: 'pending', generation_progress: null });
      return { retryLater: true, item_id: item.id, reason: error.message };
    }

    await logSystemError('OPC Generator', error, item.campaign_id);
    await updatePillarCampaignItem(item.id, { generation_status: 'failed', generation_progress: null });
...
export async function processPillarTts(payload, job) {
  const { getNextPendingPillarTtsItem, updatePillarCampaignItem, getPillarCampaign, createTtsBatch, createTtsClip, updateTtsClip, getDb, getSetting } = await import('./db.js');
  const db = getDb();
  const { generateMinimaxVO } = await import('./minimax-tts.js');
  const { callGeminiTtsApi } = await import('./gemini.js');
  const path = await import('path');
...
export async function processPillarGlabs(payload, job) {
  const { getNextPendingPillarGlabsItem, updatePillarCampaignItem, getPillarCampaign, getSetting, setSetting, getDb, insertGlabsTask, updateGlabsTaskStatus } = await import('./db.js');
  const { generateVideo, getTaskStatus, getFileUrl } = await import('./webhook-client.js');
  const path = await import('path');
  const fs = await import('fs');
  ...
  if (item) {
    ...
    // Resolve productData using the same fuzzy lookup logic as Re-Gen to avoid mismatched start frame images
  ...
  try {
    for (let c = 1; c <= targetClips; c++) {
      ...
      if (isHybridLockClip) {
        ...
        // Layer 2: Cek pillar_campaign_item_assets
        if (!startFrameExists) {
          try {
            const { pgQuery } = await import('./db-pg.js');
            const assetRow = await pgQuery(
              `SELECT local_path FROM pillar_campaign_item_assets 
               WHERE campaign_item_id = $1 AND clip_index = $2 AND asset_type = 'start_frame' AND status = 'completed'
               ORDER BY revision DESC LIMIT 1`,
              [String(item.id), c]
            );
        ...
        // Layer 3: Cek disk pattern scanner
        if (!startFrameExists) {
          const diskMatch = findExistingStartFrameFile({ itemId: item.id, clipIndex: c, prefix: 'opc_start_frame' });
          if (diskMatch) {
            effectiveStartFrameRelativePath = diskMatch.webRelativePath;
            effectiveStartFrameLocalPath = diskMatch.absolutePath;
            startFrameExists = true;
        ...
        if (startFrameExists) {
          console.log(`[Pillar Glabs] Reusing existing start frame for clip ${c}: ${effectiveStartFrameRelativePath}`);
        } else {
          const rowPayload = item.row_creative_payload ? JSON.parse(item.row_creative_payload) : {};

          console.log(`[Pillar Glabs] Submitting T2I recovery task for clip ${c}...`);
          const { generateImage } = await import('./webhook-client.js');
          ...
          const runT2I = async (refs) => {
            const { buildOpcStartFrameRequest } = await import('./opc-start-frame-request.js');
            const { recordStartFrameRequestAudit } = await import('./opc-start-frame-audit.js');
            const built = await buildOpcStartFrameRequest({ campaign, item, clipIndex: c, prompt: t2iPromptText, origin: 'production_recovery', contextReferences: isCartoon ? (refs || []) : [] });
            const t2iResult = await generateImage(built.providerRequest);
            ...
            const t2iTaskId = t2iResult.task_id;
            await recordStartFrameRequestAudit(built.audit, t2iTaskId);
          ...
          if (isCartoon) {
            ...
            try {
              visualIdentitySnapshot = campaign.visual_overrides_json ? JSON.parse(campaign.visual_overrides_json) : null;
            } catch (_) {}
          ...
          try {
            ...
            const isProductCampaign = Boolean(campaign.target_product_id || campaign.product_id);
            if (!isProductCampaign && !isCartoon && resolvedRefs.allReferences.length > 0) {
          ...
          effectiveStartFrameRelativePath = relativeStartFramePath;
          effectiveStartFrameLocalPath = startFrameLocalPath;

          console.log(`[OPC Start Frame] Saved locally at ${relativeStartFramePath}`);
        }

        updatePillarCampaignItem(item.id, { t2i_start_frame_path: effectiveStartFrameRelativePath });

        const startFrameBase64 = fileToBase64(effectiveStartFrameRelativePath);
        const i2vPromptObj = (parsed.i2v_prompts || []).find(p => Number(p.clip) === c);
        ...
        videoResult = await generateVideo({
          ...
          mode: 'start_image',
          reference_images: [startFrameBase64],
          video_length: Number(campaign.clip_duration) || 8,
          duration: Number(campaign.clip_duration) || 8,
          webhookOverride: brandProfile
        });
      } else {
        const t2vPromptObj = (parsed.t2v_prompts || []).find(p => Number(p.clip) === c);
...
export async function processPillarFfmpeg(payload, job) {
  const { getNextPendingPillarFfmpegItem, updatePillarCampaignItem, getPillarCampaign, getSetting, setSetting, getDb } = await import('./db.js');
  const { processTtsStudioMuxing } = await import('./video-studio-processor.js');
  const { google } = await import('googleapis');
  const { getAuthorizedClient } = await import('./google-auth.js');
  const path = await import('path');
  ...
  if (campaign.enable_ffmpeg === 0) {
    if (campaign.enable_tts === 0 && campaign.enable_social_post === 0) {
      ...
      try {
        ...
        const { uploadVideoToFolder, getOrCreateCampaignFolder, getOrCreateFolderInFolder } = await import('./drive-uploader.js');
  ...
  // Preflight validation: Pastikan visual_clip_paths tersedia dan valid sebelum memulai proses FFmpeg
  let parsedClips = [];
  try {
    parsedClips = typeof item.visual_clip_paths === 'string' ? JSON.parse(item.visual_clip_paths) : (item.visual_clip_paths || []);
  } catch (_) {}

  if (!Array.isArray(parsedClips) || parsedClips.filter(Boolean).length === 0) {
    console.warn(`⚠️ [Pillar FFmpeg] Item #${item.id} visual_clip_paths belum tersedia atau kosong. Memulihkan visual_status.`);
    updatePillarCampaignItem(item.id, {
      visual_status: item.visual_tasks_json ? 'processing' : 'pending',
      ffmpeg_status: 'pending'
    });
    return { skip: true, reason: `Item #${item.id} visual_clip_paths belum siap untuk FFmpeg muxing.` };
  }
...
export async function processPillarSocialPoster(payload, job) {
  const { getNextPendingPillarSocialPostItem, updatePillarCampaignItem, getPillarCampaign } = await import('./db.js');
  const { google } = await import('googleapis');
  const { getAuthorizedClient } = await import('./google-auth.js');
  const path = await import('path');
...
export async function processInstantRowProductSourcing(payload, job) {
  const { updateInstantCampaignItem, getDb } = await import('./db.js');
  const { downloadECommerceImage } = await import('./url-scraper.js');
  const path = await import('path');
  const fs = await import('fs');
  ...
  if (itemId) {
    ...
    const { getNextPendingInstantSourcingItem } = await import('./db.js');
...
export async function processInstantGenerator(payload, job) {
  const { parseGeminiJSON } = await import('./json-parser.js');
  const { getInstantCampaign, updateInstantCampaignItem, getDb } = await import('./db.js');
  const { executeWithKeyPool } = await import('./gemini.js');
  const { GoogleGenerativeAI } = await import('@google/generative-ai');
  const path = await import('path');
  const fs = await import('fs');
  ...
  if (itemId) {
    ...
    const { getNextPendingInstantGeneratorItem } = await import('./db.js');
...
========================================================================\n`;
    ...
    const { getPackagingInstruction } = await import('./prompts.js');
...

{
  ...
  "distribution_assets": {
    ...
    const rawResponse = await executeWithKeyPool(1, async (apiKey) => {
      ...
      const resilientModel = makeModelResilient(model, apiKey, models.PRIMARY, true);
      const result = await resilientModel.generateContent(geminiPayload);
...
export async function processInstantTts(payload, job) {
  const { getNextPendingInstantTtsItem, updateInstantCampaignItem, getInstantCampaign, createTtsBatch, createTtsClip, updateTtsClip } = await import('./db.js');
  const { generateMinimaxVO } = await import('./minimax-tts.js');
  const { generateGeminiExact8sAudio } = await import('./gemini-8s-tts.js');
  const path = await import('path');
  const fs = await import('fs');
...
export async function processInstantGlabs(payload, job) {
  const { getNextPendingInstantGlabsItem, updateInstantCampaignItem, getInstantCampaign, getSetting, getDb, updateGlabsTaskStatus, insertGlabsTask } = await import('./db.js');
  const { generateVideo, getTaskStatus, getFileUrl } = await import('./webhook-client.js');
  const path = await import('path');
  const fs = await import('fs');
  ...
  try {
    for (let c = 1; c <= targetClips; c++) {
      ...
      if (isHybridLockClip) {
        ...
        // Layer 2: Cek disk pattern scanner
        if (!startFrameExists) {
          const diskMatch = findExistingStartFrameFile({ itemId: item.id, clipIndex: c, prefix: 'ifc_start_frame' });
          if (diskMatch) {
            effectiveStartFrameRelativePath = diskMatch.webRelativePath;
            effectiveStartFrameLocalPath = diskMatch.absolutePath;
            startFrameExists = true;
        ...
        if (startFrameExists) {
          console.log(`[IFC Glabs] Reusing existing start frame for clip ${c}: ${effectiveStartFrameRelativePath}`);
        } else {
          productBase64 = await resolveProductBase64(campaign, productData, rowPayload);
...
export async function processInstantFfmpeg(payload, job) {
  const { getNextPendingInstantFfmpegItem, updateInstantCampaignItem, getInstantCampaign, getSetting, getDb } = await import('./db.js');
  const { processTtsStudioMuxing } = await import('./video-studio-processor.js');
  const path = await import('path');
  const fs = await import('fs');
...
export async function processInstantSocialPoster(payload, job) {
  const { getNextPendingInstantSocialPostItem, updateInstantCampaignItem, getInstantCampaign, getSetting } = await import('./db.js');
  const { google } = await import('googleapis');
  const { getAuthorizedClient } = await import('./google-auth.js');
  const path = await import('path');
...
export async function processProductScraper(payload, job) {
  ...
  const { scrapeUrl, downloadECommerceImage } = await import('./url-scraper.js');
  const { createProductExtraction, updateProductExtraction, getDb } = await import('./db.js');
  const { buildProductAgentPrompt, buildBatchProductExtractionPrompt } = await import('./prompts.js');
  const { executeWithKeyPool, generateWithImage } = await import('./gemini.js');
  const { GoogleGenerativeAI } = await import('@google/generative-ai');
  const { parseGeminiJSON } = await import('./json-parser.js');
  const { v4: uuidv4 } = await import('uuid');
  const path = await import('path');
  ...
  if (rawProductsArray.length > 0) {
    ...
    const extractedText = await executeWithKeyPool(1, async (apiKey) => {
      ...
      const resilientModel = makeModelResilient(model, apiKey, models.PRIMARY, true);
      const result = await resilientModel.generateContent(extractionPrompt);
      return result.response.text();
    });
  ...
  for (const productData of extractedList) {
    ...
    if (needsClean) {
      if (rawPhotoPath) {
        ...
        try {
          const { createCleanProductShot } = await import('./bg-remover.js');
    ...
    if (needsStudio) {
      if (cleanPhotoPath && productData.t2i_prompt) {
        try {
          const { generateImage: generateGlabsImage, getTaskStatus: getGlabsTaskStatus, getFileUrl: getGlabsFileUrl } = await import('./webhook-client.js');
...
const DECONSTRUCT_DELAY_MS = 20000;
...
export async function processDeconstruct(payload, job) {
  ...
  const { downloadFromUrl, cleanupTempFile } = await import('./video-downloader.js');
  const { uploadVideoToGemini, generateWithVideo, executeWithKeyPool } = await import('./gemini.js');
  const { buildDeconstructPhase1Prompt } = await import('./prompts.js');
  const { parseGeminiJSON } = await import('./json-parser.js');
  const pathMod = await import('path');

  let assetId = payload?.item_id;
  let asset = null;
  ...
  if (assetId) {
    ...
    asset = await getDeconstructAssetById(assetId, tenantId);
    if (asset) {
    ...
    asset = await getNextPendingDeconstructAsset(batchId, tenantId);
  }

  if (!asset) {
    return { skip: true, reason: 'No pending deconstruct asset found' };
  }
  ...
  let totalAssets = 1;
  try {
    const allBatchAssets = await db.prepare(
      "SELECT id FROM re_deconstructed_assets WHERE batch_id = ? ORDER BY created_at ASC, id ASC"
    ).all(batchId);
    totalAssets = allBatchAssets.length || 1;
    const idx = allBatchAssets.findIndex(a => a.id === asset.id);
    if (idx !== -1) {
      assetIndex = idx + 1;
    }
  } catch (err) {
    console.error('[DECONSTRUCT] Error calculating asset batch index:', err.message);
  }
  const label = `[Video ${assetIndex}/${totalAssets}]`;

  let tempFilePath = null;

  try {
    // Step 1: Download video
    console.log(`[DECONSTRUCT] ${label} Downloading video...`);
    await updateDeconstructAsset(asset.id, { status: 'downloading' });
    ...
    // Step 2 & 3: Upload and analysis using keypool
    const responseText = await executeWithKeyPool(1, async (apiKey) => {
      console.log(`[DECONSTRUCT] ${label} Uploading video to Gemini File API...`);
      ...
      const uploadResult = await uploadVideoToGemini(tempFilePath, mimeType, apiKey);
      console.log(`[DECONSTRUCT] ${label} Upload success to Gemini.`);

      await updateDeconstructAsset(asset.id, {
        status: 'analyzing',
        gemini_file_uri: uploadResult.fileUri,
      });

      console.log(`[DECONSTRUCT] ${label} Running Gemini multimodal deconstruction...`);
      let availableNiches = ['Baking', 'Cooking', 'Health', 'Skincare', 'Home Care', 'Fashion', 'Tech & Gadgets'];
      try {
        const nichesRes = await dbAll('SELECT name FROM re_niches ORDER BY name ASC');
        if (nichesRes && nichesRes.length > 0) {
          availableNiches = nichesRes.map(n => n.name);
        }
      } catch (err) {
        console.warn('[DECONSTRUCT] Failed to load niches from DB, using fallback list:', err.message);
      }

      const targetRecommendationCount = Number(asset.target_recommendation_count || 3);
      const promptText = buildDeconstructPhase1Prompt(asset.original_caption, targetRecommendationCount, availableNiches);
      return await generateWithVideo(uploadResult.fileUri, mimeType, promptText, apiKey);
    });

    const parsed = parseGeminiJSON(responseText);

    if (!parsed || !parsed.storyboard) {
      throw new Error('Gemini failed to return valid structured JSON for storyboard.');
    }

    // Step 4: Save results
    const detectedNiche = parsed.niche || asset.niche || 'General';
    ...
    await updateDeconstructAsset(asset.id, {
      ...
      niche: (asset.niche && asset.niche !== 'Auto-Detect') ? asset.niche : detectedNiche,
      tags: tagsString,
    ...
    const progress = await updateDeconstructBatchProgress(asset.batch_id);
    console.log(`[DECONSTRUCT] ${label} Asset deconstructed successfully. Batch progress: ${progress.processed}/${progress.total}`);

    // Step 6: Cleanup temp file
    cleanupTempFile(tempFilePath);

    // Step 7: Mandatory delay (anti-block)
    console.log(`[DECONSTRUCT] ${label} Mandatory ${DECONSTRUCT_DELAY_MS / 1000}s delay before next video (anti-block safety)...`);
    await new Promise(resolve => setTimeout(resolve, DECONSTRUCT_DELAY_MS));

    // Step 8: If there are more pending assets in this batch, trigger scheduler tick immediately
    const nextAsset = await getNextPendingDeconstructAsset(batchId, tenantId);
    if (nextAsset) {
      console.log(`[DECONSTRUCT] ${label} More assets pending. Triggering next asset immediately...`);
      setTimeout(async () => {
        try {
          const { tickCampaignScheduler } = await import('./campaign-scheduler.js');
    ...
    console.error(`[DECONSTRUCT] ${label} Failed:`, error.message);
    await updateDeconstructAsset(asset.id, {
      status: 'failed',
      error_message: error.message,
    });
    await updateDeconstructBatchProgress(asset.batch_id);
    cleanupTempFile(tempFilePath);
    ...
    const nextAsset = await getNextPendingDeconstructAsset(batchId, tenantId);
    if (nextAsset) {
      console.log(`[DECONSTRUCT] ${label} Failed asset, but more assets pending. Triggering next asset immediately...`);
      setTimeout(async () => {
        try {
          const { tickCampaignScheduler } = await import('./campaign-scheduler.js');
          await tickCampaignScheduler();
...
${promptsSchema}
    ...
    const rawResponse = await executeWithKeyPool(1, async (apiKey) => {
      ...
      const resilientModel = makeModelResilient(model, apiKey, models.PRIMARY, true);
      const res = await resilientModel.generateContent(systemPrompt);
      return res.response.text();
    });
...
export async function processRecipeExporter(payload, job) {
  const { getDb, getRecipeCampaignById, updateRecipeCampaign, updateRecipeItem } = await import('./db.js');
  const { createRecipeGrid } = await import('./recipe-grid-helper.js');
