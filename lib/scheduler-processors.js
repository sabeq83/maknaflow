...
export async function processDeconstruct(payload, job) {
  ...
  const { batch_id: batchId, tenant_id: tenantId = 'default_tenant' } = payload || {};
  if (!batchId) {
    return { skip: true, reason: 'Payload is missing batch_id' };
  }

  const asset = await getNextPendingDeconstructAsset(batchId, tenantId);
  if (!asset) {
    return { skip: true, reason: `No pending deconstruct assets for batch ${batchId}` };
  }
  ...
  try {
    ...
    // Step 8: If there are more pending assets in this batch, enqueue another job
    const nextAsset = await getNextPendingDeconstructAsset(batchId, tenantId);
    if (nextAsset) {
      console.log(`[DECONSTRUCT] More assets pending. Enqueuing next job for batch ${batchId}...`);
      await createJob('re_deconstruct', { batch_id: batchId, tenant_id: tenantId });
    }
