    ...
    if (shouldRunT2i) {
      ...
      try {
        ...
        const submitStartFrame = async (clipIndex, prompt) => {
          const built = await buildOpcStartFrameRequest({ campaign: tempCampaign, item: { ...item, new_video_plan_json: JSON.stringify(newVideoPlan) }, clipIndex, prompt, origin: 'phase_1_initial' });
