    ...
    for (let c = 1; c <= targetClips; c++) {
      ...
      if (isHybridLockClip) {
        ...
        if (Array.isArray(t2iImages) && t2iImages[c - 1]) {
          ...
          const localCheck = path.isAbsolute(registered) ? registered : path.join(process.cwd(), 'public', registered.replace(/^\//, ''));
          if (fs.existsSync(localCheck)) {
            effectiveStartFrameRelativePath = registered.startsWith('/') ? registered : `/${registered}`;
            effectiveStartFrameLocalPath = localCheck;
            startFrameExists = true;
          }
        }

        // 2. Cek pillar_campaign_item_assets
        if (!startFrameExists) {
          try {
            ...
            if (assetRow.rows.length > 0 && assetRow.rows[0].local_path) {
              ...
              const localCheck = path.isAbsolute(regAsset) ? regAsset : path.join(process.cwd(), 'public', regAsset.replace(/^\//, ''));
              if (fs.existsSync(localCheck)) {
        ...
        // 3. Cek legacy pattern
        if (!startFrameExists) {
          const defaultFilename = `opc_start_frame_${item.id}_clip_${c}.png`;
          const defaultLocalPath = path.join(process.cwd(), 'public', 'uploads', 'start_frames', defaultFilename);
          if (fs.existsSync(defaultLocalPath)) {
            effectiveStartFrameRelativePath = `/uploads/start_frames/${defaultFilename}`;
            effectiveStartFrameLocalPath = defaultLocalPath;
            startFrameExists = true;
          }
        }
