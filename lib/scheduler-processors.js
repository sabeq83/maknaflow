        ...
        for (let poll = 0; poll < 40; poll++) {
          ...
          if (statusRes?.status === 'completed') {
            ...
            if (imgFile) {
              ...
              await db.prepare('UPDATE strategic_campaign_scenes SET start_frame_path = ? WHERE id = ?').run(relPath, sc.id);
...
export async function processStrategicTts(payload, job) {
  ...
  try {
    ...
    for (const scene of scenes) {
      ...
      await db.prepare('UPDATE strategic_campaign_scenes SET voice_over_audio_path = ? WHERE id = ?').run(localRelPath, scene.id);
    }
  ...
  if (item.visual_status !== 'processing') {
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      if (res && res.task_id) {
        ...
        await db.prepare('UPDATE strategic_campaign_scenes SET task_id = ? WHERE id = ?').run(res.task_id, sc.id);
      } else {
      ...
      if (taskStatus === 'completed') {
        ...
        if (videoFile) {
          ...
          if (res.ok) {
            ...
            await db.prepare('UPDATE strategic_campaign_scenes SET local_clip_path = ? WHERE id = ?').run(localRelPath, scenes[i].id);
          } else {
            allCompleted = false;
          }
        } else {
