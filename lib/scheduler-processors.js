    ...
    if (shouldRunT2i) {
      ...
      try {
        ...
        const downloadAndSaveStartFrame = async (clipIndex, taskId, files) => {
          ...
          try {
            ...
            if (imgResponse.ok) {
              ...
              fs.writeFileSync(startFrameLocalPath, imgBuffer);
              t2iImagePaths[clipIndex - 1] = `/uploads/start_frames/${startFrameFilename}`;
              console.log(`[OPC Analyzer T2I] Saved image for clip ${clipIndex}: ${t2iImagePaths[clipIndex - 1]}`);
        ...
        const executeSingleClipIsolated = async (clipIndex, promptText) => {
          try {
            console.log(`[OPC Analyzer T2I] [Isolated] Submitting T2I task for clip ${clipIndex}...`);
