...
export async function processDeconstruct(payload, job) {
  ...
  try {
    ...
    console.log(`[DECONSTRUCT] Downloading video for asset ${asset.id}: ${asset.source_url} (tenant: ${tenantId})`);
    ...
    const responseText = await executeWithKeyPool(1, async (apiKey) => {
      ...
      console.log(`[DECONSTRUCT] Upload success to Gemini: ${uploadResult.fileUri}`);
      ...
      console.log(`[DECONSTRUCT] Running Gemini multimodal deconstruction for asset ${asset.id}`);
    ...
    if (nextAsset) {
      console.log(`[DECONSTRUCT] More assets pending. Triggering next tick immediately...`);
      setTimeout(async () => {
        try {
    ...
    const nextAsset = await getNextPendingDeconstructAsset(batchId, tenantId);
    if (nextAsset) {
      console.log(`[DECONSTRUCT] Failed asset, but more assets pending. Triggering next tick immediately...`);
      setTimeout(async () => {
        try {
          const { tickCampaignScheduler } = await import('./campaign-scheduler.js');
          await tickCampaignScheduler();
