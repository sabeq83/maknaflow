...
export async function dispatchValidatedResearchToOperator() {
  ...
  const runs = await pgQuery(`
    SELECT DISTINCT ON (r.id)
      r.*,
      rev.id as research_revision_id,
      rev.payload_sha256 as research_snapshot_sha256,
      rev.payload_json as research_brief,
      s.operator_request_json
    FROM agent_automation_runs r
    JOIN agent_research_revisions rev ON rev.run_id = r.id
    JOIN content_automation_schedules s ON s.id = r.schedule_id
    WHERE r.tenant_id = $1 AND r.status = 'research_ready'
    ORDER BY r.id, rev.revision DESC
  `, [tenantId]);
  ...
  for (const run of runs.rows) {
    try {
      ...
      const operatorRequest = {
        ...parsedRequest,
        research_revision_id: run.research_revision_id,
        research_snapshot_sha256: run.research_snapshot_sha256,
...
export function startAgentAutomationWorker() {
  ...
  if (process.env.DISABLE_WORKER_AUTOTICK !== 'true') {
...
export function stopAgentAutomationWorker() {
  if (state.interval) {
