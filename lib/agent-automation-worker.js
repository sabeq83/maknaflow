...
async function transitionTowardProduction(run, operatorStatus) {
  ...
  if (run.status === 'generating' && operatorStatus === 'awaiting_approval') {
    await transitionAgentRun(run.id, 'generating', 'awaiting_creative_approval');
    return;
  }
...
export async function reconcileAgentAutomationRuns() {
  ...
  for (const run of rows) {
    ...
    if (policy.mode === 'draft_only') {
      await pgQuery("UPDATE content_automation_runs SET status='completed',completed_at=CURRENT_TIMESTAMP WHERE id=$1 AND tenant_id=$2", [run.run_id, tenantId]);
