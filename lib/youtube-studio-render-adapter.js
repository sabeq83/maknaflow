...
async function processRender({ pkgId, assets, fileName }) {
  ...
  const packageResult = await pgQuery(`
    SELECT p.context_snapshot_json, s.script_json, e.narration_profile_key
    FROM youtube_production_packages p
    JOIN youtube_episodes e ON p.episode_id = e.id
    LEFT JOIN youtube_episode_scripts s ON p.approved_script_id = s.id
    WHERE p.id = $1
  `, [pkgId]);
  const packageContext = packageResult.rows[0] || {};
  const nativeSceneAudio = packageContext.context_snapshot_json?.audio_production_snapshot?.audio_production_mode === 'native_scene_audio';
  const approvedScriptScenes = packageContext.script_json?.scenes || [];
  ...
  for (const sceneIdx of sceneIndices) {
    ...
    if (nativeSceneAudio) {
      const invalidNativeAsset = visAssets.find(asset =>
        asset.asset_type === 'generated_visual' &&
        asset.output_asset_json?.video_path &&
        asset.output_asset_json?.embedded_audio_validated !== true
      );
      if (invalidNativeAsset) {
        throw new Error(`Native audio was not validated for visual asset ${invalidNativeAsset.id}`);
      }
    }
    ...
    const scriptScene = approvedScriptScenes.find(item => Number(item.scene_index) === Number(sceneIdx));
    const scriptText = (scriptScene?.audio_blocks || [])
      .filter(block => ['narration', 'dialogue', 'whisper'].includes(block.type))
      .map(block => block.subtitle_cue || block.text)
      .filter(Boolean)
      .join(' ');
    ...
    scenes.push({
      ...
      duration: duration,
      voiceover: voAssets.map(a => a.prompt_snapshot || '').join(' ') || scriptText
    });
  }

  // Fetch episode narration profile key
  const narrationProfileKey = packageContext.narration_profile_key || 'general_id';
