...
export async function processNextMultiplierTask() {
  const tasks = await getPendingMultiplierTasks(4); // Process up to 4 tasks in parallel
...
async function handleProductResolution(task) {
  ...
  if (productSnapshot && productSnapshot.product_name && productSnapshot.product_description) {
...
async function handleT2iGeneration(task) {
  updateMultiplierTask(task.id, { status: 'generating_t2i' });
  const bridging = JSON.parse(task.bridging_config_json || '{}');
  ...
  logToMultiplier(`[Multiplier Worker] Generating start frame image for hybrid lock clip of task ${task.id}`);
  const promptObj = prompts[bridgeAtClip - 1];
  ...
  if (!t2iResult?.task_id) {
    throw new Error(`T2I image generation failed to submit for task ${task.id}`);
  }
  ...
  // Simpan URL start frame gambar hasil render dan ubah status ke waiting_approval
  bridging.generatedStartFrameUrl = t2iImageUrl;
  
  // Download secara lokal untuk display preview di UI
  try {
    const tempDir = path.join(process.cwd(), 'public', 'temp');
    if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });
    const localFilename = `generated_start_frame_${task.id}_${Date.now()}.png`;
    const localPath = path.join(tempDir, localFilename);
    const { downloadECommerceImage } = await import('./url-scraper');
    await downloadECommerceImage(t2iImageUrl, localPath);
    bridging.productRefImagePath = `/temp/${localFilename}`;
    bridging.productFilenameDeclare = localFilename;
  } catch (err) {
    console.error('[Multiplier T2I Local Save Warning]:', err.message);
  }
