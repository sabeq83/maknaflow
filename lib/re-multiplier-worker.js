...
import { buildMultiplierPhase2Prompt } from './prompts.js';
import { parseGeminiJSON } from './json-parser.js';
import { generateMinimaxVO } from './minimax-tts.js';
import { generateVideo, generateImage, getTaskStatus, getFileUrl } from './webhook-client.js';
...
export async function processNextMultiplierTask() {
  const tasks = await getPendingMultiplierTasks(8);
  if (!tasks || tasks.length === 0) return;

  logToMultiplier(`[Multiplier Worker] Found ${tasks.length} pending tasks to process in parallel.`);
  ...
  await Promise.all(tasks.map(async (task) => {
    ...
    await tenantContext.run(tenantId, async () => {
      logToMultiplier(`[Multiplier Worker] Starting concurrent processing for task ${task.id} (Status: ${task.status})`);
      ...
      try {
        ...
        while (maxIterations > 0) {
          logToMultiplier(`[Multiplier Worker] [Chain Loop] Task ${activeTask.id} running stage: ${currentStatus}`);

          if (currentStatus === 'pending_resolution' || currentStatus === 'resolving_product') {
            await handleProductResolution(activeTask);
          } else if (currentStatus === 'remaking') {
            await handleStoryboardRemake(activeTask);
          } else if (currentStatus === 'generating_t2i') {
            await handleT2iGeneration(activeTask);
          } else if (currentStatus === 'generating_audio') {
            await handleAudioGeneration(activeTask);
          } else if (currentStatus === 'generating_visuals') {
            await handleVisualGeneration(activeTask);
          } else if (currentStatus === 'ffmpeg_muxing') {
            await handleFFmpegMuxing(activeTask);
          } else {
            break; // Terminal state (completed, failed, waiting_approval)
          }

          // Fetch updated status to check if we can transition immediately
          const nextTask = await db.prepare('SELECT * FROM re_multiplier_tasks WHERE id = ?').get(activeTask.id);
          if (!nextTask || nextTask.status === currentStatus) {
            break;
          }

          // Yield if waiting for G-Labs async rendering tasks
          if (nextTask.status === 'generating_visuals' && nextTask.glabs_task_ids) {
            break;
          }
...
async function handleProductResolution(task) {
  ...
  const isPureStorytelling = task.campaign_type === 'pure_storytelling' ||
                             bridging.campaign_type === 'pure_storytelling' ||
                             bridging.is_bridging_active === false ||
                             (!task.target_product_id && !task.target_product_url && !bridging.targetProductId && !bridging.productUrl && !bridging.manualProductName);

  if (isPureStorytelling) {
    logToMultiplier(`[Multiplier Worker] Task ${task.id} running in Pure Storytelling / Non-Product mode`);
    
    // Load niche KB snippet if specified
    if (bridging.nicheKbName && !bridging.nicheKbContent) {
      try {
        const kbPath = path.join(process.cwd(), 'kb', bridging.nicheKbName);
        if (fs.existsSync(kbPath)) {
          bridging.nicheKbContent = fs.readFileSync(kbPath, 'utf8').slice(0, 1500);
        }
      } catch (_) {}
    }
    ...
    productDNA = {
      product_name: '',
      product_description: bridging.angleTitle || bridging.editorialTopic || task.editorial_topic || '',
      unique_selling_point: bridging.editorialOutro || task.editorial_outro || '',
      is_pure_storytelling: true
    };
    ...
    // Fetch live product from product_extractions to check for fresh truth/geometric data
    let liveProduct = null;
    try {
      const prodId = productSnapshot.product_id || resolvedProductId;
      if (prodId) {
        liveProduct = await db.prepare('SELECT * FROM product_extractions WHERE id = ?').get(prodId);
      }
    } catch (err) {
      console.warn(`[Multiplier Worker] Failed to fetch live product info for fallback:`, err.message);
    }
    ...
    productDNA = {
      ...
      product_truth: productSnapshot.product_truth || (liveProduct ? (liveProduct.product_truth || liveProduct.clean_photo_t2i_prompt) : '') || productSnapshot.clean_photo_t2i_prompt || '',
      geometric_truth: productSnapshot.geometric_truth || (liveProduct ? liveProduct.geometric_truth : '') || '',
      clean_photo_t2i_prompt: productSnapshot.clean_photo_t2i_prompt || (liveProduct ? liveProduct.clean_photo_t2i_prompt : '') || '',
      t2i_prompt: productSnapshot.t2i_prompt || (liveProduct ? liveProduct.t2i_prompt : '') || ''
    };
    resolvedProductId = productSnapshot.product_id || resolvedProductId;

    // Fallback: Resolve reference image path if missing in bridging config
    if (!bridging.product_ref_image_path && !bridging.productRefImagePath) {
      const liveImg = liveProduct ? (liveProduct.clean_photo_url || liveProduct.photo_url || liveProduct.cleaned_photo_url) : null;
      const snapshotImg = productSnapshot.product_image_url || productSnapshot.photo_url;
      const resolvedImgPath = liveImg || snapshotImg || null;
      if (resolvedImgPath) {
        bridging.productRefImagePath = resolvedImgPath;
      }
    }
    ...
    // Fallback: Resolve reference image path if missing in bridging config
    if (!bridging.product_ref_image_path && !bridging.productRefImagePath) {
      const resolvedImgPath = product.clean_photo_url || product.photo_url || product.cleaned_photo_url || null;
      if (resolvedImgPath) {
        bridging.productRefImagePath = resolvedImgPath;
      }
    }
...
async function handleStoryboardRemake(task) {
  ...
  // Fallback to live product extractions if truth or geometric_truth is empty in productDNA
  if (productDNA.product_id || task.target_product_id || bridging.targetProductId) {
    const prodId = productDNA.product_id || task.target_product_id || bridging.targetProductId;
    if (!productDNA.product_truth || !productDNA.geometric_truth) {
      try {
        const liveProduct = await db.prepare('SELECT * FROM product_extractions WHERE id = ?').get(prodId);
        if (liveProduct) {
          productDNA.product_truth = productDNA.product_truth || liveProduct.product_truth || liveProduct.clean_photo_t2i_prompt || '';
          productDNA.geometric_truth = productDNA.geometric_truth || liveProduct.geometric_truth || '';
          productDNA.clean_photo_t2i_prompt = productDNA.clean_photo_t2i_prompt || liveProduct.clean_photo_t2i_prompt || '';
          productDNA.t2i_prompt = productDNA.t2i_prompt || liveProduct.t2i_prompt || '';
        }
      } catch (err) {
        console.warn(`[Multiplier Worker] Storyboard Remake live product fallback failed:`, err.message);
      }
    }
  }
...
async function handleT2iGeneration(task) {
  ...
  let imagePath = bridging.product_ref_image_path || bridging.productRefImagePath;

  // Fallback: Fetch live product image if imagePath is empty in bridging config
  if (!imagePath && task.target_product_id) {
    try {
      const db = getDb();
      const product = await db.prepare('SELECT * FROM product_extractions WHERE id = ?').get(task.target_product_id);
      if (product) {
        imagePath = product.clean_photo_url || product.photo_url || product.cleaned_photo_url || null;
      }
    } catch (err) {
      console.warn(`[Multiplier Worker] T2I Generation live product image fallback failed:`, err.message);
    }
  }
...
async function handleAudioGeneration(task) {
  ...
  const provider = audioConfig.voiceProvider || audioConfig.voice_provider || 'minimax';
  const persona = audioConfig.voicePersona || audioConfig.voice_persona || 'Indonesian_SweetGirl';
  ...
  await createTtsBatch({
    ...
    config_speed: audioConfig.voiceSpeed || audioConfig.voice_speed || 1.0,
    config_volume: audioConfig.voiceVolume || audioConfig.voice_volume || 1.0,
    tts_model_quality: audioConfig.ttsModelQuality || audioConfig.tts_model_quality || 'speech-2.8-turbo'
  });
  ...
  for (let c = 0; c < storyboard.length; c++) {
    ...
    if (provider === 'minimax') {
      await generateMinimaxVO(text, persona, localClipPath, {
        speed: audioConfig.voiceSpeed || audioConfig.voice_speed || 1.0,
        volume: audioConfig.voiceVolume || audioConfig.voice_volume || 1.0,
        model: audioConfig.ttsModelQuality || audioConfig.tts_model_quality || 'speech-2.8-turbo',
        language: audioConfig.targetLanguage || audioConfig.target_language || 'id-ID'
      });
    } else {
...
async function handleVisualGeneration(task) {
  ...
  // Determine if we need to submit or re-verify G-Labs tasks
  let needsSubmission = !task.glabs_task_ids;
  if (task.glabs_task_ids) {
    try {
      const ids = JSON.parse(task.glabs_task_ids);
      const failedCount = await db.prepare("SELECT count(*) as count FROM glabs_tasks WHERE task_id IN (" + ids.map(() => '?').join(',') + ") AND status = 'failed'").get(...ids);
  ...
  // Case A: Submit visual tasks
  if (needsSubmission) {
    logToMultiplier(`[Multiplier Worker] Submitting/re-verifying video tasks to G-Labs for task ${task.id}`);
    
    const existingTaskIds = [];
    try {
      if (task.glabs_task_ids) {
        existingTaskIds.push(...JSON.parse(task.glabs_task_ids));
      }
    } catch (_) {}
    ...
    if (task.brand_profile_id) {
      ...
      brandProfile = await getBrandProfile(task.brand_profile_id).catch(() => null);
    }
    ...
    for (let c = 1; c <= prompts.length; c++) {
      ...
      const expectedPrompt = isHybridLockClip ? (promptObj.i2v_prompt || '') : (promptObj.t2v_prompt || promptObj.prompt || '');
            ...
            try {
              ...
              if (taskStatus === 'completed') {
                ...
                if (videoFile) {
                  ...
                  await updateGlabsTaskStatus(existingTaskId, 'completed', completedUrl);
                  existingTask = await db.prepare('SELECT * FROM glabs_tasks WHERE task_id = ?').get(existingTaskId);
                }
              } else if (taskStatus === 'failed') {
                await updateGlabsTaskStatus(existingTaskId, 'failed');
                existingTask = await db.prepare('SELECT * FROM glabs_tasks WHERE task_id = ?').get(existingTaskId);
              }
            } catch (err) {
              console.warn(`[Multiplier Worker] Failed to poll G-Labs status:`, err.message);
            }
          }

          if (existingTask.status === 'completed' && existingTask.video_url && existingTask.prompt === expectedPrompt) {
            logToMultiplier(`[Multiplier Worker] Reusing completed task ${existingTaskId} for clip ${c}`);
            submittedTaskIds.push(existingTaskId);
      ...
      if (isHybridLockClip) {
        ...
        logToMultiplier(`[Multiplier Worker] Submitting T2V video task for clip ${c}: ${expectedPrompt}`);
        const videoResult = await generateVideo({
          prompt: expectedPrompt,
          model: videoModel,
          aspect_ratio,
      ...
      await insertGlabsTask(finalVideoTaskId, task.id, c, expectedPrompt);
    }
  ...
  // Case B: Poll G-Labs task IDs (all submitted and not failed)
  else {
...
async function handleFFmpegMuxing(task) {
  ...
  await processTtsStudioMuxing({
    ...
    syncOption: audioConfig.ffmpegSyncOption || audioConfig.ffmpeg_sync_option || 'smart_sync',
    bgmPath: null,
    bgmVolume: 0,
    sfxVolume: audioConfig.ffmpegSfxVolume !== undefined ? audioConfig.ffmpegSfxVolume : (audioConfig.ffmpeg_sfx_volume !== undefined ? audioConfig.ffmpeg_sfx_volume : 0.00),
    videoScale: audioConfig.ffmpegVideoScale !== undefined ? audioConfig.ffmpegVideoScale : (audioConfig.ffmpeg_video_scale !== undefined ? audioConfig.ffmpeg_video_scale : 1.1),
  ...
  // Trigger ContentFlow database sync immediately upon completion
  try {
    const { scanAndSyncExistingCampaigns } = await import('./contentflow-ingest');
    await scanAndSyncExistingCampaigns(task.batch_id);
    logToMultiplier(`[Multiplier Worker] Task ${task.id} successfully synced to ContentFlow database.`);
  } catch (syncErr) {
    console.error(`[Multiplier Worker] ContentFlow sync failed for task ${task.id}:`, syncErr.message);
  }
