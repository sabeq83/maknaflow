...
export function buildExcelBuffer(result) {
  ...
  // Sheet 4: Captions
  const universalCaption = result.caption || result.universal_caption || (typeof result.social_media_package === 'object' ? result.social_media_package?.caption : '') || result.tiktok_caption || result.ig_caption || '';
  const captions = [
    { 'Platform': 'Universal Caption', 'Caption': universalCaption },
  ];
...
export function buildMarkdownContent(result, batchId) {
  ...
  const universalCaption = result.caption || result.universal_caption || (typeof result.social_media_package === 'object' ? result.social_media_package?.caption : '') || result.tiktok_caption || result.ig_caption || '';
  if (universalCaption) {
    lines.push('### 📲 Universal Social Media Package & Caption');
    lines.push(universalCaption);
    lines.push('');
  }
...
export function buildRECaptionsRow(result, batchId) {
  ...
  const universalCaption = parsed.caption || parsed.universal_caption || (typeof parsed.social_media_package === 'object' ? parsed.social_media_package?.caption : '') || parsed.tiktok_caption || parsed.ig_caption || '';
  return [
    result.id || '',
    batchId,
    universalCaption,
...
export function buildBatchMarkdownContent(campaign, items) {
  ...
  if (campaign.visual_overrides_json) {
    try {
      ...
      if (vso.identity_ref) {
        lines.push(`  - **Visual Lineage:** id: ${vso.identity_ref.id}, version: ${vso.identity_ref.version}, source: ${vso.identity_ref.source}`);
      }
...
export function buildPillarReviewMarkdown(campaign, items, metadata = {}) {
  const lines = [
    `# Review Storyboard — ${campaign.campaign_name}`,
    '',
    `- **Campaign:** \`${campaign.id}\``,
    `- **Review Revision:** \`${metadata.revision || '-'}\``,
    `- **Review SHA-256:** \`${metadata.sha256 || 'calculated-after-render'}\``,
    `- **Brand Account:** ${campaign.account_name || '-'}`,
    `- **Target Demographic:** ${campaign.target_demographic_custom || campaign.target_demographic || '-'}`,
    `- **Narrative Mode:** ${campaign.narrative_mode || 'auto'}`,
    `- **Visual:** ${campaign.visual_style || '-'} / ${campaign.visual_mode || '-'} / ${campaign.video_model || '-'}`,
    `- **Format:** ${campaign.target_clips_count || 4} klip × ${campaign.clip_duration || 8}s / ${campaign.aspect_ratio || '9:16'}`,
    `- **Approval:** Belum disetujui`,
    '',
    '> Dokumen ini adalah snapshot review. Produksi TTS, G-Labs, dan FFmpeg belum boleh dimulai sebelum approval eksplisit.',
    ''
  ];
  let campaignVso = {};
  try { campaignVso = JSON.parse(campaign.visual_overrides_json || '{}'); } catch (_) {}
  lines.push('## Konfigurasi Visual Swap');
  lines.push(`- Character: ${campaignVso.character_concept || '-'}`);
  lines.push(`- Subject: ${campaignVso.subject_demographic || '-'}`);
  lines.push(`- Wardrobe mode: ${campaignVso.wardrobe_style || '-'}`);
  lines.push(`- Lighting: ${campaignVso.lighting_style || '-'}`);
  lines.push('');
  items.forEach((item, index) => {
    ...
    lines.push('---', '', `## Konten ${index + 1}: ${payload.content_pillar || 'Tanpa Pilar'}`, '');
    lines.push(`- **Hook:** ${payload.custom_hook || '-'}`);
    lines.push(`- **Content Subject:** ${payload.content_subject || '-'}`);
    lines.push(`- **Narrative Mode:** ${payload.narrative_mode || campaign.narrative_mode || 'auto'}`);
    const resolved = result.resolved_visual_overrides || {};
    lines.push(`- **Resolved Wardrobe:** ${resolved.wardrobe_resolved_key || resolved.wardrobe_style_custom || campaignVso.wardrobe_style || '-'}`);
    lines.push('', '### Storyboard', '| Klip | Durasi | Visual | Kamera | Audio Mood |', '|---|---:|---|---|---|');
    (result.storyboard || []).forEach((scene, sceneIndex) => {
      lines.push(`| ${scene.scene || scene.clip || sceneIndex + 1} | ${scene.duration || '-'} | ${scene.visual_description || '-'} | ${scene.camera_movement || '-'} | ${scene.audio_mood || '-'} |`);
    });
    lines.push('', '### Voiceover');
    (result.voiceover || []).forEach((voice, voiceIndex) => lines.push(`${voiceIndex + 1}. ${voice.narration || voice.voiceover_text || '-'}`));
    for (const [label, key] of [['T2V', 't2v_prompts'], ['T2I', 't2i_prompts'], ['I2V', 'i2v_prompts']]) {
      lines.push('', `### Prompt ${label}`);
      (result[key] || []).forEach((prompt, promptIndex) => lines.push(`${promptIndex + 1}. ${prompt.prompt || '-'}`));
    }
    const caption = result.social_media_package?.caption || result.caption || result.universal_caption || result.tiktok_caption || result.ig_caption || '-';
    lines.push('', '### Caption / CTA / Hashtag', caption, '');
  });
  return lines.join('\n');
}
