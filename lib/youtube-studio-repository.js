...
import { EPISODE_STATES, SCRIPT_STATES, normalizeLocale, assertEpisodeTransition, resolveEpisodeDuration, normalizeTargetDuration } from './youtube-studio-contract.js';
...
export async function setEpisodeGenerationProfile(episodeId, profileKey, actor) {
  const tenantId = getActiveTenantId();
  const ep = await getEpisode(episodeId);
  if (!ep) throw new Error('Episode not found');
  if (ep.status !== 'Script Approved') throw new Error('Generation profile can only be selected after the script is approved.');
  const profile = getGenerationProfile(profileKey);
