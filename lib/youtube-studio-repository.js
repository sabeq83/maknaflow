...
export async function setEpisodeGenerationProfile(episodeId, profileKeyOrObj, actor) {
  ...
  let key, voiceProvider = 'google_tts', voicePersona = 'Orus';
  ...
  const res = await pgQuery(`
    UPDATE youtube_episodes
    SET generation_profile_key = $1, voice_provider = $2, voice_persona = $3, updated_at = CURRENT_TIMESTAMP
    WHERE id = $4 AND tenant_id = $5 RETURNING *
  `, [profile.key, voiceProvider, voicePersona, episodeId, tenantId]);
  return res.rows[0];
}
