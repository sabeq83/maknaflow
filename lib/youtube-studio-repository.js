...
export async function setEpisodeGenerationProfile(episodeId, profileKeyOrObj, actor) {
  ...
  const res = await pgQuery(`
    UPDATE youtube_episodes
    SET generation_profile_key = $1, updated_at = CURRENT_TIMESTAMP
    WHERE id = $2 AND tenant_id = $3 RETURNING *
  `, [profile.key, episodeId, tenantId]);
  return res.rows[0];
}
...
export async function getResolvedNarrativeSnapshot(episodeId) {
  ...
  const locale = normalizeLocale(episode.locale || 'id-ID');
  const audioConfig = await getChannelAudioConfig(episode.channel_id, locale);
  let registrySpeakers = null;

  if (audioConfig) {
    let selected = await getEpisodeCastBindings(episodeId);
    if (selected.length === 0) selected = await getSeriesCastBindings(episode.series_id);
    registrySpeakers = await Promise.all(selected.map(async (speaker) => {
      const castings = await getSpeakerVoiceCastings(speaker.id);
      const tts = castings.find(c => c.locale === locale && c.binding_kind === 'tts') || null;
      const flow = castings.find(c => c.locale === locale && c.binding_kind === 'flow_native') || null;
      return {
        ...speaker,
        source: 'channel_registry',
        tts_binding: tts ? { id: tts.id, version: tts.version, provider: tts.provider, persona_key: tts.persona_key, speed: Number(tts.speed || 1), delivery: tts.delivery_json || {}, pronunciation: tts.pronunciation_json || {} } : null,
        flow_binding: flow ? { id: flow.id, version: flow.version, voice_reference_id: flow.voice_reference_id, descriptive_voice_prompt: flow.descriptive_voice_prompt, delivery: flow.delivery_json || {}, pronunciation: flow.pronunciation_json || {} } : null
      };
    }));
  }

  return resolveNarrativeConfiguration({ channelStrategy: strategy, series, episode, audioConfig, registrySpeakers });
}
...
export async function saveSpeakerVoiceCasting(channelSpeakerId, locale, input, actor) {
  ...
  const audioConfig = await getChannelAudioConfig(speaker.channel_id, locale);
  if (!audioConfig) throw new Error(`Configure channel audio for ${locale} before assigning voices`);
  const validated = validateVoiceCasting(input, audioConfig);
...
export async function createDraftChannelAudioConfig(channelId, input, actor) {
  ...
  const validated = validateChannelAudioConfig(input, getTtsCapabilities(input.provider) || {});
...
export async function activateChannelAudioConfig(channelId, configId, actor) {
  ...
  const requiresVoice = config.audio_experience !== 'no_talking_asmr';
  for (const speaker of requiresVoice ? activeSpeakers : []) {
    const castings = await getSpeakerVoiceCastings(speaker.id);
...
export async function updateChannelSonicIdentity(channelId, sonicIdentityInput, actor) {
  ...
  return createDraftChannelAudioConfig(channelId, { ...activeConfig, sonic_identity_json: validated }, actor);
}
...
export async function replaceSeriesCastBindings(seriesId, speakerIds, actor) {
  ...
  return await withPgTransaction(async (client) => {
    ...
    const seriesRes = await client.query('SELECT channel_id FROM youtube_series WHERE id = $1 AND tenant_id = $2', [seriesId, tenantId]);
    const channelId = seriesRes.rows[0]?.channel_id;
    if (!channelId) throw new Error('Series not found');
    const bindings = [];
    for (const speakerId of speakerIds) {
      const speakerRes = await client.query(`
        SELECT id FROM youtube_channel_speakers 
        WHERE id = $1 AND channel_id = $2 AND tenant_id = $3 AND status = 'active'
      `, [speakerId, channelId, tenantId]);
