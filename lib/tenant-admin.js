...
export async function listTenantsWithCounts() {
  const result = await pgQuery(`
    SELECT t.id, t.name, t.slug, t.timezone, t.status, t.created_at, t.updated_at,
...
export async function updateTenant(tenantId, updates, actor) {
  ...
  return withPgTransaction(async client => {
    const result = await client.query(`
      ...
      WHERE id = $1
      RETURNING id, name, slug, timezone, status, updated_at
    `, [tenantId, status || null, timezone || null]);
    ...
    await client.query(`INSERT INTO tenant_audit_events (actor_user_id, tenant_id, event_type, event_json) VALUES ($1, $2, 'tenant.updated', $3)`, [actor.id, tenantId, JSON.stringify({ status, timezone })]);
