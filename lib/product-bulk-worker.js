...
export async function claimPendingEnrichmentProducts({ tenantId, limit = 10 }) {
  ...
  try {
    ...
    const res = await client.query(`
      SELECT id, product_name, raw_description, product_description, packaging_status, packaging_type, raw_photo_url, photo_provider
  ...
  try {
    ...
    const res = await client.query(`
      SELECT id, product_name, raw_photo_url, clean_photo_t2i_prompt, photo_provider,
             packaging_status, packaging_type, enrichment_status
      FROM product_extractions
      WHERE tenant_id = $1
...
async function enrichProduct(job) {
  const { id, product_name, raw_description, product_description, packaging_status, packaging_type, raw_photo_url, tenantId } = job;
  logToBulk(`[Enrichment] Memulai untuk "${product_name}" (${id})`, tenantId);

  const description = raw_description || product_description || '';
  const prompt = buildEnrichmentPrompt([{ id, product_name, raw_description: description, packaging_status, packaging_type }]);

  try {
    const rawResponse = await executeWithKeyPool(1, async (apiKey) => {
      const { GoogleGenerativeAI } = await import('@google/generative-ai');
      const { getResolvedGeminiModels } = await import('./gemini.js');
      const { resolvedPrimary } = await getResolvedGeminiModels({ taskType: 'SCRAPER' });
    ...
    // [Opsi B] Generasi foto dipisah secara total dari proses enrichment otomatis.
    // Kami TIDAK LAGI mengubah photo_status dari 'approved' menjadi 'pending' secara otomatis di sini.
    /*
    await pgQuery(`
      UPDATE product_extractions
    ...
    */
    ...
    logToBulk(`[Enrichment] ✅ "${product_name}" selesai diperkaya (generasi foto clean otomatis dilewati)`, tenantId);
  } catch (err) {
...
1. JSON wajib valid sesuai schema di atas.
2. ID harus sesuai dengan input.
3. Semua prompt wajib ditulis dalam Bahasa Inggris.
4. PENTING: Jika properti 'packaging_type' tersedia di input data produk (seperti 'Jar plastik' atau 'Botol Spray'), Anda WAJIB mematuhinya untuk menyusun 'geometric_truth' dan 'clean_photo_t2i_prompt'. Jangan pernah mengubah tipe kemasan atau mengasumsikannya menjadi bentuk lain (seperti pouch berdiri) jika tipe kemasan sudah dispesifikasikan oleh pengguna.`;
}
...
async function saveAndCommitCleanPhoto({ tenantId, productId, imageBuffer, mimeType, provider, productName }) {
  try {
    ...
    const autoApprove = Number(getSetting('product_photo_auto_approve') || '0') === 1;
...
// ============================================================
// AUTO-RECOVERY UNTUK STALE / ZOMBIE JOBS
// ============================================================

/**
 * Auto-recovery untuk mendeteksi job yang macet/basi di status 'processing' (> 10 menit)
 */
export async function healStaleProcessingJobs({ tenantId }) {
  try {
    // 1. Reset enrichment_status yang gantung di 'processing' > 10 menit ke 'failed'
    await pgQuery(`
      UPDATE product_extractions
      SET enrichment_status = 'failed',
          enrichment_error = 'Pengayaan AI terputus (stale processing timeout)',
          extraction_status = 'completed',
          updated_at = NOW()
      WHERE tenant_id = $1
    ...
    // 2. Reset photo_status yang gantung di 'processing' tanpa photo_task_id > 10 menit ke 'failed'
    await pgQuery(`
      UPDATE product_extractions
      SET photo_status = 'failed',
          photo_error = 'Pemrosesan foto terputus (stale processing timeout)',
          extraction_status = 'completed',
          updated_at = NOW()
      WHERE tenant_id = $1
        AND photo_status = 'processing'
    ...
    // 3. Sinkronisasi extraction_status yang tertinggal sebagai pending_image padahal foto sudah selesai/approved/failed
    await pgQuery(`
      UPDATE product_extractions
      SET extraction_status = 'completed',
          updated_at = NOW()
      WHERE tenant_id = $1
        AND extraction_status IN ('pending_image', 'generating_image', 'pending')
        AND (
    ...
    console.error('[Bulk Worker Heal Error]:', err.message);
  }
}
...
async function runBulkCycleForTenant(tenantId) {
  ...
  // Auto-heal stale jobs
  await healStaleProcessingJobs({ tenantId });
