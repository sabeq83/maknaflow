...
async function resolveLineageAndAffiliate({
  ...
  // Silent Guard: Jika tidak ada target product id dan tidak ada fallback affiliate link / produk spesifik
  const isNoProductMode = !fallbackProductId && !fallbackAffiliateLink && (!fallbackProductName || fallbackProductName.toLowerCase() === 'umum');
  if (isNoProductMode) {
    return {
...
export async function scanAndSyncExistingCampaigns(targetCampaignId = null) {
  ...
  try {
    ...
    for (const item of opcItems) {
      ...
      const isRecipe = item.recipe_data_json || item.recipe_text_plain || item.content_kind === 'recipe';
      const p = upsertContentFlowItem({
        ...
        recipe_text_markdown: item.recipe_text_markdown || null,
        recipe_text_plain: item.recipe_text_plain || null,
        recipe_payload_json: item.recipe_data_json || null,
        social_media_package_json: item.social_media_package_json || null,
        content_kind: isRecipe ? 'recipe' : 'generic',
        recipe_revision: item.recipe_revision || 1
      });
  ...
  try {
    let strategicWhereClause = 'WHERE sc.tenant_id = ?';
    const strategicQueryParams = [tenantId];
    if (targetCampaignId) {
      strategicWhereClause = 'WHERE sc.tenant_id = ? AND sc.id = ?';
      strategicQueryParams.push(targetCampaignId);
    }
    ...
    const scItems = await db.prepare(`
      ...
      ${strategicWhereClause}
      ORDER BY sci.sequence ASC, sci.id ASC
    `).all(...strategicQueryParams);
  ...
  // 6. Scan Multiplier Lab Tasks
  try {
    let whereClause = 'WHERE t.tenant_id = ?';
    const queryParams = [tenantId];
    if (targetCampaignId) {
      whereClause = 'WHERE t.tenant_id = ? AND t.batch_id = ?';
      queryParams.push(targetCampaignId);
    }

    const multiplierTasks = await db.prepare(`
      SELECT t.*, pe.product_name AS pe_product_name, pe.source_url AS pe_source_url, pe.affiliate_link AS pe_affiliate_link
      FROM re_multiplier_tasks t
      LEFT JOIN product_extractions pe ON t.target_product_id = pe.id
      ${whereClause}
      ORDER BY t.id ASC
    `).all(...queryParams);

    for (const item of multiplierTasks) {
      const cfId = `multiplier_${item.id}`;

      if (!campaignSeqCounters[item.batch_id]) {
        campaignSeqCounters[item.batch_id] = 1;
      }
      const seqNum = campaignSeqCounters[item.batch_id]++;
      ...
      const accountName = 'umum';
      const productName = item.pe_product_name || 'Umum';
      const linkAffiliate = item.affiliate_url || item.pe_affiliate_link || '';
      const productUrl = item.target_product_url || item.pe_source_url || '';
      ...
      const p = upsertContentFlowItem({
        ...
        source_campaign_id: item.batch_id,
        source_item_id: item.id,
        account_name: accountName,
        video_id: `multiplier_${item.id}`,
        campaign_title: `Multiplier Lab - ${item.batch_id.slice(-6)}`,
        hook: `Multiplier Lab Item #${item.id}`,
        nama_produk: lineage.nama_produk,
        link_affiliate: lineage.link_affiliate,
        link_produk: lineage.link_produk,
        caption: item.new_caption || `Multiplier Lab video for ${productName}`,
        production_date: item.created_at || new Date().toISOString().split('T')[0],
        url_asset: item.ffmpeg_output_path || '',
        drive_link: '',
        nextcloud_url: item.nextcloud_video_url || item.ffmpeg_output_path || '',
        pipeline_status: item.status === 'completed' ? 'Completed' : 'In Production',
        brand_profile_id: lineage.brand_profile_id,
        brand_product_id: lineage.brand_product_id,
        product_id: lineage.product_id,
    ...
    console.error('[ContentFlow Sync] Error scanning Multiplier Lab Tasks:', err);
  }
