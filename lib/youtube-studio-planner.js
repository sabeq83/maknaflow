...
import { validateResearchBrief, validateBlueprint, validateSceneScript } from './youtube-studio-contract.js';
...
export async function generateBlueprint(episode, strategy, researchBrief, universe, visualIdentity) {
  ...
  const prompt = `
    ...
    Universe Context: ${universe ? JSON.stringify(universe) : 'None'}
    Visual Style: ${visualIdentity ? JSON.stringify(visualIdentity) : 'None'}
  ...
  const rawText = result.response.text();
  const parsed = parseGeminiJSON(rawText);
  return validateBlueprint(parsed, episode.target_duration_seconds || 600);
}
...
export async function generateScript(episode, blueprint, researchBrief, universe, visualIdentity) {
  ...
  const rawText = result.response.text();
  const parsed = parseGeminiJSON(rawText);
  return validateSceneScript(parsed, blueprint, episode.target_duration_seconds || 600);
}
