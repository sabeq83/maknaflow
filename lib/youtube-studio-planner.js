...
export async function generateResearchBrief(episode, strategy, universe, visualIdentity) {
  ...
  const prompt = `
    ...
    Language/Bahasa: Seluruh teks dalam field JSON (seperti "episode_angle", "audience_intent", "viewer_questions", "keyword_cluster", "key_claims.claim", "editorial_risks", "recommended_structure", "source_requests") WAJIB ditulis dalam bahasa yang sesuai dengan Locale ("${episode.locale}").
...
export async function generateScript(episode, blueprint, researchBrief, universe, visualIdentity) {
  ...
  const prompt = `
    ...
    - Bahasa Penulisan: Seluruh teks "voiceover" dan "subtitle_cue" WAJIB ditulis dalam bahasa yang sesuai dengan Locale ("${episode.locale}"). Jika locale adalah "en-US", tulis "voiceover" and "subtitle_cue" dalam Bahasa Inggris yang natural, ekspresif, dan menarik. Jika locale adalah "id-ID", tulis dalam Bahasa Indonesia. Bidang deskriptif seperti visual_direction, purpose, transition_note, atau audio_cue boleh ditulis dalam Bahasa Indonesia atau Bahasa Inggris (disarankan Bahasa Inggris pada visual_direction untuk mempermudah generator visual t2i/i2v).
    ...
    - Target total kata untuk seluruh video: ${budget.min} sampai ${budget.max} kata (Ideal: ${budget.ideal} kata).
    - Batasan kata per scene:
      ${sceneBudgets.map((b, idx) => `Scene ${idx+1}: target ${b.min}-${b.max} kata (Ideal: ${b.ideal} kata)`).join('\n      ')}
    - Aturan Kalimat: Panjang kalimat ideal ${profile.sentenceWords.min}-${profile.sentenceWords.max} kata. Gunakan kalimat pendek, sederhana, edukatif, dan mudah dipahami.
    - Tambahkan "narration_beats" per scene (misalnya, poin-poin ide edukasi yang dibahas).
      ...
      "scenes": [
        {
          ...
          "narration_beats": ["point 1", "point 2"]
        }
      ]
    }
  `;
