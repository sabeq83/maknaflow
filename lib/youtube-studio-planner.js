...
export async function generateResearchBrief(episode, strategy, universe, visualIdentity, resolvedNarrative) {
  ...
  const narrativePromptContext = resolvedNarrative ? `
    ...
    - Cast Members (Use ONLY these exact speaker IDs, do not invent others): ${JSON.stringify(resolvedNarrative.speakers.map(s => s.speaker_id))}
    - Dialogue Guardrails: ${JSON.stringify(resolvedNarrative.guardrails)}
    - Audio Experience: ${resolvedNarrative.audio_experience || 'narrative_dialogue'}
  ` : '';
...
export async function generateScript(episode, blueprint, researchBrief, universe, visualIdentity, resolvedNarrative) {
  ...
  const narrativePromptContext = resolvedNarrative ? `
    ...
    - Cast Members (Use ONLY these exact speaker IDs, do not invent others): ${JSON.stringify(resolvedNarrative.speakers.map(s => s.speaker_id))}
    - Audio Experience: ${resolvedNarrative.audio_experience || 'narrative_dialogue'}
    ${resolvedNarrative.audio_experience === 'no_talking_asmr' ? 'CRITICAL ASMR RULE: Do NOT generate ANY narration, dialogue, or whisper audio blocks. Only use sound_event, breath, ambience, or silence. Do NOT include a narrator speaker.' : ''}
    ${resolvedNarrative.audio_experience === 'spoken_asmr' ? 'CRITICAL ASMR RULE: You MUST generate whisper audio blocks for dialogue/narration.' : ''}
  ` : '';
      ...
      "scenes": [
        {
          ...
          "audio_blocks": [
            ...
            },
            {
              ...
              "order": 2,
              "type": "dialogue",
              "speaker_id": "${resolvedNarrative?.speakers?.find(s => s.speaker_type !== 'narrator')?.speaker_id || resolvedNarrative?.speakers?.[0]?.speaker_id || 'registered_speaker'}",
  ...
  return validateSceneScript(parsed, blueprint, episode.target_duration_seconds, {
    allowedSpeakerIds: (resolvedNarrative?.speakers || []).map(s => s.speaker_id),
    audioExperience: resolvedNarrative?.audio_experience || 'narrative_dialogue'
  });
}
...
export async function generateSeriesConcepts(channel, strategy) {
  ...
  if (!parsed || !Array.isArray(parsed.concepts)) {
    throw new Error('Format respon AI tidak valid: list concepts tidak ditemukan');
  }

  return parsed.concepts.map(c => validateSeriesConcept(c));
}
