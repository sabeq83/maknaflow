...
export async function claimDueAutomation(workerId,{tenantId=null}={}) {
  return withPgTransaction(async client => {
    const queued=await client.query(`SELECT r.*,s.name AS schedule_name,s.tenant_id AS schedule_tenant_id,s.operator_request_json,s.retry_policy_json,s.auto_pause_threshold,s.execution_mode
      FROM content_automation_runs r JOIN content_automation_schedules s ON s.id=r.schedule_id
      WHERE (((s.status='active' OR s.execution_mode='run_once') AND r.status='queued') OR (r.status='retry_wait' AND r.next_attempt_at<=CURRENT_TIMESTAMP))
...
export async function listRunsToReconcile(){
  return (await pgQuery(`
    SELECT r.*, s.name AS schedule_name, s.execution_mode,
           COALESCE(o.status, op_ar.status) AS operator_status,
           COALESCE(o.current_stage, op_ar.current_stage) AS current_stage,
           COALESCE(r.operator_job_id, ar.operator_job_id) AS effective_operator_job_id
    ...
    LEFT JOIN operator_jobs op_ar ON op_ar.id = ar.operator_job_id
    WHERE r.status IN ('dispatching', 'job_created', 'planning', 'generating_creative', 'generating_start_frames', 'awaiting_approval', 'partially_approved', 'producing', 'syncing_contentflow')
