...
export async function previewSchedulePurge(id){const tenantId=getActiveTenantId();const schedule=(await pgQuery('SELECT id,name,status FROM content_automation_schedules WHERE id=$1 AND tenant_id=$2',[id,tenantId])).rows[0];if(!schedule)return null;const counts=(await pgQuery(`SELECT COUNT(*)::int AS run_count,counts_all.blocker_count FROM content_automation_schedules s, LATERAL (SELECT COUNT(*) FILTER(WHERE status=ANY($3))::int AS blocker_count FROM content_automation_runs WHERE schedule_id=$1 AND tenant_id=$2) counts_all WHERE id=$1 AND tenant_id=$2`,[id,tenantId,PURGE_BLOCKER_RUNS])).rows[0];const countsRuns=(await pgQuery(`SELECT COUNT(*)::int AS run_count FROM content_automation_runs WHERE schedule_id=$1 AND tenant_id=$2`,[id,tenantId])).rows[0];const preview={schedule_id:id,schedule_name:schedule.name,run_count:countsRuns.run_count,blocker_count:counts.blocker_count};return {...preview,preview_token:previewToken(preview)};}
