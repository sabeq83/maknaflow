...
export async function listRunsToReconcile(){return (await pgQuery(`SELECT r.*,s.name AS schedule_name,s.execution_mode,o.status AS operator_status,o.current_stage FROM content_automation_runs r JOIN content_automation_schedules s ON s.id=r.schedule_id LEFT JOIN operator_jobs o ON o.id=r.operator_job_id WHERE r.status IN('job_created','generating_creative','generating_start_frames','awaiting_approval','partially_approved','producing','syncing_contentflow') ORDER BY r.created_at LIMIT 100`)).rows;}
