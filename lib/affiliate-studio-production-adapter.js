...
export async function listBrandContentRuns(user, brandId) {
  if (!user || user.tenantId === '__none__') return [];
  ...
  const runs = (await pgQuery(
    `SELECT r.*, cpr.sequence, cpr.pillar, cpr.category_cep, cpr.context, ap.name as program_title
     FROM affiliate_content_runs r
     JOIN content_planner_rows cpr ON cpr.id = r.planner_row_id
     JOIN affiliate_programs ap ON ap.id = r.affiliate_program_id
     WHERE r.tenant_id = $1 AND r.brand_profile_id = $2
     ORDER BY r.created_at DESC`,
    [tenantId, brand.id]
  )).rows;
  ...
  return runs.map(r => ({
    ...
    affiliateProgramId: r.affiliate_program_id,
    engineType: r.engine_type,
    engineCampaignId: r.engine_campaign_id,
    engineItemId: r.engine_item_id,
    normalizedStatus: r.normalized_status,
...
export async function reconcileProgramRuns(user, brandId, programId) {
  ...
  return reconcileRunsInternal(tenantId, runs);
}

export async function reconcileBrandRuns(user, brandId) {
  if (!user || user.tenantId === '__none__') return false;
...
async function reconcileRunsInternal(tenantId, runs) {
