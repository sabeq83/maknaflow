...
import { resolveWebhookEndpoint } from './webhook-client.js';
...
export function buildGlabsImageLaneKey(webhookOverride) {
  ...
  return crypto.createHash('sha256').update(`${endpoint.host.toLowerCase()}:${endpoint.port}:image`).digest('hex');
}
...
export async function tryAcquireImageLane(leaseId, { leaseSeconds = 120 } = {}) {
  ...
  try {
    ...
    // and expire any orphaned waiting leases that have been waiting > 5 minutes without activity
    await client.query(
    ...
    await client.query(
      `UPDATE glabs_image_lane_leases
       SET status = 'expired', released_at = CURRENT_TIMESTAMP, release_reason = 'wait_abandoned'
       WHERE lane_key = $1 AND status = 'waiting' AND requested_at < CURRENT_TIMESTAMP - INTERVAL '5 minutes'`,
      [target.lane_key]
    );
  ...
  const sql = tenantId
    ? `UPDATE glabs_image_lane_leases
       SET status = 'expired', released_at = CURRENT_TIMESTAMP,
           release_reason = CASE WHEN status = 'waiting' THEN 'wait_abandoned' ELSE 'lease_expired' END
       WHERE tenant_id = $1 AND (
         (status = 'active' AND lease_expires_at < CURRENT_TIMESTAMP) OR
         (status = 'waiting' AND requested_at < CURRENT_TIMESTAMP - INTERVAL '5 minutes')
       )
       RETURNING id`
    : `UPDATE glabs_image_lane_leases
       SET status = 'expired', released_at = CURRENT_TIMESTAMP,
