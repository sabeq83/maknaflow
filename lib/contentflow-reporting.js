...
export function parseAndValidateReportingFilters(rawInput = {}) {
  ...
  const productName = params.product || params.productName || 'all';
...
export async function getContentFlowReporting(filters = {}) {
  ...
  if (productName && productName !== 'all') {
    if (productName.toLowerCase() === 'umum') {
      baseWhere.push(`(nama_produk IS NULL OR TRIM(nama_produk) = '' OR LOWER(nama_produk) = 'umum')`);
    } else {
      baseParams.push(productName);
      baseWhere.push(`nama_produk = $${baseParams.length}`);
    }
  }
  ...
  const cteSql = `
    WITH normalized AS (
      SELECT
        ...
        COALESCE(NULLIF(TRIM(nama_produk), ''), 'Umum') AS product_name,
        pipeline_status,
  ...
  // 6. Available accounts & products facet queries
  ...
  const productFacetParams = [tenantId];
  let productFacetScope = "tenant_id = $1 AND nextcloud_url IS NOT NULL AND nextcloud_url <> ''";
  if (Array.isArray(allowedAccounts)) {
    if (allowedAccounts.length === 0) productFacetScope += ' AND FALSE';
    else {
      productFacetParams.push(allowedAccounts.map(a => String(a).toLowerCase()));
      productFacetScope += ` AND LOWER(account_name) = ANY($2::text[])`;
    }
  }
  if (accountName && accountName !== 'all') {
    productFacetParams.push(accountName.toLowerCase());
    productFacetScope += ` AND LOWER(account_name) = LOWER($${productFacetParams.length})`;
  }

  const productFacetsSql = `
    SELECT DISTINCT COALESCE(NULLIF(TRIM(nama_produk), ''), 'Umum') AS product_name
    FROM content_flow_items
    WHERE ${productFacetScope}
    ORDER BY product_name
  `;

  // Run queries in parallel
  const [summaryRes, timelineRes, brandsRes, productsRes, anomaliesRes, facetsRes, productFacetsRes] = await Promise.all([
    pgQuery(summarySql, scopedParams),
    pgQuery(timelineSql, scopedParams),
    pgQuery(brandsSql, scopedParams),
    pgQuery(productsSql, scopedParams),
    pgQuery(anomaliesSql, scopedParams),
    pgQuery(facetsSql, facetParams),
    pgQuery(productFacetsSql, productFacetParams)
  ]);
  ...
  const products = productsRes.rows.map(r => ({
    ...
    brand: r.brand,
    total_assets: Number(r.total_assets || 0),
    completed_assets: Number(r.completed_assets || 0),
    in_production_assets: Number(r.in_production_assets || 0),
    published_any_assets: Number(r.published_any_assets || 0),
    never_published_assets: Number(r.never_published_assets || 0),
    tiktok: Number(r.tiktok || 0),
    facebook: Number(r.facebook || 0),
  ...
  const available_products = productFacetsRes.rows.map(r => r.product_name);
