...
import { assertProductionTransition, assertHybridBatchTransition, assertProductionMode } from './youtube-studio-contract.js';
...
export async function createProductionPlanDraft({ episodeId, plan, snapshot, approvedScriptId, actor }) {
  ...
  return await withPgTransaction(async (client) => {
    ...
    // 1b. Get universe character canonical prompts
    const epRes = await client.query('SELECT channel_id FROM youtube_episodes WHERE id = $1 AND tenant_id = $2', [episodeId, tenantId]);
    const ep = epRes.rows[0];
    const stratRes = ep ? await client.query("SELECT universe_id FROM youtube_channel_strategies WHERE channel_id = $1 AND tenant_id = $2 AND status = 'active'", [ep.channel_id, tenantId]) : null;
    const universeId = stratRes?.rows[0]?.universe_id;
    
    const characterCanonicals = {};
    if (universeId) {
      const charsRes = await client.query('SELECT character_key, canonical_prompt FROM universe_characters WHERE universe_id = $1', [universeId]);
      for (const row of charsRes.rows) {
        if (row.character_key && row.canonical_prompt) {
          characterCanonicals[row.character_key] = row.canonical_prompt;
        }
      }
    }
    ...
    let scriptJson = {};
    if (approvedScriptId) {
      const scriptRes = await client.query('SELECT script_json FROM youtube_episode_scripts WHERE id = $1', [approvedScriptId]);
      scriptJson = scriptRes.rows[0]?.script_json || {};
    }
    const audioProductionMode = snapshot?.audio_production_snapshot?.audio_production_mode || 'standalone_tts';

    // Pre-process plan to inject character canonicals into the JSON structure (for UI preview consistency)
      ...
      for (const scene of plan.scenes) {
        if (Array.isArray(scene.shots)) {
          for (const shot of scene.shots) {
            ...
            // Auto-fill t2i_prompt, i2v_prompt, and t2v_prompt from shot.prompt if they are missing (for legacy/failsafe compatibility)
            if (shot.asset_type === 'generated_visual') {
              if (!shot.t2i_prompt) shot.t2i_prompt = shot.prompt || null;
              if (!shot.i2v_prompt) shot.i2v_prompt = shot.prompt || null;
              if (!shot.t2v_prompt) shot.t2v_prompt = shot.prompt || null;
            }
            
            if (subjects.length > 0) {
              const canonicals = subjects.map(s => characterCanonicals[s]).filter(Boolean);
              if (canonicals.length > 0) {
                const canonicalPrefix = canonicals.join('. ');
                if (shot.t2i_prompt) {
                  shot.t2i_prompt = `${canonicalPrefix}. ${shot.t2i_prompt}`;
                }
                if (shot.t2v_prompt) {
                  shot.t2v_prompt = `${canonicalPrefix}. ${shot.t2v_prompt}`;
                }
              }
            }
          }
    ...
    for (let i = 0; i < plan.scenes.length; i++) {
      ...
      const scriptScene = scriptJson.scenes?.find(s => s.scene_index === scene.scene_index) || {};
      const isV2 = scriptJson.schema_version === 2;

      if (audioProductionMode === 'standalone_tts' && isV2 && scriptScene.audio_blocks && scriptScene.audio_blocks.length > 0) {
        for (const ab of scriptScene.audio_blocks) {
          ...
          await client.query(`
            INSERT INTO youtube_production_assets (
              id, tenant_id, production_package_id, scene_index, shot_index, asset_type, prompt_snapshot, status, generation_mode, audio_block_id, speaker_id
            )
            VALUES ($1, $2, $3, $4, -1, 'voiceover', $5, 'draft', 'static_asset', $6, $7)
          `, [voId, tenantId, packageId, i, ab.text, ab.block_id, ab.speaker_id]);
        }
      } else if (audioProductionMode === 'standalone_tts') {
        // Voiceover Asset (represented by shot_index = -1) - legacy fallback
        const voId = `ytpa_vo_${Math.random().toString(36).slice(2, 10)}`;
        await client.query(`
          INSERT INTO youtube_production_assets (
            id, tenant_id, production_package_id, scene_index, shot_index, asset_type, prompt_snapshot, status, generation_mode, speaker_id
          )
          VALUES ($1, $2, $3, $4, -1, 'voiceover', $5, 'draft', 'static_asset', 'narrator')
        `, [voId, tenantId, packageId, i, scene.voiceover]);
      }

      // Visual Shots
      const nativeBlocksByShot = audioProductionMode === 'native_scene_audio'
        ? distributeAudioBlocksAcrossShots(scriptScene.audio_blocks || [], scene.shots.length)
        : [];
      for (let j = 0; j < scene.shots.length; j++) {
        const shot = scene.shots[j];
        const visId = `ytpa_vis_${Math.random().toString(36).slice(2, 10)}`;

        const finalT2iPrompt = shot.t2i_prompt || null;
        const finalT2vPrompt = shot.t2v_prompt || null;
        ...
        `, [
          ...
          shot.generation_mode || 't2v', finalT2iPrompt, shot.i2v_prompt || null, 
          finalT2vPrompt, shot.negative_prompt || null, shot.continuity_tokens || null,
          JSON.stringify({
            ...shot,
            t2i_prompt: finalT2iPrompt,
            t2v_prompt: finalT2vPrompt,
            script_scene_index: scriptScene.scene_index ?? scene.scene_index,
            native_audio_blocks: nativeBlocksByShot[j] || []
          })
        ]);
      }
    }
...
function distributeAudioBlocksAcrossShots(audioBlocks, shotCount) {
  if (!Number.isInteger(shotCount) || shotCount < 1) return [];
  const result = Array.from({ length: shotCount }, () => []);
  if (!Array.isArray(audioBlocks) || audioBlocks.length === 0) return result;
  audioBlocks.forEach((block, index) => {
    const target = Math.min(shotCount - 1, Math.floor(index * shotCount / audioBlocks.length));
    result[target].push(block);
  });
  return result;
}

export function assertPackageProductionMode(pkg, expectedMode) {
  assertProductionMode(pkg?.plan_json, expectedMode);
}
...
export async function approveProductionPlan(packageId, actor) {
  ...
  return await withPgTransaction(async (client) => {
    ...
    const audioProductionMode = pkg.context_snapshot_json?.audio_production_snapshot?.audio_production_mode || 'standalone_tts';

    for (const asset of assets) {
      if (audioProductionMode === 'native_scene_audio' && asset.asset_type === 'voiceover') continue;
...
export async function triggerI2VGeneration(assetId, actor) {
  const tenantId = getActiveTenantId();
  ...
  return await withPgTransaction(async (client) => {
    ...
    // Preserve image_path, clear video_path references so worker runs I2V (Step 2)
    const currentJson = asset.output_asset_json || {};
    const newOutputJson = {
      image_path: currentJson.image_path || null
    };

    await client.query(`
      UPDATE youtube_production_assets 
      SET status = 'queued', output_asset_json = $1, error_code = NULL, error_message = NULL, attempt_count = attempt_count + 1, updated_at = CURRENT_TIMESTAMP 
      WHERE id = $2
    `, [JSON.stringify(newOutputJson), assetId]);
    ...
    // Create visual job — worker sees image_path exists, runs I2V (Step 2)
    const jobId = `ytpj_${Math.random().toString(36).slice(2, 10)}`;
    const idempotencyKey = `idemp_prod_job_${asset.id}_v${asset.attempt_count + 2}`;
...
export async function approvePromptPackage(packageId, actor) {
  ...
  return await withPgTransaction(async (client) => {
    ...
    // Auto-succeed any static visual assets (e.g. diagram, map, text_overlay) that do not require processing
    await client.query(`
      UPDATE youtube_production_assets
      SET status = 'succeeded',
          output_asset_json = $1,
          updated_at = CURRENT_TIMESTAMP
      WHERE production_package_id = $2
        AND asset_type NOT IN ('generated_visual', 'voiceover')
    `, [JSON.stringify({ video_path: 'templates/placeholder_16_9.mp4', bypass: true, mode: 'static_asset' }), packageId]);
...
export async function approveStartFrameBatch(batchId, actor) {
  ...
  return await withPgTransaction(async (client) => {
    ...
    const packageRes = await client.query(
      'SELECT * FROM youtube_production_packages WHERE id = $1 AND tenant_id = $2',
      [batch.production_package_id, tenantId]
    );
    const productionPackage = packageRes.rows[0];
    if (!productionPackage) throw new Error('Production package not found');
    const audioMode = productionPackage.context_snapshot_json?.audio_production_snapshot?.audio_production_mode || 'standalone_tts';
    ...
    // Native scene audio is rendered together with each visual clip. There is no
    // external TTS artifact or voiceover approval gate in this mode.
    if (audioMode === 'native_scene_audio') {
      const videoBatchId = await createAndQueueVisualVideoBatch(
        client,
        tenantId,
        batch.production_package_id,
        actor
      );
      return { approved: true, voiceover_skipped: true, video_batch_id: videoBatchId };
    }
...
async function createAndQueueVisualVideoBatch(client, tenantId, packageId, actor) {
  const videoBatchId = `ytpb_vv_${Math.random().toString(36).slice(2, 10)}`;
  await client.query(`
    INSERT INTO youtube_production_batches (id, tenant_id, production_package_id, batch_type, status, created_by)
    VALUES ($1, $2, $3, 'visual_video', 'approved', $4)
  `, [videoBatchId, tenantId, packageId, actor?.username || 'system']);
  ...
  for (const asset of assetsResult.rows) {
    ...
    `, [videoBatchId, tenantId, asset.id]);
    await client.query(`
      INSERT INTO youtube_production_jobs (id, tenant_id, production_package_id, asset_id, job_kind, idempotency_key, status)
      VALUES ($1, $2, $3, $4, 'visual', $5, 'queued')
    `, [jobId, tenantId, packageId, asset.id, idempotencyKey]);
...
export async function invalidateAllVoiceoverAssets(episodeId, actor) {
  const tenantId = getActiveTenantId();

  return await withPgTransaction(async (client) => {
    const pkgRes = await client.query(
      'SELECT * FROM youtube_production_packages WHERE episode_id = $1 AND tenant_id = $2 AND status != \'archived\'',
      [episodeId, tenantId]
    );
    const activePkg = pkgRes.rows[0];
    if (!activePkg) throw new Error('No active production package found');

    const packageId = activePkg.id;

    const assetsRes = await client.query(
      'SELECT * FROM youtube_production_assets WHERE production_package_id = $1 AND asset_type = \'voiceover\'',
      [packageId]
    );
    const voiceoverAssets = assetsRes.rows;
    if (voiceoverAssets.length === 0) return { success: true, count: 0 };
    ...
    for (const asset of voiceoverAssets) {
      const nextAttempt = asset.attempt_count + 1;
      await client.query(`
        UPDATE youtube_production_assets 
        SET status = 'queued', output_asset_json = NULL, error_code = NULL, error_message = NULL, attempt_count = $2, updated_at = CURRENT_TIMESTAMP 
        WHERE id = $1
      `, [asset.id, nextAttempt]);
      ...
      const jobId = `ytpj_${Math.random().toString(36).slice(2, 10)}`;
      const idempotencyKey = `idemp_prod_job_${asset.id}_v${nextAttempt + 1}`;
    ...
    return { count: voiceoverAssets.length };
  });
}

/**
 * Manually trigger an assembly job for a production package.
 * Deletes any existing (failed) assembly job first so it can be re-triggered.
 * Used when user explicitly requests assembly from the Video Production stage.
 */
export async function triggerAssemblyManually(packageId) {
  const tenantId = getActiveTenantId();

  // Move the package back into an active processing state. Without this reset,
  // re-assembling a preview_ready package leaves the UI poller idle even though
  // a new assembly job is queued.
  await pgQuery(`
    UPDATE youtube_production_packages
    SET status = 'generating', preview_asset_json = NULL, updated_at = CURRENT_TIMESTAMP
    WHERE id = $1 AND tenant_id = $2
  `, [packageId, tenantId]);

  // Remove previous assembly jobs (e.g. failed ones) so we can re-queue
  await pgQuery(`
    DELETE FROM youtube_production_jobs
    WHERE production_package_id = $1 AND tenant_id = $2 AND job_kind = 'assembly'
  `, [packageId, tenantId]);

  const assemblyJobId = `ytpj_asm_${Math.random().toString(36).slice(2, 10)}`;
  // Use timestamp in idempotency key so repeated manual triggers are each allowed
  const assemblyIdemp = `idemp_assembly_manual_${packageId}_${Date.now()}`;
...
/**
 * Retrieve the status of the latest assembly job for a production package.
 */
export async function getLatestAssemblyJob(packageId) {
  const tenantId = getActiveTenantId();
  const res = await pgQuery(`
    SELECT status, error_message, created_at
    FROM youtube_production_jobs
    WHERE production_package_id = $1 AND tenant_id = $2 AND job_kind = 'assembly'
    ORDER BY created_at DESC LIMIT 1
  `, [packageId, tenantId]);
  return res.rows[0] || null;
