  ...
  return await withPgTransaction(async (client) => {
    ...
    for (let i = 0; i < plan.scenes.length; i++) {
      ...
      for (let j = 0; j < scene.shots.length; j++) {
        ...
        let finalT2iPrompt = shot.t2i_prompt || null;
        let finalT2vPrompt = shot.t2v_prompt || null;
        ...
        if (subjects.length > 0) {
          ...
          if (canonicals.length > 0) {
            ...
            if (finalT2iPrompt) {
              finalT2iPrompt = `${canonicalPrefix}. ${finalT2iPrompt}`;
            }
            if (finalT2vPrompt) {
              finalT2vPrompt = `${canonicalPrefix}. ${finalT2vPrompt}`;
            }
          }
        }
