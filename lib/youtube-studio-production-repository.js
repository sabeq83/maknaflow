...
export async function createProductionPlanDraft({ episodeId, plan, snapshot, approvedScriptId, actor }) {
  ...
  return await withPgTransaction(async (client) => {
    ...
    const stratRes = ep ? await client.query('SELECT universe_id FROM youtube_channel_strategies WHERE channel_id = $1 AND tenant_id = $2', [ep.channel_id, tenantId]) : null;
