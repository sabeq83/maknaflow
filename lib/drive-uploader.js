 ...
 */
export async function uploadUrlToPublicDrive(sourceUrl, filename, folderId, options = {}) {
  const auth = getAuthorizedClient();
  const drive = google.drive({ version: 'v3', auth });
  const appProperties = options.appProperties || {};

  // Check existing upload by appProperties.jobId for idempotency
  if (appProperties.jobId) {
    try {
      ...
      if (searchRes.data.files && searchRes.data.files.length > 0) {
        ...
        const directDownloadUrl = `https://drive.google.com/uc?export=download&id=${existing.id}`;
  ...
  // Set public agar Repliz bisa download tanpa auth
  try {
    await drive.permissions.create({
      fileId,
      requestBody: { role: 'reader', type: 'anyone' },
    ...
    console.warn(`[Drive Uploader] Failed to set public reader permission for ${fileId}:`, permErr.message);
  }
