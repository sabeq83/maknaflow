...
/**
 * Safely resolves webhook endpoint coordinates from Settings (tenant_settings/env), with optional valid brand override.
 * Guaranteeing no 'Invalid URL' errors due to invalid host formatting or non-numeric port strings.
 */
export function resolveWebhookEndpoint(webhookOverride = null) {
  const cleanEndpointHost = (input) => {
    let h = String(input || '').trim().replace(/^https?:\/\//i, '').replace(/\/.*$/, '').trim();
    let extractedPort = null;
    if (h.includes(':')) {
      const parts = h.split(':');
      h = parts[0].trim();
      extractedPort = parts[1]?.trim();
    }
    return { host: h, extractedPort };
  };

  const globalCleaned = cleanEndpointHost(getSetting('webhook_host') || process.env.WEBHOOK_HOST);
  const globalPortRaw = parseInt(getSetting('webhook_port') || process.env.WEBHOOK_PORT || globalCleaned.extractedPort || '8765', 10);
  const globalPort = (!isNaN(globalPortRaw) && globalPortRaw > 0 && globalPortRaw <= 65535) ? String(globalPortRaw) : '8765';
  const globalApiKey = (getSetting('webhook_api_key') || '').trim();

  const overrideRawHost = webhookOverride?.webhook_host || webhookOverride?.host || '';
  const overrideCleaned = cleanEndpointHost(overrideRawHost);
  const overridePortRaw = parseInt(webhookOverride?.webhook_port || webhookOverride?.port || overrideCleaned.extractedPort, 10);
  const hasValidOverrideHost = overrideCleaned.host.length > 0;

  let effectiveHost = globalCleaned.host || '127.0.0.1';
  let effectivePort = globalPort;
  let effectiveApiKey = globalApiKey;

  if (hasValidOverrideHost) {
    effectiveHost = overrideCleaned.host;
    if (!isNaN(overridePortRaw) && overridePortRaw > 0 && overridePortRaw <= 65535) {
      effectivePort = String(overridePortRaw);
    }
    const overrideKey = (webhookOverride?.webhook_api_key || webhookOverride?.api_key || '').trim();
    if (overrideKey) effectiveApiKey = overrideKey;
  }
...
export function getBaseUrl(taskId = null) {
  if (taskId) {
    try {
      const route = getGlabsTaskRouteSync(taskId);
      if (route && route.host) {
        const resolved = resolveWebhookEndpoint({ webhook_host: route.host, webhook_port: route.port, webhook_api_key: route.api_key });
        return resolved.baseUrl;
      }
    } catch (_) {}
  }
  return resolveWebhookEndpoint().baseUrl;
}

export function getApiKey(taskId = null) {
  if (taskId) {
    try {
      const route = getGlabsTaskRouteSync(taskId);
  ...
  return resolveWebhookEndpoint().apiKey;
}

export function getHeaders(taskId = null) {
  return {
...
/**
 * Calculate delay range based on mediaType (image vs video)
 */
export function resolveWebhookDelayRange(mediaType = 'image') {
  const isVideo = mediaType === 'video';
  const minKey = isVideo ? 'webhook_video_delay_min' : 'webhook_image_delay_min';
  const maxKey = isVideo ? 'webhook_video_delay_max' : 'webhook_image_delay_max';

  const defaultMin = isVideo ? 10 : 3;
  const defaultMax = isVideo ? 20 : 7;

  const minSetting = getSetting(minKey) ?? getSetting('webhook_delay_min');
  const maxSetting = getSetting(maxKey) ?? getSetting('webhook_delay_max');

  const min = minSetting !== null && minSetting !== undefined ? parseInt(minSetting, 10) : defaultMin;
  const max = maxSetting !== null && maxSetting !== undefined ? parseInt(maxSetting, 10) : defaultMax;

  return {
    min: isNaN(min) || min < 0 ? defaultMin : min,
    max: isNaN(max) || max < min ? defaultMax : max
  };
}
 ...
 */
export async function applyRandomDelay(mediaType = 'image') {
  ...
  const { min, max } = resolveWebhookDelayRange(mediaType);
  if (max === 0) return 0;

  const seconds = Math.floor(Math.random() * (max - min + 1)) + min;
  if (seconds > 0) {
    console.info(`[Webhook Client] Safety delay active (${mediaType}). Waiting ${seconds}s before submitting request to G-Labs...`);
...
export async function webhookHealth(hostOverride, portOverride) {
  const endpoint = resolveWebhookEndpoint({ webhook_host: hostOverride, webhook_port: portOverride });
  const targetUrl = `${endpoint.baseUrl}/api/health`;
...
export function sanitizeImagePrompt(prompt) {
  ...
  // 2. Strip Midjourney syntax (aspect ratio & negative clauses up to layer tags or brackets)
  cleaned = cleaned.replace(/--ar\s+\d+:\d+/gi, '');
  cleaned = cleaned.replace(/--no\s+[^\[\(\n]+/gi, '');
...
export function normalizeGlabsImageModel(model) {
  const valid = ['nano_banana_pro', 'nano_banana_2', 'nano_banana_2_lite'];
  if (typeof model === 'string' && valid.includes(model.trim().toLowerCase())) {
    return model.trim().toLowerCase();
  }
  return GLABS_IMAGE_MODEL;
}

/**
 * Submit image generation request
 */
export async function generateImage({ prompt, model, aspect_ratio, reference_images, expected_reference_sha256s, upscale, webhookOverride }) {
  await applyRandomDelay('image');
  const primaryModel = normalizeGlabsImageModel(model || getSetting('webhook_image_model') || GLABS_IMAGE_MODEL);
  const sanitizedPrompt = sanitizeImagePrompt(prompt);
  const referenceMetadata = inspectBase64ImageReferences(reference_images || []);
  const payloadReferenceSha256s = referenceMetadata.map(metadata => metadata.sha256);
  if (expected_reference_sha256s && JSON.stringify(payloadReferenceSha256s) !== JSON.stringify(expected_reference_sha256s)) {
    const error = new Error('SHA reference image berubah sebelum dikirim ke provider.');
    error.code = 'IMAGE_REFERENCE_SHA_MISMATCH';
    throw error;
  }
  if (referenceMetadata.length) {
    console.info('[Webhook Client] Safe image reference metadata:', {
      reference_count:referenceMetadata.length,
      reference_sha256s:payloadReferenceSha256s,
      reference_mime_types:referenceMetadata.map(metadata => metadata.mimeType),
      reference_byte_lengths:referenceMetadata.map(metadata => metadata.byteLength)
    });
  }
  ...
  const endpoint = resolveWebhookEndpoint(webhookOverride);
  const submitBaseUrl = endpoint.baseUrl;
  const submitHeaders = {
    'Content-Type': 'application/json',
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (primaryModel === 'nano_banana_pro' && isQuotaError(errorMsg)) {
        ...
        return generateImage({ prompt, model: 'nano_banana_2', aspect_ratio, reference_images, expected_reference_sha256s, upscale, webhookOverride });
      }
  ...
  if (resData?.task_id) {
    taskParamsMap.set(resData.task_id, { prompt, model: primaryModel, aspect_ratio, reference_images, expected_reference_sha256s, upscale, webhookOverride });
    try {
      const { saveGlabsTaskRoute } = await import('./db.js');
      saveGlabsTaskRoute(resData.task_id, endpoint.host, endpoint.port, endpoint.apiKey);
    } catch (e) {
 ...
 */
export async function generateVideo({ prompt, model, aspect_ratio, mode, resolution, video_length, duration, reference_images, webhookOverride, noSoundOverride }) {
  await applyRandomDelay('video');
  ...
  const rawLength = Number(video_length || duration);
  const finalVideoLength = (!isNaN(rawLength) && rawLength > 0) ? rawLength : null;

  const silentPrompt = noSoundOverride ? finalPrompt : `silent video, mute, no sound: ${finalPrompt}`;
  const body = {
    prompt: silentPrompt,
  ...
  if (finalVideoLength) {
    body.video_length = finalVideoLength;
    body.duration = finalVideoLength;
  }
  ...
  if (useQueue) {
    try {
      ...
      const job = await addGlabsJob('video', {
        ...
        aspect_ratio: aspect_ratio || '9:16',
        mode: videoMode,
        video_length: finalVideoLength || undefined,
        duration: finalVideoLength || undefined,
      ...
      if (params && params.model === 'nano_banana_pro') {
        ...
        try {
          const retryResult = await generateImage({
            ...
            expected_reference_sha256s: params.expected_reference_sha256s,
...
export async function getTaskResult(taskId) {
  if (process.env.NODE_ENV === 'test') {
    return { results: ['templates/placeholder_16_9.mp4'] };
  }
...
export function getFileUrl(filename, taskId = null) {
  ...
  if (host) {
    const endpoint = resolveWebhookEndpoint({ webhook_host: host, webhook_port: port });
    return `${endpoint.baseUrl}/api/files/${filename}`;
  }
