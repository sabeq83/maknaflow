...
async function syncContentFlowSummary(job, outcome, permalink = null) {
  try {
    ...
    if (platform === 'facebook') {
      ...
      const statusText = statusMap[outcome] || 'Failed';
      
      await pgQuery(`
        UPDATE content_flow_items
        SET 
          facebook_status = $1,
          facebook_publish_date = CASE WHEN $1 = 'Published' THEN $2 ELSE facebook_publish_date END,
...
export async function processReplizJob(job, credentials, { correlationId, attemptNumber, startedAt }) {
  try {
    ...
    if (!job.external_schedule_id) {
      // Resolve any redirect on the media URL so Repliz gets a direct download link
      let resolvedMediaUrl = job.media_url_snapshot;
      if (resolvedMediaUrl) {
        try {
          const headRes = await fetch(resolvedMediaUrl, { method: 'HEAD', redirect: 'follow', signal: AbortSignal.timeout(10000) });
          if (headRes.url && headRes.url !== resolvedMediaUrl) {
            console.log(`[Publishing Worker] Resolved media URL redirect: ${resolvedMediaUrl} → ${headRes.url}`);
            resolvedMediaUrl = headRes.url;
          }
        } catch (resolveErr) {
          console.warn(`[Publishing Worker] Could not resolve media redirect, using original URL: ${resolveErr.message}`);
        }
      }
