...
import { uploadUrlToPublicDrive } from './drive-uploader.js';
...
function resolveToProxyUrl(mediaUrl) {
  const publicMediaProxyUrl = process.env.PUBLIC_MEDIA_PROXY_URL || '';
  if (!mediaUrl || !publicMediaProxyUrl) return mediaUrl;

  try {
    const mediaProxyAllowedHosts = (process.env.MEDIA_PROXY_ALLOWED_HOSTS || 'cloud.ast402.my.id').split(',');
    const parsed = new URL(mediaUrl);
    if (mediaProxyAllowedHosts.includes(parsed.hostname)) {
      const encoded = Buffer.from(mediaUrl).toString('base64url');
      const proxyUrl = `${publicMediaProxyUrl}/api/media-proxy?url=${encoded}`;
...
/**
 * Pastikan media URL bisa diakses publik oleh Repliz.
 * Jika URL dari Nextcloud (diblokir Cloudflare robots.txt), otomatis upload ke Google Drive.
 * Simpan Google Drive URL ke DB agar retry berikutnya tidak upload ulang.
 */
async function ensurePublicMediaUrl(job) {
  const mediaUrl = job.media_url_snapshot;
  if (!mediaUrl) return mediaUrl;

  try {
    const parsed = new URL(mediaUrl);
    const nextcloudUploadHosts = (process.env.MEDIA_PROXY_ALLOWED_HOSTS || 'cloud.ast402.my.id').split(',');

    // Jika bukan dari Nextcloud, URL sudah bisa diakses publik
    if (!nextcloudUploadHosts.includes(parsed.hostname)) return mediaUrl;

    const publishingDriveFolderId = getSetting('repliz_drive_folder_id') || process.env.MAKNA_PUBLISHING_DRIVE_FOLDER_ID || '';
    const nextcloudInternalBaseUrl = process.env.NEXTCLOUD_INTERNAL_BASE || 'http://100.78.186.123';

    // Upload ke Google Drive jika folder dikonfigurasi
    if (publishingDriveFolderId) {
      console.log(`[Publishing Worker] Media dari Nextcloud — upload ke Google Drive folder ${publishingDriveFolderId}...`);
      const internalUrl = `${nextcloudInternalBaseUrl}${parsed.pathname}${parsed.search}`;
      const filename = `publishing_${job.id}_${job.platform}.mp4`;

      const { directDownloadUrl } = await uploadUrlToPublicDrive(
        internalUrl,
        filename,
        publishingDriveFolderId
      );

      // Simpan ke DB — retry berikutnya tidak perlu upload ulang
      await markPublishingResult(job.tenant_id, job.id, { media_url_snapshot: directDownloadUrl });
      console.log(`[Publishing Worker] GDrive upload sukses: ${directDownloadUrl}`);
      return directDownloadUrl;
    }

    // Fallback: proxy URL jika GDrive tidak dikonfigurasi
    return resolveToProxyUrl(mediaUrl);

  } catch (err) {
    console.error(`[Publishing Worker] ensurePublicMediaUrl error: ${err.message} — fallback ke URL original`);
    return mediaUrl;
  }
}
...
export async function processReplizJob(job, credentials, { correlationId, attemptNumber, startedAt }) {
  try {
    ...
    if (!job.external_schedule_id) {
      // Resolve media URL: jika dari Nextcloud, upload ke Google Drive agar Repliz
      // bisa download tanpa diblokir robots.txt Cloudflare.
