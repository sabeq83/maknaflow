 ...
 * DEPRECATED: Digantikan oleh ensurePublicMediaUrl() yang upload ke Google Drive.
 * Dipertahankan sebagai fallback jika GDrive tidak terkonfigurasi.
 */
 ...
 */
const NEXTCLOUD_UPLOAD_HOSTS = (process.env.MEDIA_PROXY_ALLOWED_HOSTS || 'cloud.ast402.my.id').split(',');
const NEXTCLOUD_INTERNAL_BASE_URL = process.env.NEXTCLOUD_INTERNAL_BASE || 'http://100.78.186.123';
const PUBLISHING_DRIVE_FOLDER_ID = process.env.MAKNA_PUBLISHING_DRIVE_FOLDER_ID || '';
...
async function ensurePublicMediaUrl(job) {
  ...
  try {
    ...
    if (!NEXTCLOUD_UPLOAD_HOSTS.includes(parsed.hostname)) return mediaUrl;

    // Upload ke Google Drive jika folder dikonfigurasi
    if (PUBLISHING_DRIVE_FOLDER_ID) {
      console.log(`[Publishing Worker] Media dari Nextcloud — upload ke Google Drive...`);
      const internalUrl = `${NEXTCLOUD_INTERNAL_BASE_URL}${parsed.pathname}${parsed.search}`;
      const filename = `publishing_${job.id}_${job.platform}.mp4`;

      const { directDownloadUrl } = await uploadUrlToPublicDrive(
        internalUrl,
        filename,
        PUBLISHING_DRIVE_FOLDER_ID
      );
