...
import { uploadUrlToPublicDrive, verifyAnonymousDriveDownload, normalizeGoogleDriveDirectUrl } from './drive-uploader.js';
import { stageMediaForRepliz, isNextcloudMediaUrl } from './publishing-drive-staging.js';

import {
  startPublishingMediaCleanupWorker,
  stopPublishingMediaCleanupWorker
} from './publishing-media-cleanup-worker.js';
...
function getReplizPostType(platform, mediaType) {
  ...
  // NOTE: Instagram 'reel' type causes Repliz to use Instagram native scheduling yang
  // mengembalikan { type: 'schedule' } postId — tidak bisa di-resolve. Gunakan 'video' seperti TikTok.
  if (['tiktok', 'youtube', 'linkedin', 'threads', 'instagram'].includes(platform)) return 'video';
  if (platform === 'facebook') return mediaType === 'reels' ? 'reel' : 'video';
  return 'video';
}

/**
 * Fail-closed Google Drive staging for Repliz publishing:
 * 1. If media is already a Google Drive direct download URL, verifies anonymous probe.
 * 2. If media is from Nextcloud or other storage, uploads to Google Drive with idempotency metadata.
 * 3. Verifies anonymous download probe before returning.
 * 4. NEVER falls back to raw Nextcloud URL on failure.
 */
async function stagePublishingMediaFailClosed(job, { correlationId, attemptNumber } = {}) {
  let mediaUrl = job.media_url_snapshot;
  if (!mediaUrl) return mediaUrl;

  mediaUrl = normalizeGoogleDriveDirectUrl(mediaUrl);

  // If already a Google Drive direct download URL, verify anonymous probe
  if (mediaUrl.includes('drive.usercontent.google.com') || mediaUrl.includes('drive.google.com/uc?export=download') || mediaUrl.includes('drive.google.com/file/d/')) {
    try {
      await verifyAnonymousDriveDownload(mediaUrl);
      console.log(`[Publishing Worker] Existing Drive URL verified via anonymous probe for job ${job.id}: ${mediaUrl}`);
      return mediaUrl;
    } catch (probeErr) {
      console.warn(`[Publishing Worker] Existing Drive URL failed anonymous probe (${probeErr.message}), re-staging...`);
    }
  }


  console.log(`[Publishing Worker] Staging media for job ${job.id} to Google Drive...`);
  if (correlationId && attemptNumber) {
    await appendPublishingAttempt({
      ...
      stage: 'media_staging',
      outcome: 'processing',
      sanitizedMessage: 'Mengunggah media ke Google Drive staging...'
    });
  }
  ...
  if (correlationId && attemptNumber) {
    await appendPublishingAttempt({
      ...
      stage: 'media_staging',
      outcome: 'success',
      sanitizedMessage: 'Media berhasil diunggah ke Google Drive dan lolos anonymous probe.'
    });
  }

  return staged.directDownloadUrl;
}
...
export async function processReplizJob(job, credentials, { correlationId, attemptNumber, startedAt }) {
  try {
    ...
    if (!job.external_schedule_id) {
      // Step 1: Create schedule with fail-closed Google Drive staging
      let resolvedMediaUrl = job.media_url_snapshot;
      if (job.media_type !== 'text_only' && resolvedMediaUrl) {
        resolvedMediaUrl = await stagePublishingMediaFailClosed(job, { correlationId, attemptNumber });
      }
      if (resolvedMediaUrl) {
        resolvedMediaUrl = normalizeGoogleDriveDirectUrl(resolvedMediaUrl);
      }

      const isAiGeneratedContent = job.platform === 'tiktok'
        ? (job.is_ai_generated !== false)
        : !!job.is_ai_generated;
      ...
      const payload = {
        ...
        title: job.platform === 'youtube' ? (job.content_title || job.content_id || '') : '',
        ...
        additionalInfo: {
          isAiGenerated: isAiGeneratedContent,
          isDraft: job.publish_mode === 'draft',
      ...
      const externalScheduleId = scheduleState.id || String(res?.id || res?._id || res?.scheduleId);

      await markPublishingResult(job.tenant_id, job.id, {
        external_schedule_id: externalScheduleId,
        status: 'processing',
        provider_stage: scheduleState.status || 'scheduled'
      });

      await recordReplizProviderState(job.tenant_id, job.id, scheduleState);
      await syncContentFlowSummary(job, 'processing');

      // Poll immediately or on next tick (10s)
      const nextAttempt = new Date(Date.now() + 10000).toISOString();
      await markPublishingResult(job.tenant_id, job.id, {
        nextAttemptAt: nextAttempt
      });
    } else if (job.status === 'retry_wait') {
      // Step 2: Retrying a failed Repliz schedule
      console.log(`[Publishing Worker] Retrying Repliz schedule ${job.external_schedule_id} for job ${job.id}...`);
      const res = await retryReplizSchedule(credentials, job.external_schedule_id);
      const scheduleState = extractReplizScheduleState(res);
      ...
      // Step 3: Polling / reconciling status
      ...
      if (['completed', 'success', 'published'].includes(replizStatus)) {
        ...
        if (!postId && res?.postId && typeof res.postId === 'object' && res.postId.type === 'schedule') {
        ...
        const permalink = scheduleState.permalink || buildPlatformPermalink(job.platform, postId, username, pageId);
        ...
        await recordPublishingAccountHealth(job.tenant_id, job.account_id, {
          isConnected: true,
          lastErrorCode: null,
        ...
        throw new ReplizApiError(scheduleState.errorMessage || 'Repliz schedule execution failed', {
          code: scheduleState.errorCode,
          logId: scheduleState.providerLogId
        });
    ...
    await recordReplizProviderState(job.tenant_id, job.id, {
      status: 'failed',
      errorCode: failure.code,
      errorMessage: failure.message,
      action: failure.action,
      providerLogId: err.logId || null
    });
    ...
    if (failure.code === 'REPLIZ_FACEBOOK_PERMISSION_REQUIRED') {
      await recordPublishingAccountHealth(job.tenant_id, job.account_id, {
        isConnected: false,
      ...
      await syncContentFlowSummary(job, 'needs_review');
    } else if (failure.class === 'provider_transient' && attemptNumber < (job.max_attempts || 3)) {
      const retryDelayMs = calculateRetryDelay(attemptNumber);
      const nextAttempt = new Date(Date.now() + retryDelayMs).toISOString();
      ...
      await markPublishingResult(job.tenant_id, job.id, {
        status: 'needs_review',
        lastErrorCode: failure.code,
        lastErrorMessage: `${failure.message} Batas percobaan otomatis (${job.max_attempts || 3}) telah habis.`
      });
      await syncContentFlowSummary(job, 'needs_review');
    } else if (failure.class === 'media_invalid') {
      await markPublishingResult(job.tenant_id, job.id, {
        status: 'needs_review',
        lastErrorCode: failure.code,
        lastErrorMessage: failure.message
      ...
      await syncContentFlowSummary(job, 'needs_review');
    } else if (failure.class === 'request_invalid') {
      await markPublishingResult(job.tenant_id, job.id, {
        status: 'failed',
        lastErrorCode: failure.code,
        lastErrorMessage: failure.message
      ...
      await markPublishingResult(job.tenant_id, job.id, {
        status: failure.targetStatus || 'needs_review',
        lastErrorCode: failure.code,
        lastErrorMessage: failure.message
      });
      await syncContentFlowSummary(job, failure.targetStatus || 'needs_review');
    }
  }
}

let lastReplizHealthSyncAt = 0;
const REPLIZ_HEALTH_SYNC_INTERVAL_MS = 30 * 60 * 1000; // 30 minutes

/**
 * Sinkronisasi status kesehatan akun Repliz secara berkala di background.
 */
async function runPeriodicReplizHealthSync() {
  const now = Date.now();
  if (now - lastReplizHealthSyncAt < REPLIZ_HEALTH_SYNC_INTERVAL_MS) {
    return;
  }
  lastReplizHealthSyncAt = now;
  ...
  try {
    ...
    const credentials = { apiUrl: url, accessKey, secretKey };
    const remoteAccounts = await listReplizAccounts(credentials);
    ...
    for (const remote of remoteAccounts) {
      ...
      // Update status lokal jika sebelumnya disconnected dan sekarang sudah active
      await pgQuery(`
        ...
        SET
          ...
          updated_at = CURRENT_TIMESTAMP
        WHERE provider = 'repliz' AND provider_account_id = $2 AND status != $1
      `, [newStatus, providerAccountId]);
    }
    console.log('[Publishing Worker] Periodic Repliz accounts health sync completed.');
  } catch (err) {
    console.warn('[Publishing Worker] Periodic Repliz health sync error:', err.message);
  }
}
...
export async function runPublishingTick() {
  ...
  try {
    ...
    // 2. Periodic background sync status akun Repliz (setiap 30 menit)
    await runPeriodicReplizHealthSync();

    // 3. Claim due jobs up to MAX_BATCH_PER_TICK
    for (let i = 0; i < MAX_BATCH_PER_TICK; i++) {
      ...
      // Non-Silent Handling: Jika akun terputus / tidak aktif, tandai needs_review dan catat log error eksplisit
      if (job.account_status && job.account_status !== 'active') {
        const errMsg = `Postingan tertunda: Akun '${job.account_display_name || job.account_id}' sedang berstatus '${job.account_status}'. Memerlukan otorisasi ulang atau koneksi kembali.`;
        console.warn(`[Publishing Worker] Job ${job.id} skipped due to non-active account status: ${job.account_status}`);
        ...
        await appendPublishingAttempt({
          ...
          stage: 'account_health_check',
          outcome: 'needs_review',
          sanitizedMessage: errMsg
        });
...
export function startPublishingWorker(intervalMs = DEFAULT_INTERVAL_MS) {
  ...
  // Start periodic staging media cleanup worker
  try {
    const cleanupIntervalMs = parseInt(process.env.PUBLISHING_MEDIA_CLEANUP_INTERVAL_MS || '3600000', 10);
    startPublishingMediaCleanupWorker(cleanupIntervalMs);
  } catch (cleanErr) {
    console.warn('[Publishing Worker] Failed to start media cleanup worker:', cleanErr.message);
  }
