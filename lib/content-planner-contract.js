...
import { validateRecipePlannerDraft, MIN_RECIPE_COUNT, MAX_RECIPE_COUNT } from './recipe-campaign-contract.js';
...
export function validatePlannerDraft(params) {
  ...
  if (focus === 'recipe_campaign') {
    validateRecipePlannerDraft({
      category: params.recipe_config?.category || params.recipe_category || params.category,
      count: params.planner_count,
      productIds: params.recipe_config?.product_ids || params.product_ids || (params.product_id ? [params.product_id] : []),
