...
export async function generateVoiceSegment({ text, locale, provider = 'google_tts', persona = 'Orus', speed = 1.0, outputPath }) {
  if (provider === 'minimax') {
    const resPath = await generateMinimaxVO(text, persona, outputPath, { language: locale, speed });
  ...
  const buffer = await callGeminiTtsApi(text, persona, { speed });
