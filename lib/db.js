...
export function interceptQuery(sql, params) {
  ...
  if (selectOrUpdateOrDeleteRegex.test(targetSql)) {
    ...
    let tenantColPrefix = '';
    for (const table of isolatedTables) {
      const aliasMatch = queryPart.match(new RegExp(`\\b${table}\\s+(?:AS\\s+)?([a-zA-Z0-9_]+)\\b`, 'i'));
      if (aliasMatch && !/^(WHERE|ON|JOIN|LEFT|RIGHT|INNER|OUTER|CROSS|FULL|NATURAL|GROUP|ORDER|LIMIT|HAVING|SET|UNION|WINDOW)$/i.test(aliasMatch[1])) {
        tenantColPrefix = `${aliasMatch[1]}.`;
        break;
      }
    }

    if (/\bWHERE\b/i.test(queryPart)) {
      queryPart = queryPart + ` AND ${tenantColPrefix}tenant_id = ?`;
    } else {
      queryPart = queryPart + ` WHERE ${tenantColPrefix}tenant_id = ?`;
    }
...
export const TENANT_STRICT_SECRETS = new Set([
  'gemini_api_key',
  'minimax_api_key',
  'minimax_group_id',
  'webhook_api_key',
  ...
  'google_publishing_folder_id',
  'repliz_drive_folder_id',
  'operator_presets_json'
]);

export function getSetting(key, allowDefaultFallback = true) {
  const tenantId = getActiveTenantId();
  if (tenantSettingsCache[tenantId] && tenantSettingsCache[tenantId][key] !== undefined) {
    return tenantSettingsCache[tenantId][key];
  }
  // ZERO LEAKAGE: Dilarang keras fallback ke default_tenant jika key adalah SECRET/CREDENTIAL
  if (TENANT_STRICT_SECRETS.has(key)) {
    return null;
  }
...
const DEFAULT_CONFIGS = {
  ...
  youtube_production:          { is_enabled: 1, mode: 'manual', interval_minutes: 1, jobs_per_day: 100, window_start: '00:00', window_end: '23:59' },
  youtube_production_asset:    { is_enabled: 1, mode: 'manual', interval_minutes: 1, jobs_per_day: 100, window_start: '00:00', window_end: '23:59' },
  youtube_production_assembly: { is_enabled: 1, mode: 'manual', interval_minutes: 1, jobs_per_day: 100, window_start: '00:00', window_end: '23:59' },
  youtube_production_final:    { is_enabled: 1, mode: 'manual', interval_minutes: 1, jobs_per_day: 100, window_start: '00:00', window_end: '23:59' },
  youtube_publishing:          { is_enabled: 1, mode: 'manual', interval_minutes: 1, jobs_per_day: 100, window_start: '00:00', window_end: '23:59' },
};
...
export async function createReCampaign({
  ...
  visual_identity_preset_version = null,
  ...
  return db.prepare(`
    INSERT INTO re_campaigns (
      ...
      words_per_clip, clip_duration, face_visibility, enable_tts, enable_glabs, enable_ffmpeg, enable_social_post,
...
export async function listReCampaigns() {
  ...
  return await dbAll(`
    SELECT c.*, b.brand_name,
      EXISTS(SELECT 1 FROM content_flow_items cf WHERE cf.source_campaign_id = c.id AND cf.source_type = 're' AND cf.tenant_id = c.tenant_id) as is_synced
    FROM re_campaigns c
    LEFT JOIN brand_profiles b ON c.brand_profile_id = b.id
    WHERE c.tenant_id = ?
...
export async function createBrandProfile(data) {
  ...
  if (data.webhook_port !== undefined) {
    const p = parseInt(String(data.webhook_port || '').trim(), 10);
    data.webhook_port = (!isNaN(p) && p > 0 && p <= 65535) ? String(p) : '';
  }
  if (data.webhook_host !== undefined) {
    data.webhook_host = String(data.webhook_host || '').trim().replace(/^https?:\/\//i, '').replace(/\/.*$/, '');
  }
  const db = getDb();
  const fields = [
...
export async function createPillarCampaign(campaign) {
  ...
  const data = {
    ...
    visual_identity_preset_id: campaign.visual_identity_preset_id || null,
    visual_identity_preset_version: campaign.visual_identity_preset_version || null,
    enable_tts: campaign.enable_tts !== undefined ? campaign.enable_tts : 1,
    enable_glabs: campaign.enable_glabs !== undefined ? campaign.enable_glabs : 0,
    enable_ffmpeg: campaign.enable_ffmpeg !== undefined ? campaign.enable_ffmpeg : 1,
    ...
    clip_duration: campaign.clip_duration !== undefined ? Number(campaign.clip_duration) : 8,
    ...
    approval_mode: campaign.approval_mode || 'creative',
    auto_sync_contentflow: campaign.auto_sync_contentflow ? 1 : 0
  };
  return db.prepare(`
    INSERT INTO pillar_campaigns (
      ...
      product_filename_declare, visual_overrides_json, visual_identity_preset_id, visual_identity_preset_version, enable_tts, enable_ffmpeg, enable_social_post,
      post_facebook_draft, facebook_page_id, facebook_server_url,
      voice_provider, voice_persona, words_per_clip, clip_duration, enable_glabs, upload_markdown, upload_spreadsheet, target_spreadsheet_id, target_markdown_url,
      ...
      target_demographic, target_demographic_custom, approval_mode, auto_sync_contentflow
    ) VALUES (
      @id, @account_name, @source_planner_id, @campaign_name, @status, @execution_mode, @content_pillar, @custom_hook, @visual_action_guideline, @custom_instruction, @ai_directive, @mandatory_outro_line, @brand_profile_id,
      @narrative_mode, @visual_style, @face_visibility, @is_bridging_active, @target_clips_count, @bridge_at_clip, @bridge_duration_clips, @bridging_mode,
      @target_product_id, @ephemeral_product_data, @aspect_ratio, @target_ai, @video_model, @visual_mode, @product_ref_image_path,
...
export async function listPillarCampaigns() {
  ...
  return await dbAll(`
    SELECT c.*, b.brand_name,
      EXISTS(SELECT 1 FROM content_flow_items cf WHERE cf.source_campaign_id = c.id AND cf.source_type = 'opc' AND cf.tenant_id = c.tenant_id) as is_synced
    FROM pillar_campaigns c
    LEFT JOIN brand_profiles b ON c.brand_profile_id = b.id
    WHERE c.tenant_id = ?
    ORDER BY c.created_at DESC
...
export async function updatePillarCampaign(id, updates) {
  ...
  const allowed = [
    ...
    'enable_audio_segment', 'voice_cast_json', 'clip_duration', 'words_per_clip',
    'visual_identity_preset_id', 'visual_identity_preset_version'
  ];
  const fields = [];
  const values = [];
...
export async function createPillarCampaignItem(item) {
  const db = getDb();
  return db.prepare(`
    INSERT INTO pillar_campaign_items (campaign_id, row_creative_payload, generation_status, start_frame_status) VALUES (?, ?, ?, ?)
  `).run(
    item.campaign_id,
    ...
    item.start_frame_status !== undefined ? item.start_frame_status : 'pending'
  );
}
...
export async function updatePillarCampaignItem(id, updates) {
  ...
  const allowed = [
    ...
    'review_revision','approved_revision','approved_at','approved_by','contentflow_sync_status',
    'contentflow_sync_attempts','contentflow_synced_at','contentflow_item_id','contentflow_error',
    'review_state','review_reason','review_actor_id','review_state_updated_at',
    'generation_progress'
  ];
  const fields = [];
  const values = [];
...
export async function createSheetsCampaign(campaign) {
  ...
  const stmt = db.prepare(`
    INSERT INTO sheets_campaigns (
      ...
      custom_instruction, brand_profile_id, visual_overrides_json,
      visual_identity_preset_id, visual_identity_preset_version,
      is_bridging_active,
  ...
  return await stmt.run([
    ...
    campaign.visual_overrides_json || null,
    campaign.visual_identity_preset_id || null,
    campaign.visual_identity_preset_version || null,
    campaign.is_bridging_active || 0,
...
export async function listDeconstructAssets(filters, tenantId) {
  const { q, niche, status, page = 1, limit = 25 } = filters;
  const offset = (page - 1) * limit;

  let query = 'SELECT * FROM re_deconstructed_assets WHERE tenant_id = ?';
  ...
  const countQuery = `SELECT COUNT(*)::int as total FROM (${query}) AS sub`;
  ...
  const nichesRes = await dbAll(
    'SELECT DISTINCT niche FROM re_deconstructed_assets WHERE tenant_id = ? AND niche IS NOT NULL ORDER BY niche',
    [tenantId]
  );
  const niches = nichesRes.map(n => n.niche);
...
export async function getDeconstructAssetById(id, tenantId) {
  return await dbGet(
    'SELECT * FROM re_deconstructed_assets WHERE id = ? AND tenant_id = ?',
    [id, tenantId]
  );
}
...
export async function enqueueDeconstructAssets(assetIds, options, tenantId) {
  ...
  const targetRecommendationCount = Number(options.target_recommendation_count || 3);

  await withPgTransaction(async () => {
    await dbRun(
      'INSERT INTO re_deconstruct_batches (id, tenant_id, batch_name, target_recommendation_count, total_videos, processed_videos, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [batchId, tenantId, `Batch - ${new Date().toLocaleDateString('id-ID')}`, targetRecommendationCount, assetIds.length, 0, 'processing']
    );

    for (const assetId of assetIds) {
      await dbRun(
        `UPDATE re_deconstructed_assets 
         SET batch_id = ?, status = 'pending_download', queued_at = CURRENT_TIMESTAMP, error_message = NULL 
         WHERE id = ? AND tenant_id = ? AND (status = 'saved' OR status = 'failed')`,
        [batchId, assetId, tenantId]
      );
    }
  });

  // Trigger local campaign scheduler tick immediately to process the batch
  setTimeout(async () => {
    try {
...
export async function getNextPendingDeconstructAsset(batchId, tenantId) {
  return await dbGet(`
    ...
    WHERE a.status = 'pending_download' AND a.batch_id = ? AND a.tenant_id = ?
...
export async function updateDeconstructAsset(id, updates) {
  ...
  const values = [];
  const allowed = [
    'batch_id', 'source_url', 'original_caption', 'local_video_path',
    'gemini_file_uri', 'original_storyboard_json', 'product_ideas_json',
    'viral_pattern_summary', 'status', 'error_message', 'tags', 'niche',
    'queued_at', 'deconstructed_at', 'updated_at'
  ];
  for (const key of Object.keys(updates)) {
  ...
  if (fields.length === 0) return;
  values.push(id);
  return await dbRun(`UPDATE re_deconstructed_assets SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, values);
}

export async function updateDeconstructBatchProgress(batchId) {
  const tenantId = getActiveTenantId();
  const totalRes = await dbGet('SELECT COUNT(*)::int as cnt FROM re_deconstructed_assets WHERE batch_id = ? AND tenant_id = ?', [batchId, tenantId]);
  const processedRes = await dbGet("SELECT COUNT(*)::int as cnt FROM re_deconstructed_assets WHERE batch_id = ? AND tenant_id = ? AND status IN ('deconstructed', 'failed')", [batchId, tenantId]);

  const total = Number(totalRes?.cnt || 0);
  const processed = Number(processedRes?.cnt || 0);
  const newStatus = processed >= total ? 'completed' : 'processing';

  await dbRun('UPDATE re_deconstruct_batches SET processed_videos = ?, status = ? WHERE id = ? AND tenant_id = ?', [processed, newStatus, batchId, tenantId]);
...
export async function createDeconstructBatch(data) {
  const tenantId = getActiveTenantId();
  await dbRun(`
    INSERT INTO re_deconstruct_batches (id, tenant_id, batch_name, target_recommendation_count, total_videos, processed_videos, status)
    VALUES (?, ?, ?, ?, ?, 0, 'processing')
  `, [data.id, tenantId, data.batch_name, data.target_recommendation_count, data.total_videos]);
  return data.id;
}
...
export async function createDeconstructAsset(data) {
  const tenantId = getActiveTenantId();
  await dbRun(`
    INSERT INTO re_deconstructed_assets (id, tenant_id, batch_id, source_url, original_caption, status)
    VALUES (?, ?, ?, ?, ?, 'pending_download')
  `, [data.id, tenantId, data.batch_id, data.source_url, data.original_caption || null]);
}
...
export async function getDeconstructBatchById(id) {
  const tenantId = getActiveTenantId();
  const batch = await dbGet('SELECT * FROM re_deconstruct_batches WHERE id = ? AND tenant_id = ?', [id, tenantId]);
  if (!batch) return null;
  const assets = await dbAll('SELECT * FROM re_deconstructed_assets WHERE batch_id = ? AND tenant_id = ? ORDER BY created_at ASC', [id, tenantId]);
  return { ...batch, assets };
}

export async function deleteDeconstructBatch(id) {
  const tenantId = getActiveTenantId();
  await dbRun('DELETE FROM re_deconstructed_assets WHERE batch_id = ? AND tenant_id = ?', [id, tenantId]);
  await dbRun('DELETE FROM re_deconstruct_batches WHERE id = ? AND tenant_id = ?', [id, tenantId]);
}
...
export async function createMultiplierTask(data) {
  ...
  db.prepare(`
    INSERT INTO re_multiplier_tasks (
      id, tenant_id, deconstruct_asset_id, target_product_id, target_product_url, affiliate_url,
      vso_config_json, bridging_config_json, audio_config_json, status, enable_vo_audit,
      batch_id, row_index, product_snapshot_json
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    data.id,
    tenantId,
    ...
    data.batch_id || null,
    data.row_index !== undefined ? data.row_index : null,
    data.product_snapshot_json || null
  );
  return data.id;
}
...
export async function createMultiplierBatchWithTasks(batch, rows) {
  const tenantId = getActiveTenantId();
  const { withPgTransaction } = await import('./db-pg.js');
  return await withPgTransaction(async (client) => {
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      const taskId = row.id || `mtk_${crypto.randomUUID().replaceAll('-', '').slice(0, 16)}`;
      
      let productSnapshot = null;
      if (row.target_product_id) {
        const result = await client.query(
          `SELECT * FROM product_extractions WHERE id = $1 AND tenant_id = $2 LIMIT 1`,
          [row.target_product_id, tenantId]
        );
        ...
        if (prod) {
          productSnapshot = {
            ...
            product_usp: prod.unique_selling_point || '',
            product_url: prod.source_url || row.target_product_url || '',
            product_image_url: prod.clean_photo_url || prod.raw_photo_url || '',
            captured_at: new Date().toISOString()
          };
        }
      }
      
      if (!productSnapshot && row.target_product_url) {
        productSnapshot = {
          product_name: row.product_name || batch.shared_config?.bridging?.manualProductName || '',
          product_description: row.product_description || batch.shared_config?.bridging?.manualProductDesc || '',
          product_usp: row.unique_selling_point || batch.shared_config?.bridging?.manualProductUsp || '',
          product_url: row.target_product_url,
          captured_at: new Date().toISOString()
        };
      }

      const rowBridging = {
        ...(batch.shared_config?.bridging || {}),
        ...(row.bridging || {}),
        campaign_type: row.campaign_type || batch.shared_config?.bridging?.campaign_type || 'product_campaign',
        editorialTopic: row.editorial_topic || row.angle_title || batch.shared_config?.bridging?.editorialTopic || '',
        editorialOutro: row.editorial_outro || batch.shared_config?.bridging?.editorialOutro || '',
        angleTitle: row.angle_title || row.editorial_topic || '',
        hookType: row.hook_type || '',
        hookText: row.hook_text || '',
        targetPersona: row.target_persona || '',
        emotionalTrigger: row.emotional_trigger || '',
        ctaType: row.cta_type || '',
        nicheKbName: row.niche_kb_name || batch.shared_config?.bridging?.nicheKbName || ''
      };
      ...
      `, [
        taskId,
        tenantId,
        row.deconstruct_asset_id,
        row.target_product_id || null,
        row.target_product_url || null,
        row.affiliate_url || null,
        JSON.stringify(batch.shared_config?.vso || {}),
        JSON.stringify(rowBridging),
        JSON.stringify(batch.shared_config?.audio || {}),
        'pending_resolution',
        batch.enable_vo_audit !== undefined ? Number(batch.enable_vo_audit) : 1,
        batch.batch_id,
        i,
...
export async function getMultiplierTasks() {
  const tenantId = getActiveTenantId();
  return await dbAll(`
    SELECT t.*, a.source_url as asset_source_url, a.niche as asset_niche, a.original_caption as asset_caption, a.original_storyboard_json as original_storyboard_json,
      EXISTS(SELECT 1 FROM content_flow_items cf WHERE cf.source_campaign_id = t.id AND cf.source_type = 'instant') as is_synced
...
export async function deleteMultiplierTask(id) {
  const tenantId = getActiveTenantId();
  await dbRun('DELETE FROM re_multiplier_tasks WHERE id = ? AND tenant_id = ?', [id, tenantId]);
}
...
export async function updateMultiplierTask(id, updates) {
  ...
  if (fields.length === 0) return;
  values.push(id, tenantId);
  return await dbRun(`UPDATE re_multiplier_tasks SET ${fields.join(', ')} WHERE id = ? AND tenant_id = ?`, values);
}

export async function getNextPendingMultiplierTask() {
  return await dbGet(`
    SELECT * FROM re_multiplier_tasks
    WHERE status IN ('pending_resolution', 'resolving_product', 'remaking', 'generating_t2i', 'generating_audio', 'generating_visuals', 'ffmpeg_muxing')
    ORDER BY created_at ASC
    LIMIT 1
  `);
}

export async function getPendingMultiplierTasks(limit = 4) {
  return await dbAll(`
    SELECT * FROM re_multiplier_tasks
    WHERE status IN ('pending_resolution', 'resolving_product', 'remaking', 'generating_t2i', 'generating_audio', 'generating_visuals', 'ffmpeg_muxing')
    ORDER BY created_at ASC
...
export async function getRecipeCampaigns() {
  ...
  return db.prepare(`
    SELECT c.*,
      ...
      EXISTS(SELECT 1 FROM content_flow_items cf WHERE cf.source_campaign_id = c.id AND cf.source_type = 'recipe') as is_synced
...
export async function createUniverseProfile(data) {
  ...
  const fields = ['id', 'name', 'slug', 'premise', 'tone', 'knowledge_domain', 'human_presence',
    ...
    'negative_prompts_json', 'style_reference_path', 'status', 'version', 'universe_type', 'depiction_policy', 'historical_period'];
  const values = [
    ...
    data.universe_type || 'animal', data.depiction_policy || null, data.historical_period || null
  ];
  const placeholders = fields.map(() => '?').join(', ');
...
export async function createUniverseCharacter(data) {
  ...
  const values = [
    ...
    data.depiction_mode || 'normal', data.reference_type || 'identity', data.historical_period || null
  ];
  const placeholders = fields.map(() => '?').join(', ');
...
export async function createUniverseLocation(data) {
  ...
  const values = [
    ...
    data.historical_period || null, data.reference_type || 'location'
  ];
  const placeholders = fields.map(() => '?').join(', ');
