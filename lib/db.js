...
export function interceptQuery(sql, params) {
  ...
  const selectOrUpdateOrDeleteRegex = /^(SELECT|UPDATE|DELETE)\b/i;
  if (selectOrUpdateOrDeleteRegex.test(targetSql)) {
    const suffixRegex = /\b(ORDER\s+BY|LIMIT|GROUP\s+BY|HAVING|OFFSET|UNION|JOIN)\b/i;
    ...
    targetSql = queryPart + suffixPart;
    targetParams.push(tenantId);
  }
...
export async function createProductExtraction(data) {
  ...
  return await stmt.run([safeData]);
}

export async function getProductExtraction(id) {
  const db = getDb();
  return await dbGet('SELECT * FROM product_extractions WHERE id = ?', [id]);
}

export async function getAllProductExtractions() {
  const db = getDb();
  return await dbAll('SELECT * FROM product_extractions ORDER BY created_at DESC', []);
}
...
export async function deleteProductExtraction(id) {
  ...
  await dbRun('DELETE FROM pipeline_assets WHERE product_id = ?', [id]);
  return await dbRun('DELETE FROM product_extractions WHERE id = ?', [id]);
}
  ...
  for (const item of keysArray) {
    try {
      const res = await pgQuery(
        'INSERT INTO gemini_api_keys (key_name, api_key, tier, daily_limit) VALUES ($1, $2, $3, $4) ON CONFLICT (api_key) DO NOTHING',
        [item.key_name, item.api_key, item.tier || 'FREE', item.daily_limit || 20]
      );
      if (res.rowCount > 0) {
        addedCount++;
      } else {
...
export async function getAllBrandProfiles() {
  const db = getDb();
  return await dbAll('SELECT id, brand_name, tone_of_voice, visual_signature, guideline_filename, created_at, storage_provider, webhook_host FROM brand_profiles ORDER BY created_at DESC', []);
}
...
export async function getContentFlowItemById(id) {
  const db = getDb();
  return await dbGet(`SELECT * FROM content_flow_items WHERE id = ?`, [id]);
}

export async function deleteContentFlowItem(id) {
  const db = getDb();
  return await dbRun('DELETE FROM content_flow_items WHERE id = ?', [id]).changes;
}

export async function deleteContentFlowBrandItems(accountName) {
  const db = getDb();
  return await dbRun('DELETE FROM content_flow_items WHERE account_name = ?', [accountName]).changes;
}
