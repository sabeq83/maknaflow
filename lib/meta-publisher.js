...
export async function fetchMetaPostDetails({ token, platform, externalPostId }) {
  ...
  if (platform === 'facebook') {
    const res = await fetch(`${graphUrl}/${cleanId}?fields=permalink_url,published,is_published,created_time,description,id&access_token=${encodeURIComponent(token)}`);
    const data = await res.json();
    if (data.error) throw new Error(data.error.message || 'Gagal mengambil data postingan Facebook dari Graph API');

    const isPublished = data.published !== false && data.is_published !== false;
