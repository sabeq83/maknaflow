...
export function normalizeHermesResearchBrief(input, policy = {}) {
  ...
  return Object.freeze({
    ...
    sources: sources.map(s => ({
      ...
      retrieved_at: s.retrieved_at ? new Date(s.retrieved_at).toISOString() : new Date().toISOString()
    })),
    ...
    prohibited_claims: (input.prohibited_claims || []).map(String).map(s => s.trim()).filter(Boolean),
    limitations: (input.limitations || []).map(String).map(s => s.trim()).filter(Boolean)
  });
}
