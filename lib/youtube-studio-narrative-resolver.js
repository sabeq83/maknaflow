...
export function resolveNarrativeConfiguration({ channelStrategy, series, episode, audioConfig = null, registrySpeakers = null }) {
  ...
  // Registry-backed cast is authoritative. Legacy JSON cast is read only when
  // no active Channel Audio configuration exists.
  const seriesCast = series?.config_json?.recurring_cast || [];
  const episodeCast = episode?.narrative_config_json?.episode_cast || [];

  const registryBacked = Boolean(audioConfig);
  const mergedSpeakers = registryBacked ? [...(registrySpeakers || [])] : [...episodeCast];
  if (!registryBacked) {
    seriesCast.forEach(sc => {
      if (!mergedSpeakers.some(es => es.speaker_id === sc.speaker_id)) mergedSpeakers.push({ ...sc, source: 'series' });
    });
  }

  // Ensure default narrator is present
  if (!registryBacked && !mergedSpeakers.some(s => s.speaker_id === 'narrator')) {
  ...
  return {
    ...
    audio_production_mode: audioConfig?.audio_production_mode || 'standalone_tts',
    audio_experience: audioConfig?.audio_experience || 'narrative_dialogue',
    audio_config_id: audioConfig?.id || null,
    audio_config_version: audioConfig?.version || null,
    audio_provider: audioConfig?.provider || null,
    audio_model_key: audioConfig?.model_key || null,
    native_voice_capability: audioConfig?.native_voice_capability || null,
    sonic_identity_snapshot: audioConfig?.sonic_identity_json || null,
...
export function getSpeakerVoice(speakerId, resolvedNarrative, episode) {
  ...
  if (speaker) {
    if (resolvedNarrative.audio_production_mode === 'standalone_tts' && speaker.tts_binding) {
      return {
        ...
        speed: speaker.tts_binding.speed !== undefined ? Number(speaker.tts_binding.speed) : 1.0
      };
    }
  ...
  if (resolvedNarrative?.registry_backed) {
    const error = new Error(`No active voice binding found for registered speaker "${speakerId}".`);
