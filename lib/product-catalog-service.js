...
function imageUrl(row){const value=row.clean_photo_url||row.raw_photo_url||row.generated_photo_url||row.photo_url||null;return value&&!/^https?:\/\//.test(value)?`/api/v2/products/image?path=${encodeURIComponent(value)}`:value;}
