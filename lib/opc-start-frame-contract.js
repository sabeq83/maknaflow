...
export function resolveProductReferenceRequirement({ campaign, item, clipIndex }) {
  ...
  const targetClips = positiveInteger(campaign?.target_clips_count, 4);
  const bridgeStart = positiveInteger(campaign?.bridge_at_clip, 2);
  const rawBridgeDuration = parseNonNegativeInteger(campaign?.bridge_duration_clips, 1);
  const bridgeEnd = rawBridgeDuration > 0 ? (bridgeStart + rawBridgeDuration - 1) : targetClips;
