...
import { getGraphBaseUrl, createFacebookDraft } from './meta-publisher.js';

const FB_GRAPH_URL = getGraphBaseUrl();
 ...
 */
export async function getConnectedFacebookPages(token, manualIds = null) {
  ...
  try {
    ...
    const res = await fetch(`${FB_GRAPH_URL}/me/accounts?fields=id,name,category,access_token,tasks,instagram_business_account{id,username}&access_token=${encodeURIComponent(cleanToken)}`);
    const data = await res.json();
  ...
  // 2. Muat detail Halaman secara manual dari parameter atau pengaturan (Bypass Batasan Meta Dev Mode / Portfolio)
  let rawManualIds = [];
  if (Array.isArray(manualIds)) {
    rawManualIds = manualIds;
  } else if (typeof manualIds === 'string' && manualIds.trim()) {
    rawManualIds = manualIds.split(',');
  } else {
    const manualPageIdsStr = (getSetting('fb_page_ids') || getSetting('fb_page_id') || '').trim();
    if (manualPageIdsStr) rawManualIds = manualPageIdsStr.split(',');
  }

  const ids = rawManualIds.map(id => String(id).trim()).filter(Boolean);
  if (ids.length > 0) {
    console.log(`[Facebook Helper] Fetching manual page details for: ${ids.join(', ')}`);
    for (const pageId of ids) {
      if (!seenIds.has(pageId)) {
        try {
          const pageRes = await fetch(`${FB_GRAPH_URL}/${pageId}?fields=id,name,category,access_token,instagram_business_account{id,username}&access_token=${encodeURIComponent(cleanToken)}`);
          ...
          if (pageData && !pageData.error) {
            ...
            pagesList.push({
              ...
              instagram: pageData.instagram_business_account ? {
  ...
  if (pagesList.length === 0) {
    try {
      ...
      const pageRes = await fetch(`${FB_GRAPH_URL}/me?fields=id,name,category,access_token&access_token=${encodeURIComponent(cleanToken)}`);
      const pageData = await pageRes.json();
      if (pageData && !pageData.error) {
        pagesList.push({
          id: pageData.id,
...
export async function postDraftToFacebookPage({ message, mediaUrl, mediaType = 'text_only', pageId = null, pageToken = null }) {
  ...
  try {
    const result = await createFacebookDraft({
    ...
    console.log(`[Facebook Draft] Successfully created draft post #${result.postId}`);
    return { success: true, fb_post_id: result.postId };
  } catch (error) {
    console.error('[Facebook API/Network Error]:', error.message);
    return { success: false, error: error.message };
  }
}
