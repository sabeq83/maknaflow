...
export async function buildOperatorJobStatus(jobOrId) {
  ...
  const db = getDb();
  const items = await db.prepare('SELECT * FROM pillar_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(job.campaign_id);
  return {
    ...job,
    progress: {
...
export async function processOperatorContentJob(job) {
  ...
  try {
    ...
    if (!campaignId) {
      ...
      const selectedRowIds = request.selection?.mode === 'row_ids' ? request.selection.row_ids : [];
...
export async function reconcileActiveOperatorJobs() {
  ...
  for (const job of jobs) {
    const items = job.campaign_id
      ? await db.prepare('SELECT * FROM pillar_campaign_items WHERE campaign_id = ? ORDER BY id ASC').all(job.campaign_id)
      : [];
