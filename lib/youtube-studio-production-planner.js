...
export async function generateProductionPlan({ episode, script, profile, visualIdentity, universe }) {
  ...
  const prompt = `
    ...
    Setiap scene naratif harus dipecah menjadi satu atau beberapa visual shots untuk menutup durasi voiceover/audio blocks scene tersebut.
    ...
    Script (Perhatikan apakah ini Script v2 dengan array "audio_blocks" berisi giliran bicara karakter/speaker):
    ${JSON.stringify(script.script_json || script)}

    Ketentuan Pembuatan Rencana Visual:
    ...
    5. Untuk "generated_visual" shot, "prompt" WAJIB menggunakan satu baris tunggal (single line) tanpa newline dengan template:
    ...
    6. Daftarkan karakter yang aktif/berbicara pada shot tersebut di field "subjects" menggunakan key unik dari universe (misal: "cat_detective", "broom_sweeper", dst.) jika terdaftar di Universe Context.
    7. Pemetaan Fokus Speaker/Listener (Speaker-Aware Layout): 
       - Analisis giliran bicara (turns) dalam scene. Rancang visual shots agar bergantian fokus: tunjukkan karakter yang sedang berbicara (speaker focus) atau tunjukkan reaksi karakter lain yang sedang mendengarkan (listener focus / reaction shot) untuk menjaga ritme percakapan.
          ...
          "shots": [
            {
              ...
              "prompt": "prompt visual shot sesuai template di atas",
            ...
            },
            {
              ...
              "subjects": ["nama_key_karakter"],
              "generation_duration_seconds": 8,
              "prompt": "prompt visual shot kedua sesuai template di atas",
              "transition_to_next": "cut"
            }
          ],
    ...
    parsed.scenes.forEach((scene, scIdx) => {
      ...
      if (Array.isArray(scene.shots)) {
        scene.shots.forEach((shot, shIdx) => {
          ...
          if (shot.asset_type === 'generated_visual' && shot.generation_duration_seconds !== undefined) {
            shot.generation_duration_seconds = roundToNearestAllowed(shot.generation_duration_seconds, allowedDurations);
          }
        });
      }
    });
