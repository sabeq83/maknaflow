...
export async function generateProductionPlan({ episode, script, profile, visualIdentity, universe }) {
  ...
  const prompt = `
    ...
    ${JSON.stringify(script.script_json)}
     ...
     6. Daftarkan karakter yang aktif pada shot tersebut di field "subjects" menggunakan key unik dari universe (misal: "cat_detective", "broom_sweeper", dst.) jika terdaftar di Universe Context.
