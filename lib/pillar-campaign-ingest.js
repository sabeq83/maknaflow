...
import { resolveAndValidateOpcProductLineage } from './opc-product-lineage.js';
import { resolveVisualIdentitySubmission } from './visual-override-resolver.js';
import { getDefaultWordsPerClip } from './words-per-clip-presets.js';
...
export async function ingestPlannerToPillarCampaign({
  ...
  let refImage = globalSettings.product_ref_image_path || (!isEditorial && !isRecipeCampaign ? (planner.product_ref_image || planner.product_photo_url) : null) || null;
  let targetProdId = explicitProductId || (!isEditorial && !isRecipeCampaign ? planner.product_id : null) || null;
  const brandProfileId = globalSettings.brand_profile_id || planner.brand_id || null;

  try {
    let product = null;
    if (!isEditorial && !isRecipeCampaign) {
      const lineage = await resolveAndValidateOpcProductLineage({ planner, explicitProductId, brandProfileId });
      product = lineage.product;
      targetProdId = lineage.productId;
    } else if (targetProdId) {
      product = await db.prepare('SELECT * FROM product_extractions WHERE id = ? AND tenant_id = ?').get(targetProdId, planner.tenant_id);
    }
    if (product) {
      if (!targetProdId) targetProdId = product.id;
      const { resolveActiveProductPhotoPath } = await import('./product-reference-resolver.js');
      refImage = resolveActiveProductPhotoPath(product, [refImage]);
    }
  } catch (error) {
    if (!isEditorial && !isRecipeCampaign) throw error;
  }
  ...
  if (isCartoonUniverse) {
    ...
    universeSnapshot = {
      ...defaults,
      universe_type: defaults.universe_type || 'animal',
      depiction_policy: defaults.depiction_policy || null,
      historical_period: defaults.historical_period || null,
  ...
  const identityResult = await resolveVisualIdentitySubmission({
    preset_id: globalSettings.visual_identity_preset_id || null,
    inline_config: globalSettings.visual_identity_inline_config || null,
    legacy_overrides_json: globalSettings.visual_overrides_json || null
  });
  ...
  const campaignData = {
    ...
    campaign_focus: planner.planner_focus || null,
    content_pillar: isRecipeCampaign ? 'Recipe Campaign' : (rows[0]?.pillar || 'Planner Content Pillar'),
    custom_hook: rows[0]?.hook || 'Planner Hook',
    visual_action_guideline: rows[0]?.visual_action || 'Planner Visual Action',
    custom_instruction: globalSettings.custom_instruction || planner.custom_instructions || '',
    ai_directive: globalSettings.ai_directive || globalSettings.custom_instruction || (isEditorial ? planner.brand_context : (isRecipeCampaign ? (planner.promotion_context || '') : planner.product_description)) || '',
    mandatory_outro_line: globalSettings.mandatory_outro_line || '',
    brand_profile_id: brandProfileId,
    target_demographic: globalSettings.target_demographic || planner.target_audience || null,
    target_demographic_custom: globalSettings.target_demographic_custom || null,
    narrative_mode: globalSettings.narrative_mode || (isRecipeCampaign ? 'Storytelling' : 'auto'),
    visual_style: globalSettings.visual_style || (isRecipeCampaign ? 'photorealistic_culinary' : 'Cinematic'),
    face_visibility: globalSettings.face_visibility || 'Faceless',
    is_bridging_active: (isEditorial || isRecipeCampaign) ? (bridgingRequested ? 1 : 0) : (globalSettings.is_bridging_active !== undefined ? (globalSettings.is_bridging_active ? 1 : 0) : 1),
    // Universe metadata forwarding to campaign level (Tahap 2)
    content_world: planner.content_world || 'real_world',
    knowledge_domain: isRecipeCampaign ? 'food_culinary' : null,
    ...
    words_per_clip: globalSettings.words_per_clip || (globalSettings.clip_duration ? getDefaultWordsPerClip(globalSettings.clip_duration) : '17-19 kata'),
    clip_duration: Number(globalSettings.clip_duration || 8),
    ...
    approval_mode: globalSettings.approval_mode || (globalSettings.execution_mode === 'full_autopilot' ? 'none' : 'creative'),
    auto_sync_contentflow: globalSettings.auto_sync_contentflow ? 1 : 0,
  ...
  const campaignItems = rows.map((row, index) => {
      const hasUrl = !isEditorial && !isRecipeCampaign && Boolean(planner.product_url?.trim());
      ...
      const narrativeMode = !globalSettings.narrative_mode || globalSettings.narrative_mode === 'auto'
        ? (isRecipeCampaign ? 'Storytelling' : autoDetectNarrativeMode(row))
      ...
      if (isRecipeCampaign) {
        ...
        const creativePayload = {
          ...
          content_subject: recipeIdea.title || row.title || row.context || '',
          category: recipeIdea.category || row.category_cep || 'minuman',
          hook: recipeIdea.hook || row.hook,
          custom_hook: recipeIdea.hook || row.hook || '',
          visual_action_guideline: recipeIdea.visual_highlight || row.visual_action || '',
          ...
          product_name: recipeIdea.primary_product_name || row.product || row.product_reference || null,
          ...
          target_audience: globalSettings.target_demographic || planner.target_audience || 'genz_casual',
          promotion_context: planner.promotion_context || '',
          custom_instructions: globalSettings.custom_instruction || planner.custom_instructions || '',
          visual_mode: (globalSettings.visual_mode || 'hybrid_lock'),
        ...
        return {
          ...
          generation_status: 'pending',
          start_frame_status: (globalSettings.visual_mode || 'hybrid_lock') === 'hybrid_lock' ? 'pending' : 'skipped',
  ...
  try {
    ...
    console.error('[OPC Ingest Binding Warning]: Failed to create bindings:', bindErr.message);
    if (!isEditorial && !isRecipeCampaign) {
      throw new PillarCampaignIngestError(bindErr.message, bindErr.status || 422, bindErr.code || 'OPC_PRODUCT_BINDING_UNAVAILABLE');
    }
  }
