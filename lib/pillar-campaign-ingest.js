import { createPillarCampaign, createPillarCampaignItem, getDb } from './db.js';
...
export async function ingestPlannerToPillarCampaign({
  ...
  if (existingCampaign) {
    ...
    return {
      ...
      ingestedCount: Number(existingItems?.count || 0),
  ...
  await createPillarCampaign({
    ...
    custom_instruction: globalSettings.custom_instruction || (isEditorial ? planner.brand_context : planner.product_description) || '',
    ...
    target_clips_count: globalSettings.target_clips_count ? Number(globalSettings.target_clips_count) : 4,
    bridge_at_clip: globalSettings.bridge_at_clip ? Number(globalSettings.bridge_at_clip) : 2,
    bridge_duration_clips: globalSettings.bridge_duration_clips !== undefined ? Number(globalSettings.bridge_duration_clips) : 1,
    ...
    scheduler_pause_at: globalSettings.scheduler_pause_at || null,
  ...
  await db.transaction(async () => {
    for (let index = 0; index < rows.length; index++) {
      ...
      await createPillarCampaignItem({ campaign_id: campaignId, row_creative_payload: JSON.stringify(payload), generation_status: generationStatus });
    }
  });
