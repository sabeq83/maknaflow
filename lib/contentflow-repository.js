...
export const CONTENT_FLOW_UPDATE_FIELDS = new Set([
  ...
  'recipe_text_markdown', 'recipe_text_plain', 'recipe_payload_json', 'social_media_package_json', 'content_kind', 'recipe_revision'
]);
...
const SOCIAL_STATUS_FIELDS = new Set([
  'tiktok_status', 'facebook_status', 'instagram_status', 
  'youtube_status', 'threads_status', 'linkedin_status'
]);

const SOCIAL_METADATA_FIELDS = new Set([
  'tiktok_publish_date', 'permalink_tiktok',
  'facebook_publish_date', 'permalink_facebook',
  'instagram_publish_date', 'permalink_instagram',
  'youtube_publish_date', 'permalink_youtube',
  'threads_publish_date', 'permalink_threads',
  'linkedin_publish_date', 'permalink_linkedin'
]);
...
export async function upsertContentFlowItem(item) {
  ...
  const updates = [
    ...mutable.map(field => {
      if (SOCIAL_STATUS_FIELDS.has(field)) {
        // Jangan timpa status jika status baru bernilai 'Not Published' sedangkan di DB sudah 'Published' / 'Scheduled'
        return `${field} = CASE 
          WHEN EXCLUDED.${field} IS NOT NULL AND EXCLUDED.${field} NOT IN ('Not Published', '') 
          THEN EXCLUDED.${field} 
          ELSE content_flow_items.${field} 
        END`;
      }
      if (SOCIAL_METADATA_FIELDS.has(field)) {
        // Pertahankan tanggal publish & permalink yang sudah ada di database jika input baru kosong
        return `${field} = COALESCE(NULLIF(EXCLUDED.${field}, ''), content_flow_items.${field})`;
      }
...
export async function getContentFlowItemByVideoId(videoId) {
  if (!videoId) return null;
  return (await pgQuery('SELECT * FROM content_flow_items WHERE (video_id=$1 OR id=$1) AND tenant_id=$2 LIMIT 1', [videoId, operationalTenant()])).rows[0] || null;
}
...
/**
 * Fetch satu item by row ID, dengan tenant isolation.
 * Digunakan oleh Detail modal untuk membaca status terbaru dari DB secara realtime.
 */
export async function getContentFlowItemById(tenantId, id) {
  return (await pgQuery(
    'SELECT * FROM content_flow_items WHERE id=$1 AND tenant_id=$2 LIMIT 1',
    [id, tenantId]
  )).rows[0] || null;
}
