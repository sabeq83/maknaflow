...
const CONTENT_FLOW_INSERT_FIELDS = [
  ...
  'permalink_youtube', 'catatan', 'created_at', 'updated_at'
];
...
export async function listContentFlowItems(filters = {}) {
  ...
  const accounts = (await pgQuery(
    `SELECT DISTINCT account_name FROM content_flow_items WHERE ${facetScope} AND account_name IS NOT NULL AND account_name <> '' ORDER BY account_name`,
    facetParams
...
export async function upsertContentFlowItem(item) {
  ...
  const updates = mutable.map(field => `${field}=EXCLUDED.${field}`).join(', ');
  return (await pgQuery(
    `INSERT INTO content_flow_items (${columns.join(', ')}) VALUES (${placeholders.join(', ')}) ON CONFLICT (id) DO UPDATE SET ${updates} WHERE content_flow_items.tenant_id=EXCLUDED.tenant_id RETURNING *`,
    values
  )).rows[0] || null;
}
