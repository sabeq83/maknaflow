...
export async function listBrandProducts({ brandProfileId, includeUnlinked = false, search = '' }) {
  ...
  if (includeUnlinked) {
    query = `
      SELECT p.id AS product_id, p.product_name, p.product_description, p.unique_selling_point, p.target_audience,
             p.source_url, p.category, p.raw_photo_url, p.clean_photo_url, p.packaging_type, p.packaging_status,
             bp.id AS brand_product_id, bp.affiliate_link, bp.tracking_code, bp.landing_page_url,
             bp.product_name_override, bp.cta_override, bp.notes, bp.is_active
