...
import { normalizeHermesResearchBrief } from './hermes-research-contract.js';
import { normalizePublishingPolicy, normalizeResearchRequest } from './agent-automation-contract.js';
import { ALL_WORDS_PER_CLIP_VALUES } from './words-per-clip-presets.js';
...
function normalizeSelection(input = {}) {
  ...
  if (!['all', 'row_ids', 'best_one'].includes(mode)) {
    throw new OperatorContractError('selection.mode hanya boleh all, row_ids, atau best_one.');
  }
...
const ENUMS = {
  ...
  words_per_clip: ALL_WORDS_PER_CLIP_VALUES,
...
export function normalizeOperatorContentRequest(input) {
  ...
  const hasResearch = Boolean(input.research_brief);
  const researchRequest = input.research ? normalizeResearchRequest(input.research) : null;
  const publishingPolicy = normalizePublishingPolicy(input.publishing || input.publishing_policy || { mode: 'draft_only' });
  return {
    contract_version: hasResearch ? '3' : (normalizedOpc ? '2' : '1'),
    planner: normalizePlanner(input.planner || {}),
    selection: normalizeSelection(input.selection || {}),
    research_brief: hasResearch ? normalizeHermesResearchBrief(input.research_brief, input.research_policy) : null,
