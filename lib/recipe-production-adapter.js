...
export async function processRecipeCampaignCreative({ item, campaign, job }) {
  ...
  // Kategori-Aware Cookware & Toolset Kit Lock (V10 Culinary Continuity Standard)
  const getCookwareKit = (cat, title) => {
    const lower = ((cat || '') + ' ' + (title || '')).toLowerCase();
    if (/kue|cake|brownies|bolu|muffin|pie|tart|cookies|baking/i.test(lower)) {
      return {
        mixing_bowl: 'matte off-white ceramic mixing bowl with a rounded pouring spout',
        utensil: 'natural beechwood silicone spatula and whisk with wooden handle',
        pan_bakeware: '8x8 inch square matte charcoal non-stick baking pan lined with unbleached parchment paper',
        plating_vessel: 'minimalist matte cream ceramic dessert plate',
        category_label: 'Baking / Pastry'
      };
    }
    if (/minuman|drink|latte|kopi|coffee|tea|smoothie|juice|jus|mocktail|shake/i.test(lower)) {
      return {
        mixing_bowl: 'heatproof clear glass measuring beaker with measurement marks',
        utensil: 'long-handled gold cocktail spoon and stainless milk frother',
        pan_bakeware: 'small cast-iron warming pan or transparent blender jar',
        plating_vessel: 'clear fluted cylindrical highball glass with transparent glass straw',
        category_label: 'Beverage / Drink'
      };
    }
    return {
      mixing_bowl: 'stainless steel prep bowls with non-slip silicone base',
      utensil: 'matte dark-grey heat-resistant silicone tongs and beechwood turner',
      pan_bakeware: '28cm matte black cast-aluminum skillet with light oak wooden handle',
      plating_vessel: 'wide shallow Nordic stoneware serving bowl in warm terracotta tones',
      category_label: 'Culinary / Savory'
    };
  };

  const cookwareKit = getCookwareKit(category, recipeTitle);
  ...
  const visualMandate = [
    ...
    ``,
    `STRICT CULINARY CONTINUITY & COOKWARE TRUTH LOCK (CRITICAL):`,
    `- FOOD SUBJECT TRUTH: Makanan yang dibuat di SELURUH klip adalah 100% konsisten dengan judul resep: "${recipeTitle}". DILARANG KERAS memunculkan objek makanan lain yang tidak ada di resep (CONTOH KESALAHAN FATAL: Memasukkan croissant/roti ke dalam oven saat resep brownies fudgy!).`,
    `- STEP-BY-STEP FOOD TRANSFORMATION: Status adonan/makanan wajib mengikuti alur logis (Bahan Mentah -> Pencampuran Adonan Kental -> Dituang ke ${cookwareKit.pan_bakeware} -> Dipanggang/Dimasak dalam wadah yang SAMA -> Hasil Matang dipotong/disajikan).`,
    `- LOCKED COOKWARE PALETTE (Wajib 100% konsisten di semua klip, DILARANG mengganti warna/material wadah di tengah video):`,
    `  * Mixing Bowl: ${cookwareKit.mixing_bowl}`,
    `  * Utensil: ${cookwareKit.utensil}`,
    `  * Pan / Bakeware / Oven Vessel: ${cookwareKit.pan_bakeware}`,
    `  * Serving / Plating: ${cookwareKit.plating_vessel}`,
    `- INDONESIAN VISUAL ACTION (visual_action): Deskripsi aksi visual bahasa Indonesia WAJIB menyebutkan wadah terkunci dan status fisik adonan secara konsisten dan runtut (contoh: "Tangan menuangkan cokelat leleh ke dalam baskom keramik putih doff...", "Tangan menuangkan adonan kental dari baskom keramik putih doff ke loyang kotak hitam 20x20 cm beralas baking paper...", "Loyang kotak hitam berisi adonan brownies dimasukkan ke dalam oven...", "Brownies matang berkerak shiny dipotong di dalam loyang kotak hitam yang sama..."). DILARANG memunculkan wadah atau makanan yang melompat/inkonsisten.`
  ].join('\n');
  ...
  const systemInstruction = [
    ...
    `   - Each scene MUST contain: index, scene_function ("hook"|"cooking_step"|"product_spotlight"|"cta"), duration_seconds (${clipDuration}), visual_action (detailed Indonesian camera & action breakdown focusing on hands, locked cookware, and food transformation), on_screen_text (short overlay graphic / measurement text), voice_over (natural ${targetLanguage}, max ${wordsPerClip}), sfx_prompt (culinary ambient sounds), t2i_prompt (English 4-Layer Optics System), i2v_prompt (English 3-Layer Micro-Pacing System), recipe_step_indices, product_ids.`,
    `   - Cookware & Food Continuity: 100% unified cookware (${cookwareKit.mixing_bowl}, ${cookwareKit.pan_bakeware}) and strictly "${recipeTitle}" food state across all scenes.`,
    `   - MANDATORY T2I FORMAT (100% English): (VERTICAL 9:16) --ar 9:16 --no landscape, croissant, unrelated food [LAYER 1: OPTICS] (Shot on Phase One XF IQ4 (150MP), 100mm Macro Food Photography lens, soft appetizing shallow depth of field). [LAYER 2: SUBJECT, COOKWARE & VISUAL TRUTH] (Anchor: ${subjectAnchor}), (Wardrobe: ${wardrobeLock}), (Food Truth: [Current state of ${recipeTitle}]), (Cookware Truth: [Vessel from locked cookware palette]), (Product Truth: High fidelity photography of registered products, label geometry locked). [LAYER 3: SCENE & LIGHT] (Environment: ${environmentLock}), (Lighting: ${lightingLock}). [LAYER 4: KINETIC IMPLICATION] (Frozen Action: [Mid-action frozen motion]).`,
  ...
  promptLines.push(
    ...
    `    "t2i_prompt": "(VERTICAL 9:16) --ar 9:16 --no landscape, croissant, unrelated food [LAYER 1: OPTICS] (Shot on Phase One XF IQ4 (150MP), 100mm Macro Food Photography lens, soft appetizing shallow depth of field). [LAYER 2: SUBJECT, COOKWARE & VISUAL TRUTH] (Anchor: ${subjectAnchor}), (Wardrobe: ${wardrobeLock}), (Food Truth: [Current state of ${recipeTitle}]), (Cookware Truth: [Vessel from locked cookware palette]), (Product Truth: High fidelity photography of source product). [LAYER 3: SCENE & LIGHT] (Environment: ${environmentLock}), (Lighting: ${lightingLock}). [LAYER 4: KINETIC IMPLICATION] (Frozen Action: [Mid-action kinetic movement]).",`,
