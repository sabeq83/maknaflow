...
export async function processRecipeCampaignCreative({ item, campaign, job }) {
  ...
  // Build scene name labels for the prompt based on clip count
  const buildSceneNames = (n) => {
    ...
    // For larger counts: Hook + (n-2) cooking/detail steps + CTA
...
2. EXACTLY ${targetClips} video scenes (${buildSceneNames(targetClips)}):
   ...
   - IMPORTANT: Output MUST contain exactly ${targetClips} scene objects in the scenes array.
...
JSON OUTPUT SCHEMA:
{
  ...
  // Map scenes → new_video_plan_json (unified clip format used by UI & TTS pipeline)
