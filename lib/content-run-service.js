...
export async function buildBoundedContentRunStatus(runId, tenantId) {
  ...
  const hasInProgressItems = opItems.length > 0 && opItems.some(i =>
    ['pending', 'processing', 'start_frames_processing'].includes(i.workflow_status) ||
    ['pending', 'processing'].includes(i.generation_status)
  );

  if (opJob && publicStatus !== 'failed') {
    if (opJob.current_stage === 'syncing_contentflow') {
      publicStatus = 'syncing_contentflow';
      stage = 'contentflow';
    } else if (hasInProgressItems || opJob.current_stage === 'generating_start_frames' || opJob.status === 'producing') {
      publicStatus = 'generating_start_frames';
      stage = 'start_frames';
    } else if (opJob.status === 'awaiting_approval' || (opItems.length > 0 && opItems.every(i => i.workflow_status === 'ready_for_review' || i.review_state === 'ready' || i.generation_status === 'completed'))) {
      publicStatus = 'awaiting_manual_review';
      stage = 'start_frames';
      actionRequired = 'Review start frames in MAKNA';
    } else if (opJob.status === 'campaign_queued') {
