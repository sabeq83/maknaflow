...
export const KNOWLEDGE_DOMAINS = [
  'general', 'pet_supplies', 'food_culinary',
  'history', 'islamic_history', 'kitchen', 'home_improvement', 'herbal'
];
...
export const DEPICTION_MODES = ['normal', 'faceless', 'back_view', 'silhouette', 'environment_only'];
export const CHARACTER_REFERENCE_TYPES = ['identity', 'wardrobe'];
export const LOCATION_REFERENCE_TYPES = ['location', 'visual_style'];

export const DOMAIN_STORY_TEMPLATE_MAP = {
  'general': 'problem_solution_7beat',
  'pet_supplies': 'pet_problem_solution_7beat',
  'food_culinary': 'problem_solution_7beat',
  'history': 'historical_explainer_7beat',
  'islamic_history': 'historical_explainer_7beat',
  'kitchen': 'problem_solution_7beat',
  'home_improvement': 'problem_solution_7beat',
  'herbal': 'educational_discovery_7beat',
};

export const DOMAIN_PROMPT_RULES = {
  'herbal': {
    forbidden_claims: ['menyembuhkan', 'mengobati', 'menghilangkan penyakit', 'terbukti secara medis'],
    allowed_content: ['edukasi bahan', 'tradisi penggunaan', 'karakteristik tanaman'],
    safety_note: 'Hindari klaim medis. Gunakan framing edukatif dan tradisional.'
  },
  'islamic_history': {
    forbidden_content: ['visualisasi Nabi dan para nabi', 'dialog tanpa sumber', 'anakronisme'],
    required_compliance: 'depiction_policy',
    safety_note: 'Selalu patuhi depiction_policy universe. Jangan membuat representasi Nabi Muhammad ﷺ.'
  },
  'history': {
    forbidden_content: ['anakronisme pakaian, arsitektur, teknologi'],
    allowed_content: ['tokoh historis umum', 'lokasi bersejarah', 'peristiwa sejarah umum'],
  }
};
...
export function getRecommendedStoryTemplate(domain) {
  return DOMAIN_STORY_TEMPLATE_MAP[domain] || 'problem_solution_7beat';
}

export function getDomainPromptRules(domain) {
  return DOMAIN_PROMPT_RULES[domain] || null;
}

export function validateUniverseType(value) {
  const type = value || 'animal';
  return UNIVERSE_TYPES.includes(type) ? type : 'animal';
}

export function validateDepictionMode(value) {
  const mode = value || 'normal';
  return DEPICTION_MODES.includes(mode) ? mode : 'normal';
}
