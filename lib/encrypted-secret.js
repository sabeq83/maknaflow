...
function key(){const source=process.env.MAKNA_SECRET_ENCRYPTION_KEY||process.env.SESSION_SECRET;if(!source)throw new Error('MAKNA_SECRET_ENCRYPTION_KEY wajib diatur untuk menyimpan credential notifikasi.');return crypto.createHash('sha256').update(source).digest();}
