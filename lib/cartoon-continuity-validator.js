...
export function validateCartoonContinuity(parsedOutput, universeConfig = {}, rowPayload = {}) {
  ...
  if (universeType !== 'human') {
    ...
    // For human-type universes, check for depiction policy compliance instead
    const depictionPolicy = universeSnapshot?.depiction_policy;
    if (depictionPolicy) {
      checks.depiction_policy = {
        passed: true, // We can't auto-validate text against policy, so mark as info
        detail: 'Human universe: depiction_policy tersedia. Verifikasi manual diperlukan.',
        severity: 'info'
      };
    }
  }
