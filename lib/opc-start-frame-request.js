...
export async function buildOpcStartFrameRequest({ campaign, item, clipIndex, prompt, origin = 'phase_1_initial', extraReferences = [] }) {
  ...
  if (requirement.required && !reference && !extraReferences.length) throw new ProductReferenceUnavailableError();
  const references = [...extraReferences];
  // DB reference sebagai fallback murni: hanya digunakan jika caller tidak mengirimkan
  // extraReferences (references kosong) DAN klip ini memang membutuhkan referensi produk.
  // Ini mencegah over-correction (foto produk di klip non-bridge) sekaligus tetap
  // memberikan fallback saat productBase64 dari caller null karena alasan apapun.
