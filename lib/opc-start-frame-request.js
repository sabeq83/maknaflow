...
export async function buildOpcStartFrameRequest({ campaign, item, clipIndex, prompt, origin = 'phase_1_initial', extraReferences = [] }) {
  ...
  // Selalu tambahkan reference dari DB jika tersedia dan belum ada di array (defensive enrichment)
  if (reference && !references.includes(reference.base64DataUrl)) references.push(reference.base64DataUrl);
  const db = getDb();
