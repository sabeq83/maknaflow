...
export async function buildOpcStartFrameRequest({ campaign, item, clipIndex, prompt, origin = 'phase_1_initial', extraReferences = [] }) {
  ...
  // Tambahkan reference dari DB HANYA jika required (bridge/product clip)
  // dan belum ada di array (mencegah duplikasi dengan extraReferences dari Phase 1)
  if (requirement.required && reference && !references.includes(reference.base64DataUrl)) {
