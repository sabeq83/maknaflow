...
export async function syncItemAssetsToCloud(campaignType, itemId) {
  ...
  if (storageProvider === 'nextcloud') {
    ...
    await db.prepare(`UPDATE ${itemTable} SET drive_link = ?, upload_status = 'completed' WHERE id = ?`).run(folderShareUrl, itemId);
  } else {
    // Google Drive
...
export async function getLocalItemAssetsManifest(campaignType, itemId) {
  ...
  return {
    ...
    nextcloudUrl: item.drive_link || item.nextcloud_folder_url || null,
    driveFolderId: item.drive_link || null
  };
}
