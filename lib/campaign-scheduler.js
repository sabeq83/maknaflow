...
export async function tickCampaignScheduler() {
  ...
  logContextStorage.run(reLogFile, async () => {
  ...
  logContextStorage.run(opcLogFile, async () => {
  ...
  logContextStorage.run(instantLogFile, async () => {
  ...
  if (recipeSchedulerActive) {
    ...
    if (recipeCampaigns.length > 0) {
      logContextStorage.run(recipeLogFile, async () => {
...
async function tickStrategicCampaigns(db) {
  ...
  try {
    const activeStrategicCampaigns = await db.prepare("SELECT * FROM strategic_campaigns WHERE status = 'running'").all();
