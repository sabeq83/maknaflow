...
async function runStep(itemId, name, processor, taskKey = itemId) {
  ...
  const execute = async () => {
    ...
    await tenantContext.run(tenantId, async () => {
      ...
      console.log(`🚀 [Campaign Scheduler] Starting step [${name}] for ${progressLabel}...`);
      try {
        ...
        console.log(`✅ [Campaign Scheduler] Finished step [${name}] for ${progressLabel}:`, result);
      } catch (e) {
        console.error(`❌ [Campaign Scheduler] Error in step [${name}] for ${progressLabel}:`, e.message);
      } finally {
