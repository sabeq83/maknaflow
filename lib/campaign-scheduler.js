  ...
  for (const campaign of campaigns) {
    ...
    await tenantContext.run(tenantId, async () => {
      ...
      for (const item of items) {
        ...
        // Strict Phase 2 Approval Guard: Jika item sudah melewati analyze/scrape tetapi belum disetujui ke tahap produksi, tahan di Fase 1.
        const isReFullAutopilot = campaign.execution_mode === 'full_autopilot';
        const isReApprovedForProduction = item.workflow_status === 'production_processing' || item.review_state === 'approved';
        const isPhase1Pending = item.scrape_status === 'pending' || (item.scrape_status === 'downloaded' && item.analyze_status === 'pending');

        if (!isReFullAutopilot && !isReApprovedForProduction && !isPhase1Pending) {
          // Item telah selesai Fase 1 namun belum di-approve untuk produksi
          continue;
        }
  ...
  for (const campaign of pillarCampaigns) {
    ...
    await tenantContext.run(tenantId, async () => {
      ...
      for (const currentItem of items) {
        ...
        // Auto-Recovery: Pulihkan item yang tersangkut di 'processing' tanpa task aktif di memori (misal setelah server restart)
        if (currentItem.generation_status === 'processing' && !state.activeTasks.has(taskKey)) {
          console.warn(`⚠️ [Campaign Scheduler] OPC item #${currentItem.id} tersangkut di 'processing' tanpa task aktif. Memulihkan ke 'pending'...`);
          await updatePillarCampaignItem(currentItem.id, { generation_status: 'pending', generation_progress: null });
          currentItem.generation_status = 'pending';
          currentItem.generation_progress = null;
        }
        ...
        // Strict Phase 2 Approval Guard: Pada mode manual/hybrid review, item yang belum disetujui (workflow_status !== 'production_processing') DILARANG masuk Fase 2 (TTS/Visuals/FFmpeg/Social).
        const isOpcFullAutopilot = campaign.execution_mode === 'full_autopilot';
        const isOpcApprovedForProduction = currentItem.workflow_status === 'production_processing' || currentItem.review_state === 'approved';

        if (!isOpcFullAutopilot && !isOpcApprovedForProduction) {
          // Item masih berada di Fase 1 (menunggu regenerasi start frame, review, atau persetujuan user).
          continue;
        }

        // Additional guard for Recipe Campaign: make sure recipe data is present before TTS/visuals
        if (campaign.campaign_focus === 'recipe_campaign' && !currentItem.recipe_data_json) {
          console.log(`⛔ [Campaign Scheduler] Paused before TTS for Recipe OPC item #${currentItem.id} — recipe data missing.`);
          continue;
        }
  ...
  const execute = async () => {
    ...
    await tenantContext.run(tenantId, async () => {
      ...
      try {
        ...
        const mockJob = { id: -1 };
        const stageMap = { pillar_tts: 'tts', pillar_ffmpeg: 'ffmpeg_upload' };
        let result;
        if (stageMap[name]) {
          ...
          // Rekonsiliasi Status DB jika Stage Idempotent (Mencegah Looping Eksekusi Berulang)
          if (result?.idempotent) {
            if (name === 'pillar_tts') {
              await updatePillarCampaignItem(itemId, { tts_status: 'completed' });
            } else if (name === 'pillar_glabs') {
              const it = await db.prepare('SELECT visual_clip_paths, visual_status FROM pillar_campaign_items WHERE id=?').get(itemId);
              let clips = [];
              try { clips = JSON.parse(it?.visual_clip_paths || '[]'); } catch (_) {}
              if (Array.isArray(clips) && clips.filter(Boolean).length > 0) {
                await updatePillarCampaignItem(itemId, { visual_status: 'completed' });
              }
            } else if (name === 'pillar_ffmpeg') {
              await updatePillarCampaignItem(itemId, { ffmpeg_status: 'completed' });
            }
          }
        } else {
