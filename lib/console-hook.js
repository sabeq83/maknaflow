...
if (!globalThis.logContextStorage) {
  globalThis.logContextStorage = new AsyncLocalStorage();
}
export const logContextStorage = globalThis.logContextStorage;
...
function maybeTruncateLog(logFile) {
  try {
    ...
    if (stat.size > MAX_LOG_BYTES) {
      ...
      const truncated =
        `[AUTO-TRUNCATED: ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta', hour12: false })} — menjaga ${KEEP_LOG_LINES} baris terbaru]\n` +
        kept.join('\n') + '\n';
