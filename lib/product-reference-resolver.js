...
export const CANONICAL_PRODUCT_PHOTO_FIELDS = Object.freeze(['clean_photo_url', 'raw_photo_url']);
...
function resolveProductReferenceFromCandidates({ product, candidates, cwd, explicitField = null }) {
  ...
  let sourceField = explicitField;
  if (!sourceField) {
    if (product?.clean_photo_url && product.clean_photo_url === selectedPath) sourceField = 'clean_photo_url';
    else if (product?.raw_photo_url && product.raw_photo_url === selectedPath) sourceField = 'raw_photo_url';
    else if (ACTIVE_PHOTO_FIELDS.includes(product?.active_photo) && product[product.active_photo] === selectedPath) sourceField = product.active_photo;
    else sourceField = ACTIVE_PHOTO_FIELDS.find(field => product?.[field] === selectedPath) || 'fallback';
  }
...
export function resolveCampaignProductReference({ product, fallbackPaths = [], cwd = process.cwd() }) {
  if (!product && fallbackPaths.length === 0) return null;

  if (product) {
    const activePhoto = product.active_photo;
    if (activePhoto === 'raw_photo_url') {
      if (!product.raw_photo_url) return null;
      return resolveProductReferenceFromCandidates({
        product,
        candidates: [product.raw_photo_url],
        cwd,
        explicitField: 'raw_photo_url'
      });
    }

    if (activePhoto === 'clean_photo_url') {
      if (!product.clean_photo_url) return null;
      return resolveProductReferenceFromCandidates({
        product,
        candidates: [product.clean_photo_url],
        cwd,
        explicitField: 'clean_photo_url'
      });
    }

    // No explicit canonical selection (or unrecognized active_photo):
    // Try clean_photo_url first, then raw_photo_url, then fallbackPaths
    const candidates = [
    ...
    return resolveProductReferenceFromCandidates({ product, candidates, cwd });
  }

  return resolveProductReferenceFromCandidates({ product: null, candidates: fallbackPaths.filter(Boolean), cwd });
}
