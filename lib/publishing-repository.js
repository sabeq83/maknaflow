...
export async function listPublishingJobs(tenantId = 'default_tenant', filters = {}) {
  ...
  if (brand && brand !== 'all') {
    conditions.push(`c.account_name = $${paramIdx++}`);
    params.push(brand);
  }
  ...
  const dataQuery = `
    SELECT 
      ...
      c.nama_produk AS product_name,
      c.url_asset AS content_media_url,
      c.account_name AS brand_profile
    FROM publishing_jobs j
    LEFT JOIN publishing_accounts a ON j.account_id = a.id
    LEFT JOIN content_flow_items c ON (c.video_id = j.content_id AND c.tenant_id = j.tenant_id)
    WHERE ${whereClause}
  ...
  try {
    ...
    const selectSql = `
      SELECT 
        ...
        a.status AS account_status, a.display_name AS account_display_name
      FROM publishing_jobs j
      ...
      WHERE 
        ...
        AND (
          (j.status = 'scheduled' AND j.scheduled_at <= CURRENT_TIMESTAMP AND j.attempt_count < j.max_attempts)
          OR (j.status = 'retry_wait' AND j.next_attempt_at <= CURRENT_TIMESTAMP AND j.attempt_count < j.max_attempts)
          OR (j.status IN ('processing', 'creating_container', 'uploading_media', 'waiting_media', 'publishing', 'verifying') AND j.next_attempt_at <= CURRENT_TIMESTAMP)
        )
    ...
    // 2. Tandai job sedang diproses & update locked_at (Hanya naikkan attempt_count pada eksekusi baru, bukan polling status)
    await client.query(`
      ...
      SET 
        ...
        locked_by = $1,
        attempt_count = CASE 
          WHEN status IN ('scheduled', 'retry_wait') THEN attempt_count + 1
          ELSE attempt_count
        END,
...
export async function markPublishingResult(tenantId = 'default_tenant', jobId, {
  ...
  const updateErrorCode = lastErrorCode !== undefined;
  const updateErrorMessage = lastErrorMessage !== undefined;
  const finalScheduleId = externalScheduleId || external_schedule_id;
  const finalMediaUrl = media_url_snapshot || mediaUrlSnapshot;
  ...
  return pgQuery(`
    ...
    SET 
      ...
      last_error_code = CASE WHEN $14 = TRUE THEN $5 ELSE last_error_code END,
      last_error_message = CASE WHEN $15 = TRUE THEN $6 ELSE last_error_message END,
      next_attempt_at = $7,
      ...
      external_schedule_id = COALESCE($18, external_schedule_id),
      media_url_snapshot = COALESCE($19, media_url_snapshot),
      updated_at = CURRENT_TIMESTAMP
  ...
  `, [
    ...
    externalPostId,
    externalPermalink,
    lastErrorCode === undefined ? null : lastErrorCode,
    lastErrorMessage === undefined ? null : (lastErrorMessage ? sanitizeErrorMessage(lastErrorMessage) : null),
    nextAttemptAt,
    publishedAt,
    externalMediaStatus,
    externalObjectType,
...
/**
 * Manual retry dengan policy safety (verifikasi status, konfirmasi reconnect jika error permission).
 */
export async function retryPublishingJobWithPolicy(tenantId = 'default_tenant', jobId, { actorId = null, confirmedReconnect = false } = {}) {
  const jobRes = await pgQuery(`
  ...
  if (jobRes.rowCount === 0) {
    const notFoundErr = new Error('Job tidak ditemukan.');
    notFoundErr.status = 404;
    throw notFoundErr;
  }

  const job = jobRes.rows[0];
  if (!['failed', 'needs_review', 'cancelled'].includes(job.status)) {
    const invalidErr = new Error('Hanya job gagal, butuh tinjauan, atau dibatalkan yang dapat dicoba ulang.');
    invalidErr.status = 400;
    throw invalidErr;
  }

  // Jika job gagal karena Facebook permission di Repliz, tolak kecuali admin mengonfirmasi reconnect
  if (job.last_error_code === 'REPLIZ_FACEBOOK_PERMISSION_REQUIRED' && !confirmedReconnect) {
    const permErr = new Error('Koneksi Facebook Page di Repliz memerlukan otorisasi ulang (reconnect). Silakan hubungkan ulang akun di Repliz dan centang konfirmasi sebelum mencoba lagi.');
    permErr.status = 409;
    permErr.code = 'RECONNECT_CONFIRMATION_REQUIRED';
    throw permErr;
  }
  ...
  await appendPublishingAttempt({
    tenantId,
    jobId,
    attemptNumber: (job.attempt_count || 0) + 1,
    correlationId: `manual_retry_${Date.now()}`,
    stage: 'manual_retry',
    outcome: 'scheduled',
    sanitizedMessage: `Manual retry diinisiasi oleh ${actorId || 'operator'}.`
  });
...
/**
 * Manual retry legacy fallback.
 */
export async function retryPublishingJob(tenantId = 'default_tenant', jobId) {
  return retryPublishingJobWithPolicy(tenantId, jobId, { confirmedReconnect: true });
}

/**
 * Simpan provider state allowlisted dan update pesan/kode error jika ada.
 */
export async function recordReplizProviderState(tenantId = 'default_tenant', jobId, safeState = {}) {
  const stateJson = JSON.stringify(safeState || {});
  const res = await pgQuery(`
    UPDATE publishing_jobs
    SET
      provider_state_json = $1::jsonb,
      provider_stage = COALESCE($2, provider_stage),
  ...
  `, [
    stateJson,
    safeState.providerStatus || safeState.status || null,
    safeState.errorCode || safeState.providerErrorCode || null,
    safeState.errorMessage || null,
    jobId,
    tenantId
  ]);
...
/**
 * Catat status kesehatan (health) akun publishing (misal isConnected dari Repliz API).
 */
export async function recordPublishingAccountHealth(tenantId = 'default_tenant', accountId, health = {}) {
  ...
  `, [
    status,
    health.errorCode || health.lastErrorCode || null,
    health.message || health.lastErrorMessage || null,
    accountId,
    tenantId
  ]);
  return res.rows[0] || null;
...
/**
 * Simpan atau perbarui record file staging media (Google Drive).
 */
export async function saveMediaStaging({
  tenantId = 'default_tenant',
  jobId,
  ...
  const id = `stg_${Date.now()}_${crypto.randomBytes(3).toString('hex')}`;
  const res = await pgQuery(`
    INSERT INTO publishing_media_staging (
      id, tenant_id, job_id, provider, external_file_id, public_url,
      status, verified_at, expires_at, last_error, updated_at
...
/**
 * Ambil record media staging berdasarkan jobId.
 */
export async function getMediaStagingByJob(tenantId = 'default_tenant', jobId, provider = 'google_drive') {
  const res = await pgQuery(`
    SELECT * FROM publishing_media_staging
    WHERE tenant_id = $1 AND job_id = $2 AND provider = $3
  `, [tenantId, jobId, provider]);
  return res.rows[0] || null;
}

/**
 * Ambil daftar file media staging yang telah melewati masa retensi (expires_at <= NOW)
 * dan parent job-nya berstatus terminal (published, failed, cancelled, draft_created).
 */
export async function listExpiredMediaStaging(limit = 20) {
  const res = await pgQuery(`
    SELECT s.*, j.status as job_status
    FROM publishing_media_staging s
    JOIN publishing_jobs j ON j.id = s.job_id
    WHERE s.deleted_at IS NULL
      AND s.expires_at <= CURRENT_TIMESTAMP
      AND j.status IN ('published', 'failed', 'cancelled', 'draft_created')
...
/**
 * Tandai record media staging sebagai telah dihapus.
 */
export async function markMediaStagingDeleted(id) {
  const res = await pgQuery(`
    UPDATE publishing_media_staging
