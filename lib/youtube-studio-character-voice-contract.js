...
export function validateVoiceCasting(input, audioConfig) {
  ...
  if (kind === 'tts') {
    if (!['minimax', 'google_tts'].includes(provider)) {
      throw new Error(`tts binding does not support provider ${provider}`);
    }
  ...
  if (audioConfig) {
    const expectedKind = audioConfig.audio_production_mode === 'native_scene_audio' ? 'flow_native' : 'tts';
    if (kind !== expectedKind) {
      throw new Error(`${audioConfig.audio_production_mode} requires ${expectedKind} voice bindings`);
    }
    if (provider !== audioConfig.provider) {
      throw new Error(`Voice binding provider ${provider} does not match channel provider ${audioConfig.provider}`);
    }
  }
