...
export async function generateHybridPromptMatrix({ episode, script, profile, visualIdentity, universe, kbSnapshot }) {
  ...
  const prompt = `
    ...
    1. Tentukan generation_mode untuk setiap shot dari: 't2i_i2v', 't2v'.
       - Gunakan 't2i_i2v' jika scene memiliki recurring subject, karakter, atau lokasi spesifik yang membutuhkan kontinuitas visual.
       - Gunakan 't2v' untuk shoot atmospheric/establishing, diagram, peta, bagan, overlay teks, atau visual generic lainnya yang tidak memerlukan aktor/karakter presisi tinggi.
  ...
  return {
    ...
    scenes: plan.scenes.map((scene, scIdx) => ({
      ...
      shots: (scene.shots || []).map((shot, shIdx) => {
        const generation_mode = shot.generation_mode || 't2v';
