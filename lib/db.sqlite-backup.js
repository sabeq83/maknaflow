...
function initSchema() {
  ...
  const migrateContentPlannersFields = () => {
    try {
      ...
      if (!cpCols.includes('is_archived')) db.prepare('ALTER TABLE content_planners ADD COLUMN is_archived INTEGER DEFAULT 0').run();
      if (!cpCols.includes('archived_at')) db.prepare('ALTER TABLE content_planners ADD COLUMN archived_at TEXT').run();
