...
export async function createCampaignProgram(user, brandId, data) {
  ...
  await pgQuery(
    `INSERT INTO affiliate_programs (
      ...
      production_target, status, created_by, updated_by
...
export async function updateCampaignProgram(user, brandId, programId, data) {
  ...
  await pgQuery(
    `UPDATE affiliate_programs
     ...
     WHERE id = $12 AND tenant_id = $13 AND brand_profile_id = $14`,
    [
      data.name || 'Untitled Campaign Program',
      data.description || null,
      data.objective || null,
...
export async function addProductsToCampaignProgram(user, brandId, programId, productIds) {
  ...
  for (const productId of productIds) {
    ...
    const safeImage = resolveSafeProductImage(
      row.photo_url || row.clean_photo_url || row.cleaned_photo_url || row.generated_photo_url || row.raw_photo_url
    );
