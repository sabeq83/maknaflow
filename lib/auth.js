...
export async function loadSessionsCache() {
  try {
    ...
    for (const sess of sessions) {
      const userObj = await buildUserObject(db, sess);
...
export async function loginUser(username, password) {
  ...
  const user = await db.prepare("SELECT * FROM users WHERE LOWER(username) = ? AND status = 'active'").get(cleanUsername);
  if (!user) {
  ...
  const userObj = await buildUserObject(db, user);
...
export async function buildUserObject(db, user) {
  ...
  return {
    ...
    email: user.email,
    role: user.role,
    tenantId: user.tenant_id || 'default_tenant',
    menuPermissions,
