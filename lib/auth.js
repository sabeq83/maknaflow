...
// Automatically load sessions cache at boot unless disabled for testing
...
export async function buildUserObject(db, user) {
  ...
  const assignedBrands = user.role === 'superadmin' ? [] : (
    user.role === 'admin'
      ? await db.prepare('SELECT id as brand_id, brand_name FROM brand_profiles WHERE tenant_id = ?').all(user.tenant_id || 'default_tenant')
      : await db.prepare(`
  ...
  let tenantDisabledMenus = [];
  if (user.role !== 'superadmin' && user.tenant_id) {
    try {
      const tRes = await pgQuery('SELECT disabled_menus FROM tenants WHERE id = $1', [user.tenant_id]);
      if (tRes.rows[0]?.disabled_menus) {
        const raw = tRes.rows[0].disabled_menus;
        tenantDisabledMenus = Array.isArray(raw) ? raw : (typeof raw === 'string' ? JSON.parse(raw) : []);
      }
    } catch {}
  }

  const resolvedTenantId = user.role === 'superadmin' ? '__none__' : (user.tenant_id || 'default_tenant');
  return {
    id: user.id,
...
export async function getCurrentUserAsync(req) {
  let token = null;
  ...
  if (token) {
    let userFromToken = getSessionUser(token);
    if (!userFromToken) {
      try {
        const now = new Date().toISOString();
        const sessRes = await pgQuery(`
          SELECT s.token, s.expires_at, u.id, u.username, u.email, u.role, u.status, u.tenant_id
          FROM sessions s
          JOIN users u ON s.user_id = u.id
          LEFT JOIN tenants t ON u.tenant_id = t.id
          WHERE s.token = $1 AND s.expires_at > $2 AND u.status = 'active'
            AND (u.role = 'superadmin' OR t.status = 'active')
          LIMIT 1
        `, [token, now]);
        if (sessRes.rows[0]) {
          ...
          const contextTenant = sess.role === 'superadmin' ? '__none__' : (sess.tenant_id || 'default_tenant');
          userFromToken = await tenantContext.run(contextTenant, () => buildUserObject(getDb(), sess));
          activeSessionsCache[token] = userFromToken;
        }
      } catch (e) {
        console.warn('[getCurrentUserAsync DB Fallback Error]:', e.message);
      }
    }
    if (userFromToken) {
      tenantContext.enterWith(userFromToken.tenantId || 'default_tenant');
...
export function withTenantContext(handler) {
  return async (request, ...args) => {
    try {
      const user = await getCurrentUserAsync(request) || getCurrentUser(request);
      if (!user) {
...
export function withYouTubeStudioAccess(mode, handler) {
  return withTenantContext(async (request, context, user) => {
    const isMenuDisabled = user.tenantDisabledMenus?.includes('youtube_studio');
    const hasPermission = user.menuPermissions?.includes('youtube_studio');
    
    if (isMenuDisabled || !hasPermission) {
      return new Response(JSON.stringify({ success: false, error: 'Forbidden' }), {
        status: 403,
        headers: { 'content-type': 'application/json' }
      });
