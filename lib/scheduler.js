...
async function getProcessors() {
  return await import('./scheduler-processors.js');
}
...
const QUEUE_DEFS = {
  ...
  youtube_production:   { label: 'YouTube Studio Production', cost: 1 },
  youtube_publishing:   { label: 'YouTube Studio Publishing', cost: 0 },
};
      ...
      await tenantContext.run(tenantId, async () => {
        try {
          ...
          const processorMap = {
            ...
            youtube_production: async (payload) => {
              const { processYouTubeRenderJob } = await import('./youtube-studio-production-worker.js');
              return await processYouTubeRenderJob(payload.job_id);
            },
            youtube_production_asset: async (payload) => {
              const { processProductionJob } = await import('./youtube-studio-production-worker.js');
              return await processProductionJob(payload.job_id);
            },
            youtube_production_assembly: async (payload) => {
              const { processYouTubeAssemblyJob } = await import('./youtube-studio-production-worker.js');
              return await processYouTubeAssemblyJob(payload);
            },
            youtube_production_final: async (payload) => {
              const { processYouTubeFinalJob } = await import('./youtube-studio-production-worker.js');
              return await processYouTubeFinalJob(payload);
            },
            youtube_publishing: async (payload) => {
              const { uploadYouTubeDraft } = await import('./youtube-studio-publishing-service.js');
              return await uploadYouTubeDraft({ episodeId: payload.episode_id });
            }
          };
