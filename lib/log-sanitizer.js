...
const SANITIZER_RULES = [
  ...
  { pattern: /\[(?:OPC|RE) Analyzer T2I\] Mode hybrid_lock aktif\. Memulai pre-rendering T2I untuk seluruh (\d+) klip\.\.\./i, replace: '🎨 Mode Hybrid Lock aktif. Memulai pembuatan gambar start frame untuk $1 klip...' },
  { pattern: /\[(?:OPC|RE) Analyzer T2I\] ✅ Product Base64 resolved successfully for bridging clip reference\./i, replace: '📸 Foto produk studio terverifikasi berhasil dimuat sebagai referensi visual.' },
  { pattern: /\[(?:OPC|RE) Analyzer T2I\] Using Pola T2I: (.*)/i, replace: '⚡ Mengaktifkan mode pemrosesan gambar paralel (Pola: $1).' },
  { pattern: /\[(?:OPC|RE) Analyzer T2I\] (?:\[(?:Threading|Sequential)\]\s*)?Submitting T2I task for clip (\d+)\.\.\./i, replace: '🚀 Mengirimkan permintaan pembuatan gambar start frame Klip $1 ke AI Studio...' },
  { pattern: /\[(?:OPC|RE) Analyzer T2I\] (?:\[(?:Threading|Sequential)\]\s*)?T2I task (.*?) submitted for clip (\d+)\./i, replace: '🚀 Permintaan gambar start frame Klip $2 terdaftar di antrean AI Studio.' },
  { pattern: /\[(?:OPC|RE) Analyzer T2I\] (?:\[(?:Threading|Sequential)\]\s*)?Starting batch polling for (\d+) T2I tasks\.\.\./i, replace: '⏳ Memantau proses rendering paralel untuk $1 gambar adegan...' },
  { pattern: /\[(?:OPC|RE) Analyzer T2I\] (?:\[(?:Threading|Sequential)\]\s*)?T2I task (.*?) for clip (\d+) completed!/i, replace: '✅ Gambar start frame Klip $2 selesai dibuat oleh AI.' },
  { pattern: /\[(?:OPC|RE) Analyzer T2I\] (?:\[(?:Threading|Sequential)\]\s*)?Downloading start frame for clip (\d+) from (.*)/i, replace: '💾 mendownload SF klip $1' },
