...
import { executeIdempotentStage } from './pillar-stage-execution-service.js';
...
export async function syncApprovedOpcItemToContentFlow({ itemId }) {
  ...
  return executeIdempotentStage({ itemId: item.id, stage: 'contentflow', revision: Number(item.start_frame_revision || 1) }, async () => {
