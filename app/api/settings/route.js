  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        webhook_api_key: await getSetting('webhook_api_key') ? '••••••••' + await getSetting('webhook_api_key').slice(-6) : '',
        has_webhook_key: !!await getSetting('webhook_api_key'),
        ...
        nextcloud_app_password: await getSetting('nextcloud_app_password') || '',
        ...
        fb_page_token: await getSetting('fb_page_token') ? '••••••••' + await getSetting('fb_page_token').slice(-6) : '',
        ...
        contentflow_api_key: await getSetting('contentflow_api_key') ? '••••••••' + await getSetting('contentflow_api_key').slice(-6) : '',
...
export async function POST(request) {
  try {
    ...
    if (nextcloud_app_password !== undefined) await setSetting('nextcloud_app_password', nextcloud_app_password);
    ...
    if (fb_page_token) await setSetting('fb_page_token', fb_page_token);
    ...
    // Content Flow Direct Ingestion API
    if (contentflow_api_key) await setSetting('contentflow_api_key', contentflow_api_key);
