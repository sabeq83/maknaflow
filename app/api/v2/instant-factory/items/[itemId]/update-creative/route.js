...
export async function PATCH(request, { params }) {
  try {
    ...
    const item = await db.prepare('SELECT * FROM instant_campaign_items WHERE id = ?').get(itemId);
    if (!item) {
    ...
    await updateInstantCampaignItem(itemId, updateFields);

    // Make sure campaign status is 'running' if it's currently paused and we did a reset
    if (resetStatus) {
      const campaign = await db.prepare('SELECT * FROM instant_campaigns WHERE id = ?').get(item.campaign_id);
      if (campaign && campaign.status !== 'running') {
        await updateInstantCampaign(item.campaign_id, { status: 'running' });
      }
    }
